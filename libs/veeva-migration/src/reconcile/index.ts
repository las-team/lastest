...
const LOADED_STATES: readonly RowState[] = [
  "loaded_created",
  "loaded_updated",
  "loaded_unchanged",
];
...
function agg(hashes: Iterable<string>): string {
  let count = 0;
  // `BigInt(0)`, not the `0n` literal: the literal needs a checker target of
  // ES2020, and one call here is cheaper than moving the whole app's tsconfig
  // off ES2017 for it.
  let sum = BigInt(0);
...
export function canonicalValue(v: unknown): unknown {
  ...
  if (typeof v === "string") {
    ...
    if (/^-?\d+(\.\d+)?$/.test(s) && s.length < 16)
      return canonicalValue(Number(s));
...
export class DefaultReconciler implements Reconciler {
  ...
  ): AsyncIterable<PayloadRow> | undefined {
  ...
  private vaultCountQuery(input: ReconcileInput): string | undefined {
    ...
    const pred = input.vaultCountryPredicate
      ? ` AND ${input.vaultCountryPredicate}`
      : "";
  ...
  async reconcileUnit(input: ReconcileInput): Promise<ReconcileResult> {
    ...
    const counts = await store.rowResults.countByState(
      runId,
      unit.objectKey,
      unit.country,
    );
    const failedByType = await store.rowResults.countFailedByType(
      runId,
      unit.objectKey,
      unit.country,
    );
    const skippedRows = await store.rowResults.query({
      runId,
    ...
    for (const r of skippedRows)
      skippedByReason[r.errorType ?? "rule"] =
        (skippedByReason[r.errorType ?? "rule"] ?? 0) + 1;
    ...
    const transformed =
      created + updated + unchanged + failed + pendingFk + c("transformed");
    const extracted = input.manifest
      ? input.manifest.extractedLive
      : transformed + skipped;
    ...
    if (payloads) {
      ...
      for (let i = 0; i < ids.length; i += 500) {
        ...
        for (const id of ids.slice(i, i + 500)) {
          const r = got.get(id);
          if (r && !r.deletedAt && !r.dryRun && r.sourceHash)
            tgt.push(r.sourceHash);
        }
      }
      aggHashSrc = agg(src);
      aggHashTgt = agg(tgt);
    ...
    log.info(
      {
        status: row.status,
        sfdc_scope_count: row.sfdcScopeCount,
        extracted,
        closure,
        created,
        updated,
    ...
    return {
      ...
      findings,
      pass: gate.pass,
      orphanFks: orphanFks.filter((o) => o.count > 0),
    };
  }
  ...
  async sample(input: ReconcileInput): Promise<SampleDiff[]> {
    ...
    for await (const p of payloads) {
      ...
      if (
        !longest ||
        name.length > String(longest.payload.name__v ?? "").length
      )
        longest = p;
    ...
    for (
      let i = 0;
      picked.size < input.sampleSize && lists.some((l) => i < l.length);
      i++
    )
      for (const l of lists)
        if (i < l.length && picked.size < input.sampleSize)
          picked.set(l[i].sfdcId, l[i]);
    for (const b of [longest, nonAscii]) if (b) picked.set(b.sfdcId, b);
    if (!picked.size) return [];

    const rows = [...picked.values()];
    const idRows = await this.deps.store.idMap.bulkGet(
      input.unit.objectKey,
      rows.map((r) => r.sfdcId),
    );
    ...
    for (const clause of vqlInClauses("id", [...byVaultId.keys()])) {
      ...
      for await (const page of this.deps.vault.vql(q))
        for (const rec of page.data) actualById.set(String(rec.id), rec);
    }
    const now = this.now();
    for (const [vaultId, r] of byVaultId) {
      ...
      for (const f of fieldList) {
        ...
        if (isDeferredValue(r.payload[f]) && expected[f] === undefined)
          continue; // unresolved optional ref
      ...
      if (clean)
        await this.deps.store.idMap.setVerified(
          input.unit.objectKey,
          r.sfdcId,
          hashObject(expected),
          now,
    ...
    if (diffs.length)
      await this.deps.store.findings.add(input.runId, [
        {
          severity: "warning",
          code: "RECON_SAMPLE_DIFF",
          objectKey: input.unit.objectKey,
          country: input.unit.country,
          detail: {
            sampled: byVaultId.size,
            diffs: diffs.length,
  ...
  async orphanFks(
    ...
    for (const f of input.mapping.fields) {
      ...
      if (!input.target.metadata.fields[f.target] || seen.has(f.target))
        continue;
      ...
      const pred = input.vaultCountryPredicate
        ? ` AND ${input.vaultCountryPredicate}`
        : "";
      const count = await this.deps.vault.vqlCount(
        `SELECT id FROM ${input.target.targetObject} WHERE ${f.target} = null AND ${legacy} != null${pred} PAGESIZE 0`,
      );
  ...
  async keySet(
    ...
    for await (const r of this.deps.store.idMap.iterate(
      input.unit.objectKey,
      input.unit.country,
    ))
      if (!r.deletedAt && !r.mergedInto && !r.dryRun)
        rows.push({ sfdcId: r.sfdcId, legacy: r.sfdcId });
    const missingInVault: string[] = [];
    if (legacy && rows.length) {
      const present = new Set<string>();
      for (const clause of vqlInClauses(
        legacy,
        rows.map((r) => r.sfdcId),
      )) {
        const q = `SELECT id, ${legacy} FROM ${input.target.targetObject} WHERE ${clause}`;
        for await (const page of this.deps.vault.vql(q))
          for (const rec of page.data) present.add(String(rec[legacy]));
      }
      for (const r of rows)
        if (!present.has(r.sfdcId)) missingInVault.push(r.sfdcId);
    }
    ...
    if (rows.length) {
      ...
      for (let i = 0; i < ids.length; i += 400)
        for await (const rec of this.deps.sfdc.queryIds(
          input.mapping.sourceObject,
          ids.slice(i, i + 400),
          ["Id", "IsDeleted"],
        ))
          if (rec.IsDeleted !== true && rec.IsDeleted !== "true")
            alive.add(rec.Id);
    ...
    if (missingInVault.length || goneInSource.length)
      await this.deps.store.findings.add(input.runId, [
        {
          severity: "warning",
          code: "RECON_KEYSET_MISMATCH",
          objectKey: input.unit.objectKey,
          country: input.unit.country,
          detail: {
            missingInVault: missingInVault.length,
            goneInSource: goneInSource.length,
            examples: {
              missingInVault: missingInVault.slice(0, 10),
              goneInSource: goneInSource.slice(0, 10),
            },
          },
        },
      ]);
  ...
  > {
    ...
    const edgesByChild = new Map<
      string,
      Array<{ field: string; targetObjectKey: RefKey; targetSfdcId: string }>
    >();
    const refs = new Map<RefKey, Set<string>>();
    for (const c of children) {
      const edges = await this.deps.store.fkIndex.get(
        input.unit.objectKey,
        c.sfdcId,
      );
      ...
      for (const e of kept)
        (
          refs.get(e.targetObjectKey) ??
          refs.set(e.targetObjectKey, new Set()).get(e.targetObjectKey)!
        ).add(e.targetSfdcId);
    }
    if (!edgesByChild.size) return out;
    const index = await RefIndex.build(this.deps.store, refs);
    const fieldSet = new Set<string>();
    for (const edges of edgesByChild.values())
      for (const e of edges) fieldSet.add(e.field);
    const byVault = new Map(
      children
        .filter((c) => edgesByChild.has(c.sfdcId))
        .map((c) => [c.vaultId, c.sfdcId] as const),
    );
    ...
    for (const clause of vqlInClauses("id", [...byVault.keys()])) {
      ...
      for await (const page of this.deps.vault.vql(q))
        for (const rec of page.data) actual.set(String(rec.id), rec);
    }
      ...
      for (const e of edgesByChild.get(sfdcId) ?? []) {
        ...
        if (String(a ?? "") !== String(expected))
          out.push({
            ...
            actual: a === null || a === undefined ? undefined : String(a),
          });
      }
    }
    if (out.length)
      await this.deps.store.findings.add(input.runId, [
        {
          severity: "warning",
          code: "RECON_FK_INCONSISTENT",
          objectKey: input.unit.objectKey,
          country: input.unit.country,
          detail: { rows: out.length, examples: out.slice(0, 10) },
          count: out.length,
        },
      ]);
    return out;
