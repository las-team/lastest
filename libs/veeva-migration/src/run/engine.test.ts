...
describe("DefaultRunEngine", () => {
  ...
  it("stamps the caller's actor on every audit entry", async () => {
    const summary = await init();
    const entries = await h.store.auditLog.list({ runId: summary.runId });
    expect(entries.length).toBeGreaterThan(0);
    // The whole point of `RunContext.actor`: before it existed this read
    // `process.env.USER`, so a regulated cutover's audit log named the server's
    // OS account rather than the person who started the run.
    expect([...new Set(entries.map((e) => e.actor))]).toEqual(["test-actor"]);
  });

  it("a cancelled run exits `aborted`, and its units are not marked failed", async () => {
    const controller = new AbortController();
    controller.abort();
    const summary = await h.engine.execute({
      mode: "init",
      config: h.config,
      context: { ...h.context, signal: controller.signal },
      wave: "w1",
    });
    expect(summary.exitCode).toBe(EXIT_CODES.aborted);
    // Not `failed`: `retry-failed` must not pick up work an operator stopped.
    expect(summary.units.filter((u) => u.status === "failed")).toEqual([]);
    const findings = await h.store.findings.list(summary.runId);
    const cancelled = findings.filter((f) => f.code === "RUN_CANCELLED");
    expect(cancelled).toHaveLength(1);
    expect(cancelled[0]!.severity).toBe("warning");
    const run = await h.store.runs.get(summary.runId);
    expect(run?.status).toBe("aborted");
  });

  it("cancelling mid-run stops before the following units", async () => {
    const baseline = await h.engine.plan({
      mode: "init",
      config: h.config,
      context: h.context,
      wave: "w1",
    });
    const total = baseline.steps.reduce((n, step) => n + step.units.length, 0);
    expect(total).toBeGreaterThan(1);

    const controller = new AbortController();
    const inner = makeHarness(runDir).deps.extractor!;
    let extracted = 0;
    const h2 = makeHarness(runDir, {
      extractor: {
        ...inner,
        extractUnit: async (unit, plan) => {
          extracted++;
          controller.abort();
          return inner.extractUnit(unit, plan);
        },
      },
    });
    const summary = await h2.engine.execute({
      mode: "init",
      config: h2.config,
      context: { ...h2.context, signal: controller.signal },
      wave: "w1",
    });
    expect(summary.exitCode).toBe(EXIT_CODES.aborted);
    // The unit already in flight finishes; the ones after it never start, so a
    // cancelled run costs at most the step it was in.
    expect(extracted).toBeLessThan(total);
  });
});
