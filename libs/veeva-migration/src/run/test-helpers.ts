...
export interface Harness {
  /**
   * The caller context every `execute()`/`plan()` call needs. Built from the
   * harness's temp `runDir` with a fixed actor, so a test that asserts on the
   * audit log sees a stable name instead of whoever's machine ran it.
   */
  context: RunContext;
  sfdc: FakeSfdcClient;
