    ...
    const out = lines([
      {
        type: "wait",
        timestamp: 1,
        data: { waitType: "duration", durationMs: 180000 },
      },
    ]);
    ...
    const out = lines([
      {
        type: "wait",
        timestamp: 1,
        data: { waitType: "duration", durationMs: 1500.7 },
      },
    ]);
    ...
    const out = lines([
      {
        type: "wait",
        timestamp: 1,
        data: { waitType: "selector", selector: "#status" },
      },
    ]);
    expect(out).toEqual([
...
describe("eventsToCodeLines — cursor tracking + selectors", () => {
  ...
  it("uses locateWithFallback (not mouse.down/up) when a click with selectors follows mouse-down/up", () => {
    const out = lines([
      {
        type: "mouse-down",
        timestamp: 10,
        data: { coordinates: { x: 100, y: 200 }, button: 0 },
      },
      {
        type: "mouse-up",
        timestamp: 20,
    ...
    expect(joined).toContain(
      'await locateWithFallback(page, [{"type":"role-name","value":"role=button[name=\\"OK\\"]"}], \'click\', null, {"x":100,"y":200}',
    );
  });
    ...
    const out = lines([
      {
        type: "mouse-down",
        timestamp: 10,
        data: { coordinates: { x: 50, y: 60 }, button: 0 },
      },
      {
        type: "mouse-up",
        timestamp: 20,
      ...
      },
      {
        type: "action",
        timestamp: 30,
        data: { action: "click", coordinates: { x: 50, y: 60 } },
      },
    ]);
    const joined = out.join("\n");
  ...
  it("tolerates cursor-move events between mouse-down/up and the click action", () => {
    const out = lines([
      {
        type: "mouse-down",
        timestamp: 10,
        data: { coordinates: { x: 10, y: 20 }, button: 0 },
      },
      {
        type: "cursor-move",
        timestamp: 12,
        data: { coordinates: { x: 11, y: 21 } },
      },
      {
        type: "mouse-up",
        timestamp: 15,
    ...
    expect(joined).not.toContain("page.mouse.up(");
    expect(joined).toContain(
      'locateWithFallback(page, [{"type":"css-path","value":"button.primary"}], \'click\'',
    );
  });
    ...
    const out = lines([
      {
        type: "mouse-down",
        timestamp: 10,
        data: { coordinates: { x: 1, y: 2 }, button: 0, modifiers: ["Shift"] },
      },
      {
        type: "mouse-up",
        timestamp: 20,
