...
export class RunnerClient {
  ...
  async stop(): Promise<void> {
    ...
    try {
      ...
      console.log(
        "[Stop] Server notification skipped (timeout or unreachable)",
      );
    }
  ...
  private async connect(): Promise<boolean> {
    try {
      ...
      if (response.status === 409) {
        ...
        console.error(
          "❌ Connection rejected: another runner instance is already connected with this token",
        );
      ...
      console.log(
        `Capabilities: ${data.capabilities?.join(", ") || "run, record"}`,
      );
    ...
    while (this.running) {
      ...
      if (this.pendingResults.length > 0) {
        ...
        if (this.pendingResults.length > 100) {
          console.warn(
            `[Retry] Dropping ${this.pendingResults.length - 100} oldest pending results`,
          );
      ...
      try {
        ...
        if (response.status === 409) {
          ...
          console.error(
            "❌ Session conflict: another runner instance has connected with this token",
          );
        ...
        if (response.ok) {
          ...
          if (data.commands && data.commands.length > 0) {
            console.log(
              `[Heartbeat] Received ${data.commands.length} commands:`,
              data.commands.map((c: Message) => c.type),
            );
            this.commandQueue.push(...data.commands);
            this.processCommandQueue();
          }
    ...
    while (this.commandQueue.length > 0) {
      ...
      if (cmd.type === "command:run_test") {
        ...
        if (this.activeTestIds.has(testId)) {
          console.log(
            `[Queue] Skipping duplicate run_test for testId ${testId} (cmd ${testCmd.id.slice(0, 8)})`,
          );
          continue;
        }
  ...
  private async handleCancelTest(command: CancelTestCommand): Promise<void> {
    ...
    if (aborted) {
      ...
      console.log(
        `Test run ${testRunId} is not the current test, ignoring cancel`,
      );
    } else {
  ...
  private async handleRunTest(command: RunTestCommand): Promise<void> {
    ...
    if (this.baseUrl) {
      console.log(
        `[Test ${testId}] Overriding targetUrl: ${command.payload.targetUrl} → ${this.baseUrl}`,
      );
    ...
    try {
      console.log(
        `[Test ${testId}] Starting test execution (commandId: ${command.id.slice(0, 8)}, timeout: ${command.payload.timeout}ms)`,
      );
      ...
      const result = await this.runner.runTest(
        ...
        (step, progress) => {
          ...
          const progressMsg = createMessage<TestProgressResponse>(
        ...
        (event) => {
          ...
          const stepMsg = createMessage<StepEventResponse>(
            "response:step_event",
            {
              ...event,
              correlationId: command.id,
      ...
      console.log(
        `[Test ${testId}] runTest returned: status=${result.status}, screenshots=${result.screenshots.length}, duration=${result.durationMs}ms`,
      );

      // Upload screenshots BEFORE result so they're in DB when executor sees "completed"
      if (result.screenshots.length > 0) {
        console.log(
          `[Test ${testId}] Uploading ${result.screenshots.length} screenshots in parallel...`,
        );
        const screenshotResults = await Promise.all(
          result.screenshots.map((screenshot) => {
            const screenshotMsg = createMessage<ScreenshotUploadResponse>(
              "response:screenshot",
              {
                correlationId: command.id,
            ...
            return this.sendMessage(screenshotMsg).then((sent) => {
              if (!sent) {
                console.warn(
                  `[Test ${testId}]   Screenshot send failed, queuing for retry`,
                );
        ...
        console.log(
          `[Test ${testId}] All screenshots uploaded (${screenshotResults.filter(Boolean).length}/${result.screenshots.length} succeeded)`,
        );
      }

      // Send result AFTER screenshots so server has them when it sees pass/fail
      const resultMsg = createMessage<TestResultResponse>(
      ...
      this.activeTasks--;
      console.log(
        `[Test ${testId}] Cleanup done (activeTasks: ${this.activeTasks})`,
      );
  ...
  private async handleRunSetup(command: RunSetupCommand): Promise<void> {
    ...
    if (this.baseUrl) {
      console.log(
        `[Setup] Overriding targetUrl: ${command.payload.targetUrl} → ${this.baseUrl}`,
      );
      command.payload.targetUrl = this.baseUrl.replace(/\/+$/, "");
    }
    ...
    try {
      console.log(
        `[Setup] Starting setup execution (commandId: ${command.id.slice(0, 8)}, timeout: ${command.payload.timeout}ms)`,
      );

      const result = await this.runner.runSetup(command.payload);

      console.log(
        `[Setup] runSetup returned: status=${result.status}, duration=${result.durationMs}ms`,
      );

      const resultMsg = createMessage<SetupResultResponse>(
    ...
    try {
      ...
      await this.recorder.start(
        ...
        (events: RecordingEventData[]) => {
          const msg = createMessage<RecordingEventResponse>(
        ...
        () => {
          ...
          const msg = createMessage<RecordingStoppedResponse>(
  ...
  private async handleCaptureScreenshot(
    ...
    try {
      ...
      if (screenshot) {
        const msg = createMessage<ScreenshotUploadResponse>(
          "response:screenshot",
          {
            correlationId: command.id,
