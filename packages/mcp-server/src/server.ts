...
export function createServer(client: LastestClient): McpServer {
  ...
  server.tool(
    ...
    async (
      ...
      const job = (await client.getJob(params.jobId)) as Record<
        string,
        unknown
      >;
      const response: ToolResponse = {
  ...
  server.tool(
    ...
    async (
      ...
      const repo = (await client.getRepo(params.repositoryId)) as Record<
        string,
        unknown
      >;
      const response: ToolResponse = {
  ...
  server.tool(
    ...
    "Create a new local repository in the current team. Use this when you need a fresh workspace for tests without connecting to GitHub/GitLab. Pass `baseUrl` to point the repo at an external app (e.g. https://staging.example.com) so generated tests target the right origin instead of localhost:3000.",
    {
      ...
      baseUrl: z
        ...
        .optional()
        .describe(
          "Base URL passed to `test(page, baseUrl, ...)` for this repo (e.g. https://staging.example.com). Defaults to http://localhost:3000.",
        ),
    },
  ...
  server.tool(
    ...
    "Update a repository's name, default branch, selected branch, or base URL. `baseUrl` is the value passed to `test(page, baseUrl, ...)` — set it to point a repo at an external app instead of the default http://localhost:3000.",
    {
      ...
      selectedBranch: z
        .string()
        .optional()
        .describe("Branch selected for test runs"),
      baseUrl: z
        .string()
        .url()
        .optional()
        .describe(
          "Base URL for tests in this repo (e.g. https://staging.example.com).",
        ),
    },
  ...
  server.tool(
    ...
    async (
      ...
      const areas = (await client.listAreas(params.repositoryId)) as Array<
        Record<string, unknown>
      >;
      const response: ToolResponse = {
        status: "ok",
  ...
  server.tool(
    ...
    "Update a functional area's name, description, or parent.",
    {
      ...
      parentId: z
        .string()
        .optional()
        .describe("New parent area ID (pass empty string to clear)"),
    },
    withActivityReporting(client, "lastest_update_area", async (params) => {
      ...
      if (rest.description !== undefined)
        cleanUpdates.description = rest.description as string;
      if (rest.parentId !== undefined)
        cleanUpdates.parentId = (rest.parentId as string) || null;
      const result = (await client.updateArea(
        functionalAreaId as string,
        cleanUpdates,
      )) as Record<string, unknown>;
      const response: ToolResponse = {
        status: "updated",
  ...
  server.tool(
    ...
    async (
      ...
      const tests = (await client.listTestsByArea(
  ...
  server.tool(
    ...
    async (
      ...
      const test = (await client.getTest(params.testId)) as Record<
        string,
        unknown
      >;
      const response: ToolResponse = {
  ...
  server.tool(
    ...
    "Update a test's name, code, URL, functional area, setup wiring, or runtime overrides. Pass `setupTestId` to point this test at another test for setup (the most common pattern when two tests share a login), or `setupScriptId` to point at a saved setup script — these two are mutually exclusive. Use `setupOverrides` / `teardownOverrides` to skip specific default steps or inject extra ones (test / script / storage-state) just for this test. `playwrightOverrides` lets the test self-configure runtime knobs (browser, timeouts, error modes, baseUrl) without touching the repo-wide settings. `diffOverrides` and `stabilizationOverrides` accept partial blocks that fall through to repo defaults. Pass `null` for any override block to clear it.",
    {
      ...
      functionalAreaId: z
        .string()
        .optional()
        .describe("New functional area ID"),
      quarantined: z
        .boolean()
        .optional()
  ...
  server.tool(
    ...
    async (
      ...
      const data = (await client.getTestRun(params.runId)) as Record<
        string,
        unknown
      >;
  ...
  server.tool(
    ...
    "List recent builds for a repository with status and test counts.",
    {
      ...
      limit: z
        .number()
        .optional()
        .describe("Number of builds to return (default 10, max 100)"),
    },
    async (
      params,
    ): Promise<{ content: Array<{ type: "text"; text: string }> }> => {
  ...
  server.tool(
    ...
    async (
      ...
      const diff = (await client.getDiff(params.diffId)) as Record<
        string,
        unknown
      >;
      const response: ToolResponse = {
  ...
  server.tool(
    ...
    async (
      ...
      const shares = (await client.listBuildShares(
        params.buildId as string,
      )) as Array<Record<string, unknown>>;
  ...
  server.tool(
    ...
    async (
      ...
      const shares = (await client.listTestShares(
  ...
  server.tool(
    ...
    "Revoke a public share by its share ID. The `/r/<slug>` URL stops resolving immediately. Find share IDs via lastest_list_build_shares or lastest_list_test_shares.",
    {
      shareId: z
        .string()
        .describe(
          "Share ID to revoke (NOT the slug — call lastest_list_build_shares to find it)",
        ),
    },
  ...
  server.tool(
    ...
    async (
      ...
      const build = (await client.getBuild(params.buildId)) as Record<
        string,
        unknown
      >;
      ...
      if (pending.length > 0) {
        actionRequired.push(
          `Review ${pending.length} pending diff(s) — use lastest_get_diff, lastest_approve_diff, or lastest_reject_diff`,
        );
      }
      if (failed > 0) {
        actionRequired.push(
          `${failed} test(s) failed — use lastest_list_failing_tests or lastest_heal_test`,
        );
      }
  ...
  server.tool(
    ...
    async (
      ...
      const [tests, builds] = await Promise.all([
        client.listTests(params.repositoryId) as Promise<
          Array<Record<string, unknown>>
        >,
        client.listBuilds(params.repositoryId, 5) as Promise<
          Array<Record<string, unknown>>
        >,
      ]);
      ...
      if (failing > 0) {
        actionRequired.push(
          `${failing} test(s) currently failing — use lastest_list_failing_tests`,
        );
      }
      if (buildsNeedingReview.length > 0) {
        actionRequired.push(
          `${buildsNeedingReview.length} build(s) need review — use lastest_review_build`,
        );
      }
      if (neverRun > 0) {
        actionRequired.push(
  ...
  server.tool(
    ...
    'Get the current status and results of a build. Use after lastest_run_tests to check progress. Returns slim build scalars + slim diff index — drill into a specific diff with lastest_get_visual_diff (or pass `includeDiffs: "full"` if you really need the joined a11y/network/AI payloads).',
    {
      ...
      includeDiffs: z
        ...
        .optional()
        .describe(
          '"slim" (default) returns only id/testId/stepLabel/status/classification/pct per diff. "full" returns every joined column (heavy — can be 100KB+ for builds with many diffs).',
        ),
    },
    async (
      params,
      ...
      const build = (await client.getBuild(params.buildId, {
      ...
      if (status === "review_required") {
        actionRequired.push(
          `Review ${changes} visual change(s) — use lastest_get_visual_diff to inspect`,
        );
      }
      if (status === "blocked") {
        actionRequired.push(
      ...
      if (failed > 0) {
        actionRequired.push(
          `${failed} test(s) failed — use lastest_list_failing_tests to see details`,
        );
      }
  ...
  server.tool(
    ...
    async (
      ...
      const tests = (await client.listTests(params.repositoryId)) as Array<
        Record<string, unknown>
      >;
  ...
  server.tool(
    ...
    "Approve visual changes, updating baselines. Accepts one or more diff IDs.",
    {
      diffIds: z
        .array(z.string())
        .describe("Array of visual diff IDs to approve"),
    },
    withActivityReporting(
      client,
      "lastest_approve_baseline",
  ...
  server.tool(
    ...
    "Reject visual changes, marking them as regressions. Accepts one or more diff IDs.",
    {
      diffIds: z
        .array(z.string())
        .describe("Array of visual diff IDs to reject"),
    },
  ...
  server.tool(
    "lastest_create_test",
    "Create a test. **Preferred (MCP-first): DIRECT mode** — YOU write the Playwright code and pass { name, code }. First read the resource lastest://repo/{repositoryId}/authoring-guide for the exact runner contract + this repo's base URL, areas, and setup; discover selectors with your Playwright MCP (or lastest_scout_url). The `author-test` prompt walks the whole flow. AI mode ({ url }/{ prompt }) asks Lastest to generate server-side and only works if in-product AI is configured there — if it isn't, fall back to direct mode.",
    {
      repositoryId: z.string().describe("Repository ID to create the test in"),
      name: z
        .string()
        .optional()
        .describe("Test name (required for direct mode)"),
      code: z
        .string()
        .optional()
        .describe(
          "Playwright test code (required for direct mode). Expected signature: `export async function test(page, baseUrl, screenshotPath, stepLogger)`",
        ),
      url: z
        .string()
        .optional()
        .describe("URL to generate a test for (AI mode)"),
      prompt: z
        .string()
        .optional()
        .describe("Natural language description of what to test (AI mode)"),
      functionalAreaId: z
        .string()
        .optional()
      ...
      targetUrl: z
        .string()
        .optional()
        .describe("Target URL for the test (direct mode)"),
      description: z
        .string()
        .optional()
        .describe("Test description (direct mode)"),
    },
  ...
  server.tool(
    ...
    async (
      ...
      const states = (await client.listStorageStates(
        params.repositoryId as string,
      )) as Array<Record<string, unknown>>;
      const response: ToolResponse = {
        status: "ok",
  ...
  server.tool(
    ...
    "Create a new storage state for a repo from a Playwright `storageState()` JSON string. The JSON should be the output of `await context.storageState({ indexedDB: true })` (Playwright v1.51+, captures Firebase Auth) or the legacy bare `storageState()` call. Use this to share a logged-in session across tests instead of re-logging in. Treat the raw JSON like a password: only call this with tokens you intend Lastest to use. Optional provenance fields (authFlavor / tokenLocations / firebaseApiKey / expiresAt) let agents pick the right re-auth strategy at re-run time without re-scouting the target.",
    {
      ...
      storageStateJson: z
        .string()
        .describe(
          'Playwright storageState JSON string — `{ "cookies": [...], "origins": [...] }`. Include `indexedDB` per-origin entries when the source app stores tokens there (Firebase Auth, Clerk, Supabase v2).',
        ),
      authFlavor: z
        .string()
        .optional()
      ...
      tokenLocations: z
        ...
        .optional()
        .describe(
          "Where the session token lives. Subset of: cookie | localStorage | sessionStorage | indexedDB. Helps surface captures that are missing a location they should have.",
        ),
      firebaseApiKey: z
        .string()
        .optional()
      ...
      expiresAt: z
        ...
        .optional()
        .describe(
          "Best-effort capture-validity hint (ISO date string). Lets surfaces flag stale captures before silent expiry. See reference_session_lifetime_by_auth_library for defaults per library.",
        ),
    },
    withActivityReporting(
      client,
      "lastest_create_storage_state",
      async (params) => {
        const result = (await client.createStorageState(
          params.repositoryId as string,
          {
            ...
            firebaseApiKey:
              (params.firebaseApiKey as string | undefined) ?? null,
  ...
  server.tool(
    ...
    withActivityReporting(
      ...
      "lastest_delete_storage_state",
      async (params) => {
        const result = await client.deleteStorageState(
          params.storageStateId as string,
        );
        const response: ToolResponse = {
          status: "deleted",
  ...
  server.tool(
    ...
    async (
      ...
      const scripts = (await client.listSetupScripts(
        params.repositoryId as string,
      )) as Array<Record<string, unknown>>;
      const response: ToolResponse = {
        status: "ok",
  ...
  server.tool(
    ...
    async (
      ...
      const script = (await client.getSetupScript(
        params.setupScriptId as string,
      )) as Record<string, unknown>;
      const response: ToolResponse = {
        status: "ok",
  ...
  server.tool(
    ...
    withActivityReporting(
      ...
      "lastest_create_setup_script",
      async (params) => {
        const result = (await client.createSetupScript(
          params.repositoryId as string,
          {
            name: params.name as string,
  ...
  server.tool(
    ...
    withActivityReporting(
      ...
      "lastest_delete_setup_script",
      async (params) => {
        const result = await client.deleteSetupScript(
          params.setupScriptId as string,
        );
        const response: ToolResponse = {
          status: "deleted",
