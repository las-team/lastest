...
describe("resolveCountry over the shipped overlays", () => {
  ...
  it("CN: separate China-hosted vault, cn residency, no cross-border transfer", () => {
    ...
    expect(cn.staging).toEqual({
      databaseUrl: "postgres://cn/mig",
      runDir: "/data/cn/runs",
    });
...
describe("mergeOverlays (user config wins)", () => {
  ...
  it("only referenced countries are merged, so CN env vars are optional until CN is used", () => {
    ...
    expect(() =>
      ...
      /CONFIG_ENV_MISSING.*MIG_DATABASE_URL_CN, VAULT_CN_PASSWORD, VAULT_CN_USER/,
    );

    // a user-supplied auth + staging removes the need for the env variables
    const own = mergeOverlays(
      baseConfig({
        CN: {
          target: { auth: { kind: "accessToken", token: "t" } },
          staging: { databaseUrl: "postgres://cn-local/mig" },
        },
      }),
      { overlays, env: {} },
    );
    ...
    expect(own.countries.CN.staging?.runDir).toBe("/data/cn/runs");
  });
