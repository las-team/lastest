...
export function parentCountryPath(mapping: MaterialisedMapping | undefined): string | undefined {
...
export function buildPredicate(input: PredicateInput): string {
  ...
  if (cutoff && scope.kind === "dated") {
    const parts = scope.predicates.map((p) => `${p.field} >= ${p.type === "datetime" ? `${cutoff}T00:00:00Z` : cutoff}`);
    ...
    terms.push(`${scope.parentField} >= ${scope.type === "datetime" ? `${cutoff}T00:00:00Z` : cutoff}`);
  }
  if (unit.country !== "GLOBAL") {
    const c = buildCountryPredicate(mapping.countryOf, unit.country, {}, input.parentPaths ?? {});
    if (c) terms.push(`(${c})`);
  }
  if (input.window)
    terms.push(`SystemModstamp >= ${soqlDate(input.window.wmLo)} AND SystemModstamp < ${soqlDate(input.window.wmHi)}`);
  const part = mapping.load.partitionBy;
  if (part && input.partition) terms.push(`${part.field} ${input.partition === "null" ? "= null" : "!= null"}`);
  return terms.join(" AND ");
}

export function columnsFor(target: ResolvedTarget, mapping: MaterialisedMapping): string[] {
  const set = new Set<string>(SYSTEM_COLUMNS);
  for (const c of target.columns) set.add(c);
  if (!target.columns.length) for (const f of mapping.fields) if (f.source && f.transform.kind !== "skip") set.add(f.source);
  return [...set];
}
...
export class SimpleExtractor implements Extractor {
  ...
  private collectFks(row: SourceRow, mapping: MaterialisedMapping, fkSets: FkIdSets, queueOwners: Set<string>): void {
    for (const c of referenceColumns(mapping)) {
      ...
      if (c.key === "user") {
        ...
        else if (isUserId(v)) (fkSets.get("user") ?? fkSets.set("user", new Set()).get("user")!).add(to18(v));
        continue;
      }
      (fkSets.get(c.key) ?? fkSets.set(c.key, new Set()).get(c.key)!).add(to18(v));
    }
  }

  async extractUnit(unit: Unit, plan: ExtractPlan): Promise<ExtractManifest> {
    const log = getLogger("Extract", { run_id: plan.runId, object_key: unit.objectKey, country: unit.country });
    ...
    const partitions: Array<"null" | "notNull" | undefined> = mapping.load.partitionBy ? (mapping.load.partitionBy.order ?? ["null", "notNull"]) : [undefined];
    ...
    for (let pi = 0; pi < partitions.length; pi++) {
      const predicate = buildPredicate({ mapping, unit, cutoffDate: plan.cutoffDate, window: plan.window, parentPaths, partition: partitions[pi] });
      if (pi === 0) manifest.predicate = predicate;
      const order = mapping.load.orderBy?.length ? ` ORDER BY ${mapping.load.orderBy.join(", ")}` : "";
      ...
      const flush = async () => {
        ...
        const file = path.join(dir, `${jobId}-page${pageNo}${pi ? `-p${pi}` : ""}.json`);
        await fs.writeFile(file, JSON.stringify(buf), "utf8");
        const ef: ExtractFile = { path: file, jobId, pageNo, rows: buf.length, closure: false, partition: mapping.load.partitionBy ? pi : undefined };
        manifest.files.push(ef);
        await this.deps.store.checkpoints.add({ runId: plan.runId, objectKey: unit.objectKey, country: unit.country, jobId, locator: null, pageNo, rows: buf.length, file });
        pageNo++;
        buf = [];
      };
      for await (const raw of this.deps.sfdc.query(soql, { all: true })) {
        ...
        if (deleted) {
          ...
          manifest.deletedIds.push({ id: row.Id, deletedDate: typeof row.SystemModstamp === "string" ? row.SystemModstamp : plan.window?.wmHi ?? new Date().toISOString() });
          continue;
        }
      ...
      if (!plan.limit) {
        try {
          const n = await this.deps.sfdc.count(mapping.sourceObject, predicate || undefined);
    ...
    if (plan.deletedSince && target.replicateable && plan.window) {
      try {
        const feed = await this.deps.sfdc.getDeleted(mapping.sourceObject, plan.deletedSince, plan.window.wmHi);
        for (const d of feed.deletedRecords) manifest.deletedIds.push({ id: to18(d.id), deletedDate: d.deletedDate });
        manifest.deletedLatestCovered = feed.latestDateCovered;
      } catch (e) {
        log.warn({ err: e }, "delete feed unavailable");
      }
    }
    log.info({ live: manifest.extractedLive, deleted: manifest.extractedDeleted, files: manifest.files.length, scope_count: manifest.sfdcScopeCount }, "extract done");
    return manifest;
  }

  async closure(req: ClosureRequest): Promise<ClosureResult> {
    const log = getLogger("Closure", { run_id: req.runId, country: req.country });
    const result: ClosureResult = { files: new Map(), rounds: 0, dangling: new Map(), fetched: new Map() };
    let needed: FkIdSets = new Map([...req.needed].map(([k, v]) => [k, new Set(v)]));
    const known = new Map<ObjectKey | "user", Set<string>>();
    for (let round = 0; round < req.maxRounds && [...needed.values()].some((s) => s.size); round++) {
      ...
      for (const [key, ids] of needed) {
        ...
        if (!mapping || !target) {
          for (const id of ids) (result.dangling.get(key) ?? result.dangling.set(key, new Set()).get(key)!).add(id);
          continue;
        }
        ...
        for (let i = 0; i < list.length; i += 500) {
          ...
          for (const id of chunk) if (!got.has(id) || got.get(id)!.dryRun) remaining.push(id);
        }
        ...
        for (let i = 0; i < remaining.length; i += 400) {
          for await (const raw of this.deps.sfdc.queryIds(mapping.sourceObject, remaining.slice(i, i + 400), columns)) {
            const row: SourceRow = { ...raw, Id: to18(raw.Id) };
            returned.add(row.Id);
        ...
        for (const id of remaining) if (!returned.has(id)) (result.dangling.get(key) ?? result.dangling.set(key, new Set()).get(key)!).add(id);
        if (rows.length) {
          const file = path.join(dir, `closure-r${round}-${Date.now().toString(36)}.json`);
          await fs.writeFile(file, JSON.stringify(rows), "utf8");
          const ef: ExtractFile = { path: file, jobId: `closure-${round}`, pageNo: round, rows: rows.length, closure: true };
          (result.files.get(key) ?? result.files.set(key, []).get(key)!).push(ef);
    ...
    if ([...needed.values()].some((s) => s.size))
      log.warn({ rounds: result.rounds }, "closure did not converge within maxRounds");
    return result;
  }

  async *readRows(files: readonly ExtractFile[]): AsyncIterable<{ row: SourceRow; file: ExtractFile }> {
    for (const file of files) {
      const rows = JSON.parse(await fs.readFile(file.path, "utf8")) as SourceRow[];
