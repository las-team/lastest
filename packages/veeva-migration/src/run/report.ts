  ...
  for (const f of findings) {
    ...
    const key =
      typeof f.detail === "string"
        ? f.detail
        : ((f.detail as { value?: string; user?: string; sfdcId?: string })
            .value ??
          (f.detail as { user?: string }).user ??
          (f.detail as { sfdcId?: string }).sfdcId ??
          cell(f.detail));
...
export function renderReport(input: RunReportInput): RunReport {
  ...
  const findings = [...input.findings].sort(
    (a, b) => SEV[a.severity] - SEV[b.severity] || a.code.localeCompare(b.code),
  );
  const summary = {
  ...
  md.push(
    `- mode: **${run.mode}**${run.dryRun ? " (dry-run)" : ""}${run.wave ? ` · wave: ${run.wave}` : ""} · countries: ${run.countries.join(", ") || "—"}`,
  );
  md.push(
    `- status: **${run.status}** · exit code: ${input.exitCode} · started ${run.startedAt}${run.finishedAt ? ` · finished ${run.finishedAt}` : ""}`,
  );
  md.push(
    `- tool ${run.toolVersion} · config ${run.configHash.slice(0, 12)} · mapping ${run.mappingHash?.slice(0, 12) ?? "—"}`,
  );
  md.push(
    `- source org ${run.sourceOrgId ?? "—"} (API ${run.sourceApiVersion ?? "—"}) · target ${run.targetVaultDns ?? "—"} (API ${run.targetApiVersion ?? "—"}) · sfdc_now ${run.sfdcNowAtStart ?? "—"}${run.freezeAt ? ` · freeze_at ${run.freezeAt}` : ""}`,
  );
  md.push(
    "",
    `## Findings (${summary.blocking} blocking, ${summary.warning} warning, ${summary.info} info)`,
    "",
  );
  md.push(
    table(
      ["severity", "code", "object", "country", "field", "count", "detail"],
      findings.map((f) => [
        f.severity,
        f.code,
        f.objectKey,
  ...
  md.push(
    table(
      [
        ...
        "unchanged",
        "failed",
        "deleted (applied/ignored/pending)",
        "vault count",
        "agg src",
        "agg tgt",
      ],
      input.reconciliation.map((r) => [
        ...
        r.unchanged,
        r.failed,
        `${r.deleted} (${r.deletedApplied ?? 0}/${r.deletedIgnored ?? 0}/${r.deletedPending ?? 0})`,
        r.vaultCount,
        r.aggHashSrc,
        r.aggHashTgt,
      ]),
  ...
  const failed = input.reconciliation.flatMap((r) =>
    Object.entries(r.failedByType ?? {}).map(([t, n]) => [
      `${r.objectKey}:${r.country}`,
      t,
      n,
    ]),
  ...
  md.push(
    ...
    "",
    table(["unit", "error type", "rows"], failed),
  );
  const skipped = input.reconciliation.flatMap((r) =>
    Object.entries(r.skippedByReason ?? {}).map(([t, n]) => [
      `${r.objectKey}:${r.country}`,
      t,
      n,
    ]),
  ...
  md.push(
    ...
    "",
    table(["unit", "reason", "rows"], skipped),
  );
  const users = countsByCode(findings, "UNMAPPED_USER");
  md.push("## Unmapped users", "", table(["user", "referencing rows"], users));
  const picklists = [
    ...countsByCode(findings, "VT_PICKLIST_VALUE_MISSING"),
    ...countsByCode(findings, "UNMAPPED_PICKLIST"),
  ];
  md.push(
    "## Unmapped picklist values",
    "",
    table(["value", "rows"], picklists),
  );
  if (input.pendingTargets && Object.keys(input.pendingTargets).length)
    md.push(
      ...
      "",
      table(
        ["unit", "target object", "sample ids"],
        Object.entries(input.pendingTargets).flatMap(([u, t]) =>
          Object.entries(t).map(([k, ids]) => [u, k, ids.join(", ")]),
        ),
      ),
    );
  if (input.ignoredDeletes && Object.keys(input.ignoredDeletes).length)
    md.push(
      "## Deletes not applied (review)",
      "",
      table(
        ["unit", "reason", "rows"],
        Object.entries(input.ignoredDeletes).flatMap(([u, r]) =>
          Object.entries(r).map(([k, n]) => [u, k, n]),
        ),
      ),
    );
  if (input.sampleDiffs && Object.keys(input.sampleDiffs).length)
    md.push(
      "## Sample read-back differences",
      "",
      table(["unit", "differences"], Object.entries(input.sampleDiffs)),
    );
  ...
  const json = {
    ...
    source: {
      orgId: run.sourceOrgId ?? null,
      apiVersion: run.sourceApiVersion ?? null,
      now: run.sfdcNowAtStart ?? null,
    },
    target: {
      vaultDns: run.targetVaultDns ?? null,
      vaultId: run.targetVaultId ?? null,
      apiVersion: run.targetApiVersion ?? null,
    },
...
export async function reportFromStore(
  store: StateStore,
  runId: string,
  now = new Date(),
): Promise<RunReportInput | undefined> {
  ...
  const units: UnitTiming[] = reconciliation.map((r) => ({
    unit: { objectKey: r.objectKey, country: r.country },
    status: r.status === "fail" ? "failed" : "succeeded",
  }));
  const exitCode =
    run.status === "blocked" ? 2 : run.status === "failed" ? 3 : 0;
  return {
    run,
    exitCode,
    units,
