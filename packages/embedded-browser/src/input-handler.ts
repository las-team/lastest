...
function _getModifierFlags(modifiers?: {
  ctrl?: boolean;
  shift?: boolean;
  alt?: boolean;
  meta?: boolean;
...
export class InputHandler {
  ...
  private buildModifierPrefix(modifiers?: {
    ctrl?: boolean;
    shift?: boolean;
    alt?: boolean;
    meta?: boolean;
  ...
  private async handleFileUpload(event: FileUploadEvent): Promise<void> {
    ...
    const tmpDir = path.join(
      os.tmpdir(),
      `lastest-stream-upload-${Date.now()}`,
    );
    ...
    try {
      ...
      console.warn(
        "[InputHandler] No file input found for upload, files written to:",
        tmpDir,
      );
    }
