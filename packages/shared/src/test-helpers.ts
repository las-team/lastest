...
export type FileUploadHelper = (selector: string, filePaths: string | string[]) => Promise<void>;
...
export interface DownloadsHelper {
  waitForDownload: (triggerAction: () => Promise<void>) => Promise<{ filename: string; path: string }>;
  list: () => DownloadInfo[];
...
export interface NetworkHelper {
  ...
  capture: (urlPattern: string) => { requests: Array<{ url: string; method: string; postData?: string }> };
}
...
// ────────────────────────────────────────────────────────────────────────
export function createClipboardHelper(page: any, opts: { granted: boolean }): ClipboardHelper | null {
...
export function createDownloadsHelper(
    ...
    return {
      helper: {
        waitForDownload: async () => {
          throw new Error('Downloads not enabled — enable "Accept Downloads" in Playwright settings');
        },
        list: () => [],
        waitForAny: async () => {},
      },
  ...
  const dlDir = opts.tmpDir ?? path.join(os.tmpdir(), `lastest-dl-${Date.now()}-${Math.random().toString(36).slice(2)}`);
...
export function decodeFixturesToTmp(
  ...
  if (!fixtures || fixtures.length === 0) return { fixturesMap, cleanupDir: '' };
