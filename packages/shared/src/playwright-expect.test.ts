...
function fakeLocator(opts: {
  ...
  return {
    ...
    waitFor: async ({ state, timeout }: { state: string; timeout: number }) => {
      ...
      const okAttached = want === 'attached' ? actuallyVisible : !actuallyVisible;
...
describe('createExpect — prior-shim parity', () => {
  it('page.toHaveURL passes on exact match', async () => {
    await exp(fakePage({ url: 'https://x.test/a' })).toHaveURL('https://x.test/a');
  });
  it('page.toHaveURL passes on regex match', async () => {
    await exp(fakePage({ url: 'https://x.test/foo/bar' })).toHaveURL(/foo\/bar/);
  });
  it('page.toHaveURL fails when no match', async () => {
    await vExpect(exp(fakePage({ url: 'https://x.test/' })).toHaveURL('https://y.test/')).rejects.toThrow();
  });
  ...
  it('locator.toBeVisible fails for hidden', async () => {
    await vExpect(exp(fakeLocator({ visible: false })).toBeVisible()).rejects.toThrow();
  });
...
describe('createExpect — added matchers', () => {
  it('locator.toBeAttached', async () => {
    ...
    await vExpect(exp(fakeLocator({ visible: false })).toBeAttached()).rejects.toThrow();
  });
  it('locator.toBeEnabled / toBeDisabled', async () => {
    ...
    await vExpect(exp(fakeLocator({ isEnabled: false })).toBeEnabled()).rejects.toThrow();
  });
  it('locator.toBeChecked', async () => {
    await exp(fakeLocator({ isChecked: true })).toBeChecked();
    await vExpect(exp(fakeLocator({ isChecked: false })).toBeChecked()).rejects.toThrow();
  });
  it('locator.toHaveValue exact + regex', async () => {
    ...
    await vExpect(exp(fakeLocator({ value: 'xx' })).toHaveValue('yy')).rejects.toThrow();
  });
  it('locator.toHaveAttribute exact', async () => {
    await exp(fakeLocator({ attrs: { href: '/home' } })).toHaveAttribute('href', '/home');
    await vExpect(
      exp(fakeLocator({ attrs: { href: '/other' } })).toHaveAttribute('href', '/home'),
    ).rejects.toThrow();
  });
  it('locator.toHaveCount', async () => {
    await exp(fakeLocator({ count: 3 })).toHaveCount(3);
    await vExpect(exp(fakeLocator({ count: 1 })).toHaveCount(3)).rejects.toThrow();
  });

  it('.not.toBeChecked (locator)', async () => {
    await exp(fakeLocator({ isChecked: false })).not.toBeChecked();
    await vExpect(exp(fakeLocator({ isChecked: true })).not.toBeChecked()).rejects.toThrow();
  });
