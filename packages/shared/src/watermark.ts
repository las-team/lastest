...
export async function watermarkVideo(
  ...
  const scaledLogo =
    "[1:v]scale=-1:40,format=rgba,colorchannelmixer=aa=0.85[logo]";
  ...
  const ok = await new Promise<boolean>((resolve) => {
    ...
    const timer = setTimeout(() => {
      ...
      console.warn(
        `[watermark] ffmpeg timed out after ${timeoutMs}ms; stderr: ${stderr.trim()}`,
      );
      resolve(false);
    }, timeoutMs);
    child.on("error", (err) => {
    ...
    child.on("close", (code) => {
      ...
      if (code !== 0) {
        console.warn(
          `[watermark] ffmpeg exit=${code}; stderr: ${stderr.trim()}`,
        );
      }
      resolve(code === 0);
    });
