...
function buildAsyncNot(positive: MatcherMap, msgPrefix: string): MatcherMap {
  ...
  for (const [name, fn] of Object.entries(positive)) {
    not[name] = async (...args: any[]) => {
      ...
      if (!threw) {
        throw new Error(
          `${msgPrefix}Expected NOT to match ${name}(${args.map((a) => safeStringify(a)).join(", ")})`,
        );
      }
    };
  }
...
export function createExpect(
  options: CreateExpectOptions = {},
): (target: any, message?: string) => any {
  ...
  return function expect(target: any, message?: string) {
    ...
    const isLocator =
      typeof target?.click === "function" && typeof target?.fill === "function";
    ...
    if (isPage) {
      const page: MatcherMap = {
        async toHaveURL(
          ...
          if (!ok)
            throw new Error(
              `${msgPrefix}Expected URL "${expected}" but got "${target.url()}"`,
            );
        },
        async toHaveTitle(
          expected: string | RegExp,
          ...
          if (!ok)
            throw new Error(
              `${msgPrefix}Expected title "${expected}" but got "${await target.title()}"`,
            );
        },
    ...
    if (isLocator) {
      const locator: MatcherMap = {
        ...
        async toBeEnabled(opts?: { timeout?: number }) {
          ...
          if (!ok)
            throw new Error(`${msgPrefix}Expected locator to be enabled`);
        },
        async toBeDisabled(opts?: { timeout?: number }) {
          const t = opts?.timeout ?? defaultTimeout;
          const ok = await pollUntil(async () => await target.isDisabled(), t);
          if (!ok)
            throw new Error(`${msgPrefix}Expected locator to be disabled`);
        },
        async toBeChecked(opts?: { timeout?: number; checked?: boolean }) {
          const t = opts?.timeout ?? defaultTimeout;
          const desired = opts?.checked ?? true;
          const ok = await pollUntil(
            async () => (await target.isChecked()) === desired,
            t,
          );
          if (!ok)
            throw new Error(
              `${msgPrefix}Expected locator to be ${desired ? "checked" : "unchecked"}`,
            );
        },
        async toBeFocused(opts?: { timeout?: number }) {
          ...
          if (!ok)
            throw new Error(`${msgPrefix}Expected locator to be focused`);
        },
        async toBeEditable(opts?: { timeout?: number }) {
          const t = opts?.timeout ?? defaultTimeout;
          const ok = await pollUntil(async () => await target.isEditable(), t);
          if (!ok)
            throw new Error(`${msgPrefix}Expected locator to be editable`);
        },
        ...
        async toBeInViewport(opts?: { timeout?: number; ratio?: number }) {
          ...
          if (!ok)
            throw new Error(`${msgPrefix}Expected locator to be in viewport`);
        },
        async toHaveText(
          expected: string | RegExp,
          opts?: { timeout?: number },
          ...
          if (!ok)
            throw new Error(
              `${msgPrefix}Expected text "${expected}" but got "${await target.textContent()}"`,
            );
        },
        async toContainText(
          expected: string | RegExp,
        ...
        async toHaveValue(
          ...
          if (!ok)
            throw new Error(
              `${msgPrefix}Expected value "${expected}" but got "${await target.inputValue()}"`,
            );
        },
        ...
        async toHaveCount(expected: number, opts?: { timeout?: number }) {
          const t = opts?.timeout ?? defaultTimeout;
          const ok = await pollUntil(
            async () => (await target.count()) === expected,
            t,
          );
          if (!ok)
            throw new Error(
              `${msgPrefix}Expected count ${expected} but got ${await target.count()}`,
            );
        },
        async toHaveClass(
          ...
          const ok = await pollUntil(async () => {
            ...
            if (Array.isArray(expected)) {
              ...
              return expected.every((e, i) =>
                typeof e === "string" ? cls[i] === e : e.test(cls[i] ?? ""),
              );
            }
            const joined = cls.join(" ");
            return typeof expected === "string"
              ? joined.split(/\s+/).includes(expected)
              : expected.test(joined);
          }, t);
          if (!ok)
            throw new Error(`${msgPrefix}Expected class to match ${expected}`);
        },
        async toHaveCSS(
          name: string,
          expected: string | RegExp,
          ...
          const ok = await pollUntil(async () => {
            const v = await target.evaluate(
              (el: Element, p: string) =>
                getComputedStyle(el as HTMLElement).getPropertyValue(p),
              name,
            );
          ...
          if (!ok)
            throw new Error(
              `${msgPrefix}Expected CSS ${name} to match ${expected}`,
            );
        },
        async toHaveJSProperty(
          ...
          const ok = await pollUntil(async () => {
            const v = await target.evaluate(
              (el: Element, p: string) => (el as any)[p],
              name,
            );
          ...
          if (!ok)
            throw new Error(
              `${msgPrefix}Expected JS property ${name} to equal ${safeStringify(expected)}`,
            );
        },
    ...
    const generic: MatcherMap = {
      toBe(expected: unknown) {
        if (target !== expected) {
          throw new Error(
            `${msgPrefix}Expected ${safeStringify(expected)} but got ${safeStringify(target)}`,
          );
        }
      },
      ...
      toStrictEqual(expected: unknown) {
        if (JSON.stringify(target) !== JSON.stringify(expected)) {
          throw new Error(
            `${msgPrefix}Expected (strict) ${safeStringify(expected)} but got ${safeStringify(target)}`,
          );
        }
      },
      toBeTruthy() {
        if (!target)
          throw new Error(
            `${msgPrefix}Expected value to be truthy but got ${safeStringify(target)}`,
          );
      },
      toBeFalsy() {
        if (target)
          throw new Error(
            `${msgPrefix}Expected value to be falsy but got ${safeStringify(target)}`,
          );
      },
      toBeNull() {
        if (target !== null)
          throw new Error(
            `${msgPrefix}Expected null but got ${safeStringify(target)}`,
          );
      },
      toBeUndefined() {
        if (target !== undefined)
          throw new Error(
            `${msgPrefix}Expected undefined but got ${safeStringify(target)}`,
          );
      },
      toBeNaN() {
        if (!(typeof target === "number" && Number.isNaN(target))) {
          throw new Error(
            `${msgPrefix}Expected NaN but got ${safeStringify(target)}`,
          );
        }
      },
      toBeInstanceOf(ctor: any) {
        if (!(target instanceof ctor)) {
          throw new Error(
            `${msgPrefix}Expected instance of ${ctor?.name ?? ctor}`,
          );
        }
      },
      toBeCloseTo(expected: number, digits = 2) {
        if (typeof target !== "number") {
          throw new Error(
            `${msgPrefix}toBeCloseTo requires a number, got ${typeof target}`,
          );
        }
        const tolerance = Math.pow(10, -digits) / 2;
        if (Math.abs(target - expected) > tolerance) {
          throw new Error(
            `${msgPrefix}Expected ${target} to be close to ${expected} (${digits} digits)`,
          );
        }
      },
      toContain(expected: unknown) {
        if (Array.isArray(target)) {
          if (!target.includes(expected)) {
            throw new Error(
              `${msgPrefix}Expected array to contain ${safeStringify(expected)}`,
            );
          }
        } else if (typeof target === "string") {
          if (!target.includes(expected as string)) {
            throw new Error(
              `${msgPrefix}Expected string to contain ${safeStringify(expected)}`,
            );
          }
        } else {
          throw new Error(
            `${msgPrefix}toContain only works on arrays and strings`,
          );
        }
      },
      toHaveLength(expected: number) {
        ...
        if (actual !== expected) {
          const details = Array.isArray(target)
            ...
            : "";
          throw new Error(
            `${msgPrefix}Expected length ${expected} but got ${actual}${details}`,
          );
        }
      },
      toBeGreaterThan(expected: number) {
        if (typeof target !== "number" || target <= expected) {
          throw new Error(
            `${msgPrefix}Expected ${target} to be greater than ${expected}`,
          );
        }
      },
      toBeGreaterThanOrEqual(expected: number) {
        if (typeof target !== "number" || target < expected) {
          throw new Error(
            `${msgPrefix}Expected ${target} to be greater than or equal to ${expected}`,
          );
        }
      },
      toBeLessThan(expected: number) {
        if (typeof target !== "number" || target >= expected) {
          throw new Error(
            `${msgPrefix}Expected ${target} to be less than ${expected}`,
          );
        }
      },
      toBeLessThanOrEqual(expected: number) {
        if (typeof target !== "number" || target > expected) {
          throw new Error(
            `${msgPrefix}Expected ${target} to be less than or equal to ${expected}`,
          );
        }
      },
