...
- The provisioner inlines `LASTEST_URL` / `EB_IMAGE` into each Job spec from the pool-service env, plus a **per-session `EB_BOOTSTRAP_TOKEN`** (HMAC-signed with `ENCRYPTION_KEY`, bound to the pod's instanceId, TTL = the Job's deadline) — dynamic pods never receive a fleet-wide secret. `SYSTEM_EB_TOKEN` remains accepted at auto-register ONLY for static fleets (Zima compose replicas). The pool service therefore needs the same `ENCRYPTION_KEY` as the app.
- Required `.env.local` keys for the host dev flow:
  ...
  - `SYSTEM_EB_TOKEN` is NOT needed for the k3d flow anymore (dynamic pods use per-session bootstrap tokens); set it only for static-fleet EBs
