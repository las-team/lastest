...
- **EB token denial:** EB pods hold no Kubernetes ServiceAccount token. Two layers: `jobSpec()` in `packages/pool-service/src/provisioner.ts` sets `automountServiceAccountToken: false` on every EB pod spec, and `k8s/namespace.yaml` patches the namespace's `default` SA to `automountServiceAccountToken: false`, so NO pod in `lastest` gets a token unless it explicitly names a scoped SA — only `lastest-app` (app pod, single-container topology, where the app runs the pool service in-process) and `lastest-pool` (split topology) do, and each carries just a namespace-local provisioner Role with identical rules (`lastest-eb-provisioner` / `lastest-pool-eb-provisioner` — separate object names on purpose: same-named RoleBindings in two files are one object, and `kubectl apply` is last-writer-wins, which would silently revoke one side's provisioning rights). The split-topology app pod (k8s/app.yaml) sets no SA at all — it provisions via the pool service and stays tokenless. This matters because the netpol egress block on the apiserver is a no-op under k3d/flannel, and the k3d/k3s default SA is often cluster-admin. EBs authenticate to the app exclusively via the per-session `EB_BOOTSTRAP_TOKEN`. Every pod spec in `k8s/` also states `automountServiceAccountToken: false` itself rather than relying on the namespace patch, so a manifest applied to a namespace that never got it still comes up tokenless. When adding a new workload to the `lastest` namespace: it must not need the K8s API (stay tokenless), or it needs its own scoped SA + Role — never the default. Both halves are enforced by tests — `src/lib/eb/k8s-manifests.test.ts` over the manifests (a new named SA must also be added to its `TOKEN_HOLDERS` allowlist) and `jobSpec pod hardening` in `packages/pool-service/src/provisioner.test.ts` over the programmatic EB Job spec.
