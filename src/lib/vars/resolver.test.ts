...
describe("resolveVarReferences", () => {
  it("resolves a static var", () => {
    ...
    const r = resolveVarReferences(
      "await fill('{{var:token}}')",
      variables,
      [],
      [],
    );
...
describe("pickRowForVariable", () => {
  ...
  it("fixed mode honours sourceRow", () => {
    expect(
      pickRowForVariable(
        { ...v, sourceRowMode: "fixed", sourceRow: 3 },
        10,
        undefined,
      ).row,
    ).toBe(3);
  ...
  it("random mode picks within bounds (seeded)", () => {
    ...
    expect(
      pickRowForVariable({ ...v, sourceRowMode: "random" }, 10, undefined, rng)
        .row,
    ).toBe(1);
    expect(
      pickRowForVariable({ ...v, sourceRowMode: "random" }, 10, undefined, rng)
  ...
  it("increment walks forward and wraps to 2 past the last row", () => {
    ...
    expect(pickRowForVariable(inc, 5, undefined)).toEqual({
      row: 0,
      nextCursor: 1,
    });
    // 1 → 2
  ...
  it("rowCount=0 falls back to row 0", () => {
    expect(
      pickRowForVariable({ ...v, sourceRowMode: "random" }, 0, undefined).row,
    ).toBe(0);
    expect(
      pickRowForVariable({ ...v, sourceRowMode: "increment" }, 0, undefined)
        .row,
    ).toBe(0);
  });
});
