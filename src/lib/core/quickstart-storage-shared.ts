 ...
 * `context.storageState()` and persist it via createStorageState so a
 ...
 * That fallback (direct `playwright` launch) is why this stays app-level code
 * rather than becoming part of `core/browser`: `ctx.browser.withBrowser`
 * claims a *pooled* EB, and this fallback exists precisely for deployments
 * where no pool exists at all.
 */
