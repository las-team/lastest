...
/**
 * Generate demo notes for a run from facts captured during scouting + the
 * build outcome. Pass facts in, get prose out — never invent observations.
 *
 * Moved here, unchanged, from `src/lib/quickstart/quickstart-notes.ts` during
 * the QuickStart plugin migration (recipe §1.6.2). `demo-notes.ts` — a
 * second, still-unmigrated pseudo-plugin's `generateNotesForBuild` — calls
 * `generateDemoNotes` directly in "reduced-facts mode" (no scout ever ran),
 * for any build, not just a QuickStart session's own. Once QuickStart became
 * `plugins/quickstart/`, that import would have become a forbidden
 * `@lastest/plugin-quickstart` cross-plugin edge, so this stays shared,
 * app-level code both `src/lib/core/quickstart-host.ts` (QuickStart's own
 * `generateNotes` host method) and `demo-notes.ts` call into — the same
 * `share-reads.ts` shape, extended to a pseudo-plugin↔pseudo-plugin share.
 *
 * Deliberately NOT routed through `ctx.ai`: since this stays app-level code
 * rather than moving inside the plugin's own actions, `ai_prompt_logs
 * .action_type` attribution is unchanged by the migration — still
 * `"agent_discover"`, still outside `ai-capability.ts`'s `ACTION_TYPES`
 * allowlist, exactly as it was before this file existed.
 */
