...
describe('computeTier', () => {
  ...
  it('awards silver at the boundary: 10 tests + 95% pass + a11y 80 + 0 critical', () => {
    const latest = snap({ totalTests: 10, passedCount: 10, a11yScore: 80, a11yCriticalCount: 0 });
    expect(computeTier(input({
      testCount: 10,
      latestBuild: latest,
      recentBuilds: [latest],
  ...
  it('drops to bronze when pass rate falls below 95%', () => {
    const latest = snap({ totalTests: 20, passedCount: 18, failedCount: 2, a11yScore: 85 });
    expect(computeTier(input({
      latestBuild: latest,
      recentBuilds: [latest],
    }))).toBe('bronze');
  ...
  it('awards bronze at the minimum: 5 tests + 80% pass + a11y 60', () => {
    const latest = snap({ totalTests: 5, passedCount: 4, failedCount: 1, a11yScore: 60 });
    expect(computeTier(input({
      testCount: 5,
      latestBuild: latest,
      recentBuilds: [latest],
...
describe('computeCategories', () => {
  it('all false when no build', () => {
    expect(computeCategories(input({ latestBuild: null }))).toEqual({
      a11y: false, allPassing: false, zeroDrift: false,
    });
  });

  it('a11y true at score 90 + 0 critical', () => {
    expect(computeCategories(input({
  ...
  it('allPassing requires failedCount=0 AND changesDetected=0 AND totalTests>0', () => {
    expect(computeCategories(input({
      latestBuild: snap({ totalTests: 20, failedCount: 0, changesDetected: 0 }),
    })).allPassing).toBe(true);
    expect(computeCategories(input({
      latestBuild: snap({ totalTests: 0 }),
    })).allPassing).toBe(false);
    expect(computeCategories(input({
  ...
  it('zeroDrift true only when no rejected diffs in 30d', () => {
    expect(computeCategories(input({ rejectedDiffsLast30Days: 0 })).zeroDrift).toBe(true);
    expect(computeCategories(input({ rejectedDiffsLast30Days: 1 })).zeroDrift).toBe(false);
  });
});
...
describe('detectConfirmedRegression', () => {
  ...
  it('true on any rejected visual diff (user explicitly rejected baseline)', () => {
    expect(detectConfirmedRegression(input({ rejectedDiffCount: 1 }))).toBe(true);
  });

  it('false on single failed build (could be flake or transient)', () => {
    expect(detectConfirmedRegression(input({ consecutiveNonFlakyFailures: 1 }))).toBe(false);
  });

  it('true on 2 consecutive non-flaky failures', () => {
    expect(detectConfirmedRegression(input({ consecutiveNonFlakyFailures: 2 }))).toBe(true);
  });

  it('flakes alone never trigger, even many of them', () => {
    expect(detectConfirmedRegression(input({
...
describe('applyDowngradeRule (the ratchet)', () => {
  it('ratchets upward when no confirmed regression', () => {
    expect(applyDowngradeRule({
      priorCurrent: 'silver', latestComputed: 'gold', hasConfirmedRegression: false,
    })).toBe('gold');
  });

  it('keeps prior when no confirmed regression and new is lower', () => {
    expect(applyDowngradeRule({
      priorCurrent: 'gold', latestComputed: 'bronze', hasConfirmedRegression: false,
    })).toBe('gold');
  });

  it('downgrades to latestComputed on confirmed regression', () => {
    expect(applyDowngradeRule({
      priorCurrent: 'gold', latestComputed: 'bronze', hasConfirmedRegression: true,
    })).toBe('bronze');
    expect(applyDowngradeRule({
      priorCurrent: 'gold', latestComputed: 'none', hasConfirmedRegression: true,
    })).toBe('none');
  });
});
