...
function deltaMarkSvg(size: number, x: number, y: number, tone: 'light' | 'dark'): string {
...
export function renderSplitShield(opts: SplitShieldOptions): string {
  ...
  const sansFont = "'Inter', system-ui, -apple-system, 'Segoe UI', Roboto, sans-serif";
...
export function renderTierBadge(tier: AwardTier, size: Size = 'md', dark = false): string {
  const m = TIER_LABEL[tier];
  return renderSplitShield({ label: 'LASTEST', value: m.value, tone: m.tone, size, dark, mark: true });
}

export function renderA11yBadge(passing: boolean, size: Size = 'md', dark = false): string {
...
export function renderAllPassingBadge(passing: boolean, total: number, size: Size = 'md', dark = false): string {
  return renderSplitShield({
    label: 'tests',
...
export function renderZeroDriftBadge(clean: boolean, size: Size = 'md', dark = false): string {
  return renderSplitShield({
    label: 'regressions',
    value: clean ? '0' : '1+',
    tone: clean ? 'ink' : 'red',
...
export function renderReviewRequiredBadge(size: Size = 'md', dark = false): string {
  return renderSplitShield({
    label: 'LASTEST',
