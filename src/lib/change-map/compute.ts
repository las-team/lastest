...
// NOTE: This module is intentionally NOT a `"use server"` file. `computeChangeMap`
// reads another build's repo data, spends the owning team's GitHub token + AI
// budget, and overwrites their stored change map — so it must never be exposed
// as a directly-invocable server action (that would be a cross-team IDOR).
// It is called only from trusted server contexts: the guarded `setBuildManualScope`
// server action, build finalization, and ownership-checked API routes.
...
export async function resolveRepoIdForBuild(
  buildId: string,
): Promise<string | null> {
  const build = await queries.getBuild(buildId);
...
async function runChangeMapAI(input: RunAIInput): Promise<AIResult> {
  ...
  // MCP-first: background AI only runs when built-in AI is switched on.
  if (!(await queries.getInProductAiEnabled(input.repoId)))
    return { kind: "skipped", reason: "Built-in AI is off (MCP mode)" };
