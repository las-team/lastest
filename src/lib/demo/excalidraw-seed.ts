/**
 * Excalidraw visual regression seed data.
 *
 * Imported by the demo bootstrap (`src/lib/auth/demo.ts`) to populate a fresh
 * demo repo on first signup, and by `scripts/seed-excalidraw-tests.ts` as a
 * one-off CLI seeder against an existing repo.
 *
...
/**
 * Seeds the given repo with the Excalidraw functional areas + tests.
 *
 * Idempotent by default: if any tests already exist on the repo, the call is
 * a no-op. Pass `force: true` to wipe and reseed (used by the CLI script).
 *
 * Returns the count of inserted tests (0 when skipped).
 */
export async function seedExcalidrawTests(
  repositoryId: string,
  ...
  for (const faDef of FUNCTIONAL_AREA_DEFINITIONS) {
    ...
    await db.insert(functionalAreas).values({
      ...
      name: faDef.name,
      parentId,
      agentPlan: faDef.description ?? null,
      planGeneratedAt: faDef.description ? new Date() : null,
    });
    faMap.set(faDef.name, id);
    log(`  Created FA: ${faDef.name}`);
  }

  // Override columns are jsonb; the canned data carries them as JSON-encoded
  // strings, so parse before insert. Returns null on missing/invalid JSON.
  const parseOverride = <T>(raw: string | undefined): T | null => {
    if (!raw) return null;
    try {
  ...
  for (const testDef of TEST_DEFINITIONS) {
    ...
    await db.insert(tests).values({
      ...
      diffOverrides: parseOverride(testDef.diffOverrides),
      playwrightOverrides: parseOverride(testDef.playwrightOverrides),
      stabilizationOverrides: parseOverride(testDef.stabilizationOverrides),
      createdAt: now,
      updatedAt: now,
    });
