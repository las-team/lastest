...
export function normalizeColor(raw: string): string | null {
  ...
  if (rgb) {
    ...
    const a =
      rgb[4] !== undefined ? Math.round(parseFloat(rgb[4]) * 255) : null;
    const hh = (n: number) =>
...
export function parseDesignSystemCss(css: string): DesignSystemConfig {
  ...
  const resolve = (raw: string, depth = 0): string | null => {
    ...
    if (referencedList && referencedList.length > 0)
      return resolve(referencedList[0], depth + 1);
...
function buildGroups(resolved: Map<string, string>): DesignSystemGroups {
  ...
  for (const [name, value] of resolved) {
    ...
    // ── Colors ──
    if (asColor) {
      const tok: DesignRoleToken = {
        name,
        value: asColor,
        label: titleCase(nameSuffix(name)),
      };

      // Surfaces — neutrals that aren't part of the brand wheel.
      if (
        /^(c-)?(white|paper|surface|neutral|light|dark|soft(-\d+)?)$/.test(
          bare,
        ) ||
        bare.includes("-surface") ||
        bare.includes("-paper")
      ) {
        surfaces.push(tok);
        continue;
      ...
      if (
        /^(action|action-hover|action-secondary|action-secondary-hover|accent|brand|danger|warning|success|info|bg|bg-elevated|bg-inverted|border|border-strong|ring)/.test(
          bare,
        )
      ) {
      ...
      const hue = BRAND_HUES.find(
        (h) => bare === `c-${h}` || bare === h || bare === `brand-${h}`,
      );
      if (hue) {
    ...
    if (
      ...
      (bare.includes("font") || bare.includes("family"))
    ) {
  ...
  const byPx = (a: DesignRoleToken, b: DesignRoleToken) =>
...
export function mergeDesignSystemConfig(
  ...
  if (override?.tokens) {
    for (const [cat, list] of Object.entries(override.tokens) as Array<
      [DesignTokenCategory, DesignToken[] | undefined]
    >) {
