...
function emptyResult(
  overrides: Partial<NonNullable<ScoreInput>> = {},
): NonNullable<ScoreInput> {
  return {
    consoleErrors: null,
...
describe("scoreMultiLayer", () => {
  ...
  it("does NOT paint verdict red when only a third-party SDK fingerprint is new", () => {
    ...
    expect(r.evidence.find((e) => e.layer === "console")?.signal).toBe(
      "medium",
    );
  });
  ...
  it("returns red on URL-trajectory divergence", () => {
    const baseline = emptyResult({
      urlTrajectory: [
        {
          stepIndex: 0,
          finalUrl: "https://x/dashboard",
          redirectChain: [],
        } as UrlTrajectoryStep,
      ],
    });
    const current = emptyResult({
      urlTrajectory: [
      ...
      urlTrajectory: [
        {
          ...
          redirectChain: ["https://excalidraw.com/"],
        } as UrlTrajectoryStep,
      ],
    });
    const current = emptyResult({
  ...
  it("returns red on a new critical a11y violation", () => {
    ...
    const baseline = emptyResult();
    const current = emptyResult({
      a11yViolations: [v("button-name", "critical")],
    });
    const r = scoreMultiLayer({ baseline, current });
    expect(r.verdict).toBe("red");
  });
  ...
  it("returns yellow for structural-break in extracted variables", () => {
    ...
    const current = emptyResult({
      extractedVariables: { user: "alice", extra: "x" },
    });
    const r = scoreMultiLayer({ baseline, current });
    expect(r.verdict).toBe("red");
  });
