...
export function classifyConsoleFingerprint(
  ...
  if (urlInStack) {
    if (THIRD_PARTY_HOSTS.some((host) => urlInStack.includes(host)))
      return "thirdParty";
  }
  // 2. CSP violation reports.
  if (
    /Content Security Policy|Refused to (execute|load|connect|frame|apply)/i.test(
      rawMessage,
    )
  ) {
    return "csp";
...
export function fingerprintConsoleMessage(raw: string): {
  fingerprint: string;
  sample: string;
  category: ConsoleFingerprintCategory;
} {
...
export function summarizeConsoleDiff(d: ConsoleDiffSummary): string {
  if (
    d.newFingerprints.length === 0 &&
    d.disappeared.length === 0 &&
    Object.keys(d.countDelta).length === 0
  ) {
    return "No console changes";
  }
  const parts: string[] = [];
  if (d.newFingerprints.length)
    parts.push(`${d.newFingerprints.length} new console error(s)`);
