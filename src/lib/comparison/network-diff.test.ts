...
describe("normalizeRequestUrl", () => {
  it("strips noisy nonces and timestamps", () => {
    expect(
      normalizeRequestUrl("https://x.com/api?foo=1&_=12345&csrf=abc"),
    ).toBe("https://x.com/api?foo=1");
  });
  it("replaces digit-only path segments", () => {
    expect(normalizeRequestUrl("https://x.com/users/123/posts/456")).toBe(
      "https://x.com/users/:id/posts/:id",
    );
  });
  it("replaces hash-like path segments", () => {
    expect(
      normalizeRequestUrl(
        "https://x.com/files/a1b2c3d4e5f67890abcdef1234567890",
      ),
    ).toBe("https://x.com/files/:hash");
  });
  it("sorts retained query params", () => {
    expect(normalizeRequestUrl("https://x.com/api?b=2&a=1")).toBe(
      "https://x.com/api?a=1&b=2",
    );
  });
