...
import {
  ...
  classifyConsoleFingerprint,
} from "./console-diff";

describe("fingerprintConsoleMessage", () => {
  it("collapses messages that differ only by ID", () => {
    const a = fingerprintConsoleMessage(
      'TypeError: Cannot read property "id" of user 123',
    );
    const b = fingerprintConsoleMessage(
      'TypeError: Cannot read property "id" of user 456',
    );
  ...
  it("collapses messages that differ only by URL", () => {
    const a = fingerprintConsoleMessage(
      "Failed to load https://api.example.com/users/1",
    );
    const b = fingerprintConsoleMessage(
      "Failed to load https://api.example.com/users/2",
    );
    expect(a.fingerprint).toBe(b.fingerprint);
  });
  ...
  it("preserves a sample of the original head", () => {
    const result = fingerprintConsoleMessage(
...
describe("classifyConsoleFingerprint", () => {
  ...
  it("returns unknown when there is no stack URL and no message pattern", () => {
    expect(classifyConsoleFingerprint("something happened", "")).toBe(
      "unknown",
    );
  });
});
