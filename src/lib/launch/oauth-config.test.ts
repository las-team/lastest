...
describe('launch/oauth-config', () => {
  ...
  it('allows the production launch origin by default and localhost in dev', () => {
    ...
    expect(isAllowedRedirectUri('http://localhost:3002/launch/submit')).toBe(true);
  ...
  it('rejects localhost in production, accepts apex + subdomain', () => {
    ...
    expect(isAllowedRedirectUri('https://launch.lastest.cloud/auth/callback')).toBe(true);
    expect(isAllowedRedirectUri('https://lastest.cloud/launch/submit')).toBe(true);
    expect(isAllowedRedirectUri('https://www.lastest.cloud/launch/submit')).toBe(true);
