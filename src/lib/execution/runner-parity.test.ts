...
function embeddedRemoveFunction(
  body: string,
  funcName: string,
...
const TS_TORTURE_FIXTURES: Array<{
  name: string;
  input: string;
  ...
  },
  {
    ...
    input:
      'await page.evaluate(() => { const i = document.querySelector("div"); (i as HTMLElement).click(); });',
  ...
  },
  {
    ...
    input:
      'await page.evaluate(() => { document.querySelectorAll("a").forEach((el: HTMLElement) => el.click()); });',
...
describe("Runner Parity — Code Transformations", () => {
  ...
  describe("function removal (locateWithFallback, replayCursorPath)", () => {
    for (const tc of FUNCTION_REMOVAL_CASES) {
      it(`both produce identical output: ${tc.name}`, () => {
        ...
        if (tc.shouldRemove) {
          expect(runnerResult).toContain(
            `/* ${tc.funcName} provided by runner */`,
          );
  ...
  describe("assertion instrumentation (shared helper used by both runners)", () => {
    it("wraps await expect(page).toHaveURL with __assertion(id, async () => { ... })", () => {
      ...
      const { instrumentedBody, wrappedCount } = instrumentAssertionTracking(
        body,
        [{ id: "aaa111" }],
      );
      expect(wrappedCount).toBe(1);
      expect(instrumentedBody).toContain('await __assertion("aaa111"');
      expect(instrumentedBody).toContain(
    ...
    it("preserves indentation", () => {
      ...
      const { instrumentedBody } = instrumentAssertionTracking(body, [
        { id: "i1" },
      ]);
