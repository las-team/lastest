...
describe("Embedded Browser Integration", () => {
  describe("EmbeddedTestExecutor lifecycle", () => {
    ...
    it("abort() returns false when no test is running", () => {
      const abort = (controller: AbortController | null) => {
        if (controller) {
          controller.abort();
          return true;
        }
  ...
  describe("Test result shape", () => {
    it("passed result has correct structure", () => {
      const result = {
        ...
        durationMs: 1234,
        logs: [
          { timestamp: Date.now(), level: "info", message: "Test passed" },
        ],
        screenshots: [
          {
            filename: "run-1-test-1-success.png",
            data: "base64data",
            width: 1280,
            height: 720,
          },
  ...
  describe("Command routing (from index.ts)", () => {
    ...
    it("maps command types to response types", () => {
      ...
      expect(commandToResponse["command:run_test"]).toBe(
        "response:test_result",
      );
      expect(commandToResponse["command:ping"]).toBe("response:pong");
      expect(commandToResponse["command:capture_screenshot"]).toBe(
        "response:screenshot",
      );
    });
  });
  ...
  describe("Stabilization delegation", () => {
    ...
    it("stabilization is called before navigation and before screenshots", () => {
      ...
      expect(executionOrder.indexOf("setupFreezeScripts")).toBeLessThan(
        executionOrder.indexOf("page.goto"),
      );
      expect(
        executionOrder.indexOf("applyPreScreenshotStabilization"),
      ).toBeLessThan(executionOrder.indexOf("page.screenshot"));
    });
  });
