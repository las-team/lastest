...
async function executeApiTests(
  ...
  for (const test of apiTests) {
    ...
    if (!def) {
      ...
      const loadResult = await runApiLoadTest(def, test.loadConfig, { baseUrl });
      result = {
        testId: test.id,
        status: loadResult.passed ? 'passed' : 'failed',
        durationMs: Date.now() - started,
        screenshots: [],
        errorMessage: loadResult.passed ? undefined : `Load thresholds breached: ${loadResult.breaches.join('; ')}`,
        loadResult,
      };
    } else {
      ...
      result = {
        ...
        errorMessage: apiResult.passed
          ? undefined
          : apiResult.error ?? `${apiResult.assertionResults.filter((a) => !a.passed).length} assertion(s) failed`,
        apiResult,
      };
    }
    results.push(result);
...
export async function executeTests(
  ...
  if (apiTests.length > 0) {
    const apiResults = await executeApiTests(apiTests, runId, options, onResult);
    if (browserTests.length === 0) return apiResults;
    const browserResults = await executeTests(browserTests, runId, options, onProgress, onResult);
