 ...
 */
async function buildAIVarRuntime(
  repositoryId?: string | null,
): Promise<AIVarRuntime | null> {
  ...
  const config: AIProviderConfig = {
    ...
    agentSdkPermissionMode: (settings.agentSdkPermissionMode ??
      undefined) as AIProviderConfig["agentSdkPermissionMode"],
  ...
  return {
    async generate(variable: TestVariable): Promise<string> {
      ...
      if (!prompt)
        throw new Error(`AI variable "${variable.name}" has no prompt`);
      ...
      try {
        ...
        if (!cleaned)
          throw new Error(`AI returned empty value for "${variable.name}"`);
        return cleaned;
      } finally {
...
async function resolveTestCodeForRunner(
  ...
  if (test.variables && test.variables.length > 0 && code.includes("{{var:")) {
    const r = await resolveVarReferencesAsync(
      code,
      test.variables,
      gsheetSources,
      csvSources,
  ...
  const { values: assignedVariables, nextLastValues: assignedNextLastValues } =
 ...
 */
function buildStabilizationPayload(
  settings?: PlaywrightSettings | null,
  testOverrides?: Partial<StabilizationSettings> | null,
): StabilizationPayload | undefined {
  ...
  return {
    ...
    freezeAnimations:
      testOverrides?.freezeAnimations ?? settings?.freezeAnimations ?? false,
...
export async function executeTests(
  ...
  if (options.teamId) {
    const runner = await getAvailableRunnerById(
      options.teamId,
      options.runnerId,
    );
    if (runner) {
    ...
    console.warn(
      `Runner ${options.runnerId} not available, using fallback chain`,
    );
  } else {
    console.warn(
      "No teamId provided for runner execution, using fallback chain",
    );
  }
...
export async function executeSetupViaRunner(
  ...
  console.log(
    `[Executor] Queuing setup command ${command.id.slice(0, 8)} for runner ${runnerId}`,
  );
  ...
  while (Date.now() - startTime < maxWait) {
    ...
    if (healthCheckCounter % 5 === 0) {
      ...
      if (runnerRow?.status === "offline") {
        throw new Error(
          `Setup failed: Embedded browser went offline during setup (possible crash-loop). Check container logs for runner ${runnerId.slice(0, 8)}.`,
        );
      }
    }
    ...
    if (
      dbCmd.status === "completed" ||
      dbCmd.status === "failed" ||
      dbCmd.status === "timeout"
    ) {
      // Get the result
      const results = await getUnacknowledgedResults([command.id]);
      const setupResult = results.find(
        (r) => r.type === "response:setup_result",
      );
      ...
      if (setupResult) {
        ...
        if (payload.status === "passed") {
          console.log(
            `[Executor] Setup completed successfully on runner ${runnerId}`,
          );
          return {
          ...
          throw new Error(
            `Remote setup failed: ${payload.error || "Unknown error"}`,
          );
        }
      }
...
async function executeViaRunner(
  ...
  if (options.setupInfo) {
    console.log(
      `[Executor] Running setup on remote runner ${runnerId} before tests...`,
    );
    ...
    options.setupContext = {
      storageState:
        setupResult.storageState ?? options.setupContext?.storageState,
      variables: {
        ...options.setupContext?.variables,
        ...setupResult.variables,
      },
    };
    console.log(
      `[Executor] Remote setup complete, storageState: ${setupResult.storageState ? "yes" : "no"}`,
    );
  }
  ...
  if (!options.maxParallelTests) {
    const [runnerRecord] = await db
      .select({ maxParallelTests: runners.maxParallelTests })
      .from(runners)
      .where(eq(runners.id, runnerId));
  ...
  try {
    ...
    const diffSettings = await getDiffSensitivitySettings(
    ...
    console.warn(
      "[executor] Failed to load diff-sensitivity settings for text capture:",
      err,
    );
  }
  ...
  const fillSlots = async () => {
    ...
    while (inFlight.size < maxParallel && pending.length > 0) {
      ...
      for (const fixture of testFixtureRecords) {
        try {
          const absPath = path.join(
            STORAGE_DIRS.fixtures,
            fixture.storagePath.replace(/^\/fixtures\//, ""),
          );
          ...
          console.warn(
            `[Executor] Failed to read fixture ${fixture.filename}: ${err}`,
          );
        }
      }
      ...
      const effectiveBrowser =
        pwOverrides?.browser ??
        ((options.playwrightSettings?.browser as
          | "chromium"
          | "firefox"
          | "webkit") ||
          undefined);
      ...
      try {
        ...
        if (recVp?.viewportWidth && recVp?.viewportHeight) {
          recordingViewport = {
            width: recVp.viewportWidth,
            height: recVp.viewportHeight,
          };
        }
      } catch {
        // Non-critical
      ...
      const {
        ...
        nextAiLastValues,
        preflightErrors,
      } = await resolveTestCodeForRunner(
        test,
        gsheetSources,
        csvSources,
        aiVarRuntime,
      ...
      const failOnRuntimeError = !(test.stepCriteria ?? []).some((c) =>
        c.rules.some(
          (r) => r.kind === "all_steps_executed" && r.severity === "warn",
        ),
      );
      ...
      let selectorStatsForRunner: Awaited<
        ReturnType<typeof getSelectorStatsForTest>
      > = [];
      try {
        selectorStatsForRunner = await getSelectorStatsForTest(test.id);
      } catch (err) {
        console.warn(
          `[executor] Failed to load selector_stats for test ${test.id}:`,
          err,
        );
      }
      ...
      const designSystemPayload = (() => {
        ...
        if (!effective) {
          console.warn(
            `[executor] design-system: toggle on but no token bundle for test ${test.id}; upload one on the Setup tab.`,
          );
          return undefined;
        }
        if (!isConfigUsable(effective)) {
          console.warn(
            `[executor] design-system: token bundle has 0 usable values for test ${test.id}; re-upload may be required.`,
          );
          return undefined;
        }
        ...
        console.log(
          `[executor] design-system: sending ${tokenCount} tokens to EB for test ${test.id}`,
        );
        return {
      ...
      const command = createMessage<RunTestCommand>("command:run_test", {
        ...
        cursorPlaybackSpeed:
          options.cursorPlaybackSpeedOverride ??
          pwOverrides?.cursorPlaybackSpeed ??
          options.playwrightSettings?.cursorPlaybackSpeed ??
          1,
        stabilization: buildStabilizationPayload(
          options.playwrightSettings,
          test.stabilizationOverrides,
        ),
        ...
        ignoreExternalNetworkErrors:
          options.playwrightSettings?.ignoreExternalNetworkErrors ?? true,
        ...
        selectorStats:
          selectorStatsForRunner.length > 0
            ? selectorStatsForRunner
            : undefined,
        selectorTimeoutMs:
  ...
  while (inFlight.size > 0) {
    ...
    for (const dbCmd of dbCommands) {
      ...
      const testResultMsg = unacked.find(
        (r) => r.type === "response:test_result",
      );
      ...
      if (
        dbCmd.status === "timeout" ||
        (dbCmd.status === "failed" && !testResultMsg)
      ) {
      ...
      const screenshotResults = unacked.filter(
        (r) => r.type === "response:screenshot",
      );
      const expectedCount =
        typeof payload.screenshotCount === "number"
          ? payload.screenshotCount
          : undefined;
      const allScreenshotsReceived =
        expectedCount !== undefined &&
        screenshotResults.length >= expectedCount;

      if (
        payload.status === "passed" &&
        !allScreenshotsReceived &&
        expectedCount !== 0
      ) {
      ...
      if (allScreenshots.length === 0) {
        allScreenshots = await findScreenshotsOnDisk(
          runId,
          info.testId,
      ...
      const networkBodiesResult = unacked.find(
        (r) => r.type === "response:network_bodies",
      );
      const networkBodiesPath = networkBodiesResult
        ? ((networkBodiesResult.payload as Record<string, unknown>)?.path as
            | string
            | undefined)
        : undefined;
      ...
      const testResult: TestRunResult = {
        ...
        status:
          ...
          payload.status === "cancelled"
            ? "failed"
            : (payload.status as "passed" | "failed"),
        ...
        consoleErrors:
          ...
          payload.consoleErrors.length > 0
            ...
            : undefined,
        networkRequests:
          Array.isArray(payload.networkRequests) &&
          payload.networkRequests.length > 0
            ? (payload.networkRequests as NetworkRequest[])
            : undefined,
        downloads:
          Array.isArray(payload.downloads) && payload.downloads.length > 0
            ? (payload.downloads as DownloadRecord[])
            : undefined,
        softErrors:
          Array.isArray(payload.softErrors) && payload.softErrors.length > 0
            ? (payload.softErrors as string[])
            : undefined,
        assertionResults:
          Array.isArray(payload.assertionResults) &&
          payload.assertionResults.length > 0
            ? (payload.assertionResults as import("@/lib/db/schema").AssertionResult[])
            : undefined,
        videoPath,
        networkBodiesPath,
        domSnapshot: payload.domSnapshot as
          | import("@/lib/db/schema").DomSnapshotData
          | undefined,
        lastReachedStep:
          typeof payload.lastReachedStep === "number"
            ? payload.lastReachedStep
            : undefined,
        totalSteps:
        ...
        extractedVariables:
          ...
          typeof payload.extractedVariables === "object"
        ...
        a11yViolations: Array.isArray(payload.a11yViolations)
          ...
          : undefined,
        a11yPassesCount:
          typeof payload.a11yPassesCount === "number"
        ...
        designSystemTokenUsage:
          payload.designSystemTokenUsage &&
          typeof payload.designSystemTokenUsage === "object"
            ? (payload.designSystemTokenUsage as import("@/lib/db/schema").DesignSystemTokenUsage)
            : undefined,
        urlTrajectory:
          Array.isArray(payload.urlTrajectory) &&
          payload.urlTrajectory.length > 0
            ? (payload.urlTrajectory as import("@/lib/db/schema").UrlTrajectoryStep[])
            : undefined,
        webVitals:
          Array.isArray(payload.webVitals) && payload.webVitals.length > 0
    ...
    for (const [commandId, info] of inFlight) {
      ...
      if (Date.now() - info.startTime > hostDeadlineMs) {
        ...
        const lastSeenStr =
          lastSeenMs >= 0 ? `${Math.round(lastSeenMs / 1000)}s ago` : "never";
        ...
        const probe = await db
          .select({
            id: runnerCommandResults.id,
            type: runnerCommandResults.type,
            payload: runnerCommandResults.payload,
          })
        ...
        const beaconPayload = (stepBeacon?.payload ?? null) as {
          stepIndex?: number;
          totalSteps?: number;
          status?: string;
        } | null;
        ...
        const stepProgress =
          beaconPayload &&
          typeof beaconPayload.stepIndex === "number" &&
          typeof beaconPayload.totalSteps === "number"
...
async function probeEBHealth(runnerId: string): Promise<boolean> {
  try {
    ...
    const healthUrl =
      row.containerUrl.replace(/:(\d+)(?=$|\/)/, ":9224") + "/health";
    const controller = new AbortController();
...
async function executeViaPoolWorkers(
  ...
  const { claimOrProvisionPoolEB, releasePoolEB } =
  ...
  try {
    const claimMaxWaitMs = parseInt(
      process.env.EB_CLAIM_MAX_WAIT_MS || "120000",
      10,
    );
    ...
    const EB_INFRA_ERR_RX =
      /offline|crash|runner went|ECONNREFUSED|EB network unhealthy|EB-stalled/i;
    const EB_MAYBE_INFRA_ERR_RX =
      /Target .*has been closed|page\.screenshot.*Target page.*closed/i;
    ...
    if (options.setupInfo) {
      ...
      for (let attempt = 1; attempt <= MAX_EB_ATTEMPTS; attempt++) {
        ...
        console.log(
          `[Dispatch] Claimed EB ${eb.runnerId.slice(0, 8)} for broadcast setup (attempt ${attempt}/${MAX_EB_ATTEMPTS})`,
        );
        try {
          const setupResult = await executeSetupViaRunner(
            options.setupInfo.code,
          ...
          const broadcastState =
            setupResult.storageStateJson ?? options.setupContext?.storageState;
          if (!setupResult.storageStateJson) {
            console.warn(
              `[Dispatch] Setup returned no storageStateJson — tests will cold-start without injected state. Expected for no-auth apps; surprising otherwise.`,
            );
          }
          effectiveOptions = {
            ...options,
          ...
          console.log(
            `[Dispatch] Broadcast setup complete (storageState: ${broadcastState ? "captured" : "none"})`,
          );
          setupErr = undefined;
          break;
        } catch (err) {
          ...
          if (
            ...
            (await isDeadEBError(msg, eb.runnerId))
          ) {
            console.warn(
              `[Dispatch] Broadcast setup hit dead-EB on attempt ${attempt}, retrying: ${msg}`,
            );
            continue;
          }
          break;
      ...
      if (setupErr) {
        ...
        for (const r of failed) {
          try {
            ...
            console.error(
              "[Dispatch] onResult threw for broadcast-setup-failed test:",
              err,
            );
          }
        }
    ...
    const fireResult = async (r: TestRunResult): Promise<TestRunResult> => {
      try {
        ...
        console.error("[Dispatch] onResult threw:", err);
      }
      return r;
    };
    ...
    const runOneTest = async (test: Test): Promise<TestRunResult> => {
      ...
      for (let attempt = 1; attempt <= MAX_EB_ATTEMPTS; attempt++) {
        ...
        try {
          ...
          const labels =
            (result?.screenshots ?? []).map((s) => s.label).join(",") || "none";
          console.log(
          ...
          if (deadResult) {
            ...
            console.warn(
              `[Dispatch] Dead-EB result on attempt ${attempt} for "${test.name}", retrying: ${lastError}`,
            );
          ...
          if (
            ...
            console.warn(
              `[Dispatch] Dead-EB exception on attempt ${attempt} for "${test.name}", retrying: ${msg}`,
            );
    ...
    console.log(
      `[Dispatch] Starting build run=${runId} — ${tests.length} tests, concurrency=${maxParallelEBs}`,
    );
    ...
    console.log(
      `[Dispatch] Build run=${runId} done — ${completedCount}/${tests.length} tests dispatched`,
    );
    ...
    for (const t of tests) {
      ...
      if (r) {
        ...
        try {
          await onResult?.(missing);
        } catch (err) {
          console.error(
            "[Dispatch] onResult threw for missing-result test:",
            err,
          );
        }
...
async function findScreenshotsOnDisk(
      ...
      for (const f of files) {
        if (f.startsWith(prefix) && f.endsWith(".png")) {
          const label = f
            ...
            .replace(/_/g, " ");
          screenshots.push({
            path: `/screenshots/${repositoryId}/${f}`,
            label,
          });
        }
      }
  ...
  if (screenshots.length > 0) {
    console.log(
      `[Executor] Found ${screenshots.length} screenshots on disk for ${testId}`,
    );
  }
...
async function executeFallbackChain(
  ...
  const recordActualRunner = async (runnerId: string) => {
    if (options.jobId) {
      const { updateBackgroundJob } =
        await import("@/lib/db/queries/background-jobs");
  ...
  const maxParallelEBs = Math.max(
    1,
    options.playwrightSettings?.maxParallelEBs ?? 10,
  );
  ...
  console.log(
    "Fallback chain: no runner available, tests queued for later execution",
  );
