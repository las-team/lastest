    ...
    const historyDesc =
      input.recentHistory.length > 0
        ? input.recentHistory
            .map(
              (r, i) =>
                `Run ${i + 1}: ${r.status}${r.errorMessage ? ` — ${r.errorMessage.slice(0, 200)}` : ""}`,
            )
            .join("\n")
...
${historyDesc}`;
    ...
    if (!parsed) {
      return {
        classification: "unknown",
        confidence: 0,
        reasoning: "AI response not parseable as JSON",
      };
    }
    ...
    const classification = (validClassifications as string[]).includes(
      rawClassification,
    )
    ...
    return {
      ...
      reasoning: String(parsed.reasoning || "No reasoning provided").slice(
        0,
        1000,
      ),
    };
 ...
 */
export async function triageBuildFailures(
  buildId: string,
  repositoryId: string,
): Promise<void> {
  try {
    ...
    if (!settings.aiDiffingEnabled) {
      console.log(
        `[triage] Skipping triage for build ${buildId} — AI Diff Analysis is disabled`,
      );
      return;
    }
    ...
    console.log(
      `[triage] Triaging ${failed.length} failed test(s) for build ${buildId}`,
    );
    ...
    for (const result of failed) {
      ...
      console.log(
        `[triage] ${test.name}: ${triageResult.classification} (${Math.round(triageResult.confidence * 100)}%) — ${triageResult.reasoning}`,
      );
    }
  } catch (error) {
