...
// Promoted to `libs/ai-kit` (RFC §9 phase 4, `authoring-ai` migration) —
// pure, and shared by core and several unmigrated pseudo-plugins before
// `authoring-ai` needed it too. Imported (not just re-exported) because
// several prompt templates below still interpolate
// `${SELECTOR_ROBUSTNESS_RULES}` directly; re-exported so this file's other
// callers (`play-agent.ts`, `spec-import.ts`, `specs.ts`, `ai.ts`) keep
// importing both names from `@/lib/ai/prompts` unchanged.
import {
  extractCodeFromResponse,
  SELECTOR_ROBUSTNESS_RULES,
} from "@lastest/ai-kit";
export { extractCodeFromResponse, SELECTOR_ROBUSTNESS_RULES };
