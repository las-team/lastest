...
const DEP_INSIGHTS: Record<string, Omit<DependencyInsight, "name">> = {
  ...
  "framer-motion": {
    category: "animation",
    testingImplication:
      "Wait for animations to complete before screenshots; use page.waitForTimeout(500) after interactions",
  },
  "react-spring": {
    category: "animation",
    testingImplication:
      "Wait for spring animations to settle before capturing screenshots",
  },
  gsap: {
    category: "animation",
    testingImplication:
      "Wait for GSAP animations to complete; consider disabling via matchMedia",
  },
  "@formkit/auto-animate": {
    category: "animation",
    testingImplication: "List animations may affect screenshot timing",
  },
  ...
  "next-auth": {
    category: "auth",
    testingImplication:
      "Requires auth setup; login flow needed before testing protected routes",
  },
  "@auth/core": {
    category: "auth",
    testingImplication:
      "Auth.js integration — requires session setup for protected routes",
  },
  "@clerk/nextjs": {
    category: "auth",
    testingImplication:
      "Clerk auth — protected routes need auth bypass or login setup",
  },
  "@clerk/clerk-react": {
    category: "auth",
    testingImplication:
  ...
  firebase: {
    category: "auth",
    testingImplication:
      "Firebase auth — may need emulator or test credentials for protected routes",
  },
  "@supabase/auth-helpers-nextjs": {
    category: "auth",
    testingImplication:
      "Supabase auth — protected routes need auth session setup",
  },
  lucia: {
    category: "auth",
    testingImplication:
      "Lucia auth — requires session cookie setup for protected routes",
  },
  passport: {
    category: "auth",
    testingImplication:
      "Passport.js auth — login flow required for protected routes",
  },
  ...
  "@tanstack/react-query": {
    category: "data",
    testingImplication:
      "Async data fetching — wait for network idle or specific elements before assertions",
  },
  swr: {
    category: "data",
    testingImplication:
      "SWR data fetching — pages may re-render after stale-while-revalidate; wait for content",
  },
  axios: {
    category: "data",
    testingImplication:
      "HTTP client — pages likely fetch data on load; wait for content to appear",
  },
  "@trpc/client": {
    category: "data",
    testingImplication:
      "tRPC client — type-safe API calls; wait for data to load",
  },
  "@apollo/client": {
    category: "data",
    testingImplication:
      "GraphQL client — queries may have loading states; wait for data",
  },
  "graphql-request": {
    category: "data",
    testingImplication:
      "GraphQL — pages may show loading states; wait for content",
  },
  ...
  "react-hook-form": {
    category: "form",
    testingImplication:
      "Form validation may be client-side; test both valid and invalid submissions",
  },
  formik: {
    category: "form",
    testingImplication:
      "Formik forms — test validation messages and submission states",
  },
  "@conform-to/react": {
    category: "form",
    testingImplication:
      "Conform forms with server validation — test error states",
  },
  zod: {
    category: "form",
    testingImplication:
      "Schema validation — forms likely have strict validation rules to test",
  },
  ...
  "@radix-ui/react-dialog": {
    category: "ui",
    testingImplication:
      "Radix dialogs use [data-state] attributes; use role-based selectors",
  },
  "@radix-ui/react-dropdown-menu": {
    category: "ui",
    testingImplication:
      'Radix dropdowns — use role="menuitem" for option selection',
  },
  "@headlessui/react": {
    category: "ui",
    testingImplication:
      "Headless UI — components are accessible; prefer aria-label and role selectors",
  },
  "@mui/material": {
    category: "ui",
    testingImplication:
      "MUI components — use data-testid or aria-label; avoid class-based selectors",
  },
  antd: {
    category: "ui",
    testingImplication:
      "Ant Design — components have ant-* class prefixes; prefer data-testid",
  },
  "@chakra-ui/react": {
    category: "ui",
    testingImplication:
      "Chakra UI — accessible components; prefer role and aria-label selectors",
  },
  shadcn: {
    category: "ui",
    testingImplication:
      "shadcn/ui with Radix primitives — use role-based and [data-state] selectors",
  },
  ...
  zustand: {
    category: "state",
    testingImplication:
      "Zustand state — UI may update asynchronously; wait for state-driven content",
  },
  jotai: {
    category: "state",
    testingImplication:
      "Jotai atoms — UI reactivity; wait for derived state to settle",
  },
  redux: {
    category: "state",
    testingImplication:
      "Redux state — actions may trigger async updates; wait for UI to reflect changes",
  },
  "@reduxjs/toolkit": {
    category: "state",
    testingImplication:
      "RTK — may use createAsyncThunk; wait for loading states to resolve",
  },
  recoil: {
    category: "state",
    testingImplication:
      "Recoil atoms — async selectors may cause loading states",
  },
  mobx: {
    category: "state",
    testingImplication:
      "MobX observables — reactions may cause delayed UI updates",
  },

  // Routing hints
  "react-router-dom": {
    category: "routing",
    testingImplication: "Client-side routing — use waitForURL after navigation",
  },
  next: {
    category: "routing",
    testingImplication:
      "Next.js routing — pages may have loading.tsx states; wait for content",
  },
  "@tanstack/react-router": {
    category: "routing",
    testingImplication:
      "TanStack Router — type-safe routes with loader patterns",
  },
  "vue-router": {
    category: "routing",
    testingImplication:
      "Vue Router — client-side navigation; wait for route transitions",
  },
  ...
  "next-intl": {
    category: "other",
    testingImplication:
      "i18n — content may vary by locale; test default locale",
  },
  i18next: {
    category: "other",
    testingImplication:
      "i18n — text content from translation keys; use getByText with actual translated text",
  },
  "next-themes": {
    category: "other",
    testingImplication:
      "Theme switching — screenshots may vary by theme; test default theme",
  },
  "@vercel/analytics": {
    category: "other",
    testingImplication: "Analytics scripts — may add loading delay",
  },
};
...
function detectAuthMechanism(deps: Record<string, string>): string {
  ...
  if ("@supabase/auth-helpers-nextjs" in deps || "@supabase/ssr" in deps)
...
function detectFramework(
  ...
  if ("next" in all) {
    ...
    if (hasAppDir && major >= 13)
      return `Next.js ${major || ""} App Router`.trim();
...
function inferSelectorStrategy(
  deps: Record<string, string>,
  sampleSelectors: string[],
): string {
...
export async function gatherCodebaseIntelligence(
  ...
  const [packageJsonRaw, readmeRaw, repoTree, nextConfigRaw] =
    await Promise.all([
      getFileContent(accessToken, owner, repo, "package.json", branch).catch(
        () => null,
      ),
      ...
      getFileContent(accessToken, owner, repo, "next.config.js", branch)
        .catch(() =>
          getFileContent(accessToken, owner, repo, "next.config.ts", branch),
        )
        .catch(() =>
          getFileContent(accessToken, owner, repo, "next.config.mjs", branch),
        )
        .catch(() => null),
    ]);
  ...
  const hasAppDir = tree.some(
    (e) => e.path.startsWith("app/") || e.path.startsWith("src/app/"),
  );
  const hasPagesDir = tree.some(
    (e) => e.path.startsWith("pages/") || e.path.startsWith("src/pages/"),
  );
  ...
  if (sampleFile) {
    const sampleCode = await getFileContent(
      accessToken,
      owner,
      repo,
      sampleFile.path,
  ...
  if (framework.includes("Next.js") && framework.includes("App Router")) {
    testingRecommendations.push(
      "[Next.js App Router] Pages may use Suspense boundaries — wait for loading states to resolve",
    );
    testingRecommendations.push(
      "[Next.js App Router] Server Components render HTML first — content should be immediately visible",
    );
  }

  if (nextConfigRaw?.includes("i18n")) {
    testingRecommendations.push(
      "[i18n config] App has locale routing — test default locale only unless specified",
    );
  }
