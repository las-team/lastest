...
async function withAILimit<T>(fn: () => Promise<T>): Promise<T> {
  ...
  try {
    ...
    activeAI--;
    if (waitingAI.length > 0) waitingAI.shift()!();
  }
}
 ...
 */
export async function triggerAIDiffAnalysis(
  diffId: string,
  ...
  return withAILimit(async () => {
    ...
    try {
      ...
      if (
        !settings.aiDiffingEnabled ||
        !(await queries.getInProductAiEnabled(repositoryId))
      ) {
      ...
      const rawProvider =
        (settings.aiDiffingProvider as AIDiffingProvider) || null;
      ...
      if (rawProvider === "same-as-test-gen") {
        ...
        if (settings.provider === "ollama") {
          ...
          effectiveProvider =
            settings.provider === "claude-agent-sdk"
              ? "claude-agent-sdk"
              : settings.provider;
          effectiveApiKey =
            settings.provider === "claude-agent-sdk"
      ...
      if (parentJobId) {
        childJobId = await createChildJob(
          "ai_diff",
          `AI Diff: ${testName}`,
          parentJobId,
          repositoryId,
      ...
      if (childJobId)
        await failJob(
          childJobId,
          error instanceof Error ? error.message : "AI diff analysis failed",
        );
    }
  });
}
