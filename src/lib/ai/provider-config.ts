...
export function aiModelId(config: AIProviderConfig): string {
  switch (config.provider) {
    case "openrouter":
      return `openrouter:${config.openrouterModel ?? "default"}`;
    case "anthropic":
      return `anthropic:${config.anthropicModel ?? "default"}`;
    case "openai":
      return `openai:${config.openaiModel ?? "default"}`;
    case "ollama":
      return `ollama:${config.ollamaModel ?? "default"}`;
    default:
