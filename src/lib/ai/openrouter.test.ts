...
describe("extractFallbackToolCall", () => {
  it("parses <tool_call>…</tool_call> tag form (Nemotron / Hermes)", () => {
    const content =
      '<tool_call>{"name":"browser_snapshot","arguments":{"url":"https://x"}}</tool_call>';
  ...
  it("parses bare JSON object in content", () => {
    const content =
      'okay -> {"name":"browser_navigate","arguments":{"url":"/home"}}';
    expect(extractFallbackToolCall(content)).toEqual({
  ...
  it("accepts `parameters` as an alias for `arguments`", () => {
    const content =
      '<tool_call>{"name":"foo","parameters":{"x":1}}</tool_call>';
    expect(extractFallbackToolCall(content)).toEqual({
      name: "foo",
      arguments: { x: 1 },
    });
  ...
  it("parses string-encoded arguments", () => {
    const content =
      '<tool_call>{"name":"foo","arguments":"{\\"x\\":2}"}</tool_call>';
    expect(extractFallbackToolCall(content)).toEqual({
      name: "foo",
      arguments: { x: 2 },
    });
