...
export function buildConfigSkeleton(
  ...
  const sf = source.config as SalesforceConnectorConfig;
  const vault = target.config as VaultConnectorConfig;
  ...
  const skeleton: MigrationConfigSkeleton = {
    ...
    source: {
      ...
      // two formats.
      apiVersion: (sf.apiVersion || DEFAULT_SALESFORCE_API_VERSION).replace(
        /^v/,
        "",
      ),
      auth: {
    ...
    target: {
      ...
      apiVersion: vault.apiVersion || DEFAULT_VAULT_API_VERSION,
      auth: {
  ...
  if (input.stateDatabaseUrl || project.runDir) {
    skeleton.staging = {
      ...(input.stateDatabaseUrl
        ? { databaseUrl: input.stateDatabaseUrl }
        : {}),
      ...(project.runDir ? { runDir: project.runDir } : {}),
    };
  }
...
export function describeEndpoints(input: BuildConfigInput): {
  ...
  const sf = (input.source?.config ?? {}) as SalesforceConnectorConfig;
  const vault = (input.target?.config ?? {}) as VaultConnectorConfig;
  const env = (e?: Environment | null) => (e ? ` (${e.label})` : "");
  return {
    source:
