import type { AgentStepId, AgentStepState } from "@/lib/db/schema";

/**
 * Pure step-definition data + the initial-state builder, split out of
 * `src/server/actions/quickstart-agent.ts` (a `"use server"` module) because
 * Next.js requires every top-level export of a `"use server"` file to be an
 * async function — `buildInitialQsSteps` never had a reason to be one, it
 * just builds a plain array in memory.
 */
...
export const QS_STEP_ORDER: AgentStepId[] = QS_STEP_DEFINITIONS.map(
  (s) => s.id,
);

// Used by integration tests to drive a quickstart run without a session —
// `startQuickstart` gates on `requireRepoAccess`, which needs
// `headers()`/cookies unavailable outside a real Next.js request. See
