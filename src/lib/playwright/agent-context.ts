...
export async function buildSeedFixture(
  ...
  if (!envConfig?.baseUrl) {
    throw new Error(
      `Base URL not configured for repository ${repositoryId}. Complete env setup first.`,
    );
  }
  ...
  if (setupSteps.length > 0) {
    ...
    for (const step of setupSteps) {
      ...
      const name =
        step.testName || step.scriptName || `Step ${step.orderIndex}`;
      if (code) {
        seedParts.push(`// ${name}\n${code}`);
      }
      if (step.storageStateName) {
        seedParts.push(
          `// Storage state: ${step.storageStateName} (pre-authenticated browser state)`,
        );
      }
    }
    ...
    if (seedParts.length > 0) {
      ...
      parts.push(
        `This seed test sets up the environment (e.g. login, authentication).`,
      );
      parts.push(
        `Execute these steps using MCP browser tools before exploring the app.`,
      );
      if (hasLoginSetup) {
        parts.push(
          `**IMPORTANT: Do NOT include these login/setup steps in your generated test code. A separate setup script runs them automatically before each test at runtime. Your test should assume the user is already logged in.**`,
        );
      }
  ...
  if (routes.length > 0) {
    ...
    parts.push(
      `These routes were discovered from the codebase. Use them as starting points for exploration.`,
    );
  ...
  if (intelligence) {
    ...
    if (intelligence.framework)
      parts.push(`- Framework: ${intelligence.framework}`);
    if (intelligence.cssFramework)
      parts.push(`- CSS: ${intelligence.cssFramework}`);
    if (intelligence.selectorStrategy)
      parts.push(`- Selector strategy: ${intelligence.selectorStrategy}`);
    if (
      intelligence.authMechanism &&
      intelligence.authMechanism !== "none detected"
    ) {
