...
export async function injectRecordingListeners(
    ...
    await page.exposeFunction(
      ...
      (
        ...
        if (action === "fill" && selectors.length > 0) {
          ...
          const now = Date.now();
          if (
            selectorKey === lastFillSelector &&
            now - lastFillTimestamp < FILL_COALESCE_MS
          ) {
            // Replace the last fill event
            const lastIdx = session.events.length - 1;
            if (
              lastIdx >= 0 &&
              session.events[lastIdx].type === "action" &&
              session.events[lastIdx].data.action === "fill"
            ) {
  ...
  const initFn = () => {
    ...
    if ((window as unknown as Record<string, unknown>).__debugRecordingActive)
      return;
    (window as unknown as Record<string, unknown>).__debugRecordingActive =
      true;
    ...
    function generateSelectors(
      ...
      if (
        ...
        selectors.push({
          type: "text",
          value: `text="${element.textContent.trim().slice(0, 30)}"`,
        });
      }
    ...
    function getImplicitRole(el: HTMLElement): string | null {
      ...
      if (tag === "li") {
        const parent = el.closest(
          '[role="listbox"], [role="menu"], [role="tablist"], [role="tree"]',
        );
        if (parent) {
    ...
    let pointerDownSelectors: Array<{ type: string; value: string }> | null =
      null;
