...
export async function runRoutePlanner(
  ...
  try {
    ...
    const groupedByArea = new Map<
      string,
      { name: string; description?: string; routes: typeof routes }
    >();

    for (const route of routes) {
      const area = route.functionalAreaId
        ? areaMap.get(route.functionalAreaId)
        : null;
      ...
      if (!groupedByArea.has(key)) {
        if (area) {
          ...
          groupedByArea.set(key, {
            name: area.name,
            description: area.agentPlan || undefined,
            routes: [],
          });
        } else {
    ...
    return {
      ...
      areas: plannerAreas,
      durationMs: Date.now() - start,
      inputSummary: `${routes.length} routes, ${areas.length} existing areas`,
    };
  } catch (error) {
    return {
      source: "routes",
...
function buildRouteTestPlan(
  ...
  for (const route of groupRoutes) {
    ...
    lines.push(
      `- Navigate to ${route.path} and verify page loads without errors`,
    );
