  ...
  try {
    ...
    if (!repoTree || repoTree.tree.length === 0) {
      return {
        ...
        areas: [],
        durationMs: Date.now() - start,
        inputSummary: `branch: ${branch}, 0 spec files`,
      };
    }
    ...
    const storiesResult = await extractUserStoriesFromFiles(
      repositoryId,
      branch,
      filePaths,
    );
    ...
    return {
      ...
      error:
        storiesResult.error ||
        `${specEntries.length} spec files found, extraction failed`,
      durationMs,
      inputSummary: `branch: ${branch}, ${specEntries.length} spec files (${filePaths.join(", ")})`,
    };
  } catch (error) {
...
function buildSpecTestPlan(story: {
  title: string;
  description: string;
  acceptanceCriteria: Array<{ description: string; testName?: string }>;
}): string {
  const lines: string[] = [
