...
export class PlaywrightRecorder extends EventEmitter {
  ...
  private async setupEventListeners(): Promise<void> {
    ...
    await this.page.exposeFunction('__recordAction', (action: string, selectors: ActionSelector[], value?: string, boundingBox?: { x: number; y: number; width: number; height: number }, actionId?: string) => {
      ...
      this.addEvent('action', { action, selector: primarySelector, selectors, value, coordinates, actionId }, 'committed', {
        syntaxValid,
    ...
    await this.page.addInitScript(({ pointerGestures: pg, cursorFPS: fps, selectorPriority: priority }) => {
      ...
      document.addEventListener('click', (e) => {
        ...
        // @ts-ignore
        window.__recordAction?.('click', selectors, undefined, boundingBox, generateActionId());
      }, true);
