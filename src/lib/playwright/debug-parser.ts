...
/**
 * Reconstruct full test `code` around a NEW body, mirroring extractTestBody's
 * three branches in reverse so any format extractTestBody accepts round-trips
 * correctly. `newBody` replaces whatever extractTestBody would have returned
 * for `code` — it does not need to preserve the original body's line count.
 */
function rewrapTestBody(code: string, newBody: string): string {
  // Branch 1: export async function test(page, ...) { BODY }
  const funcMatch = code.match(
    /export\s+async\s+function\s+test\s*\(\s*page[^)]*\)\s*\{[\s\S]*\}\s*$/,
  );
  if (funcMatch && funcMatch.index !== undefined) {
    const openBraceIdx = code.indexOf("{", funcMatch.index);
    const prefix = code.slice(0, openBraceIdx + 1);
    return `${prefix}${newBody}\n}\n`;
  }

  // Branch 2: legacy test('...', async ({ page }) => { BODY });
  // The destructured parameter `({ page })` has its OWN `{`/`}` before the
  // function body's — searching for the first `{` after the match would land
  // on that one, not the body's. Anchor on `=>` instead, which always comes
  // right before the body's opening brace.
  const legacyMatch = code.match(
    /test\([^,]+,\s*async\s*\(\{\s*page\s*\}\)\s*=>\s*\{[\s\S]*\}\);?\s*$/,
  );
  if (legacyMatch && legacyMatch.index !== undefined) {
    const arrowIdx = code.indexOf("=>", legacyMatch.index);
    const openBraceIdx = code.indexOf("{", arrowIdx);
    const prefix = code.slice(0, openBraceIdx + 1);
    return `${prefix}${newBody}\n});\n`;
  }

  // Branch 3: raw code (no wrapper) — extractTestBody returns the trimmed
  // code itself (minus leading imports/blank lines) as the body, so the new
  // body IS the new code. Any leading import lines that extractTestBody
  // skipped over are not part of `body`/`newBody` in this branch, so they'd
  // be lost on round-trip — re-attach them from the original `code`.
  const lines = code.split("\n");
...
/**
 * Splice freshly-recorded code into an existing test's body at a step
 * boundary, then re-parse so the caller gets back a consistent
 * { code, cleanBody, steps } triple.
 *
 * Operates on LINE arrays, not character offsets — steps are already
 * line-bounded (lineStart/lineEnd), so there's no need for
 * spliceWithLinePreservation (which solves a different problem: preserving
 * line COUNT for stripped helper declarations; this splice intentionally
 * changes line count).
 *
 * @param code Current full test code (with function wrapper, if any).
 * @param anchorStepIndex Index into the CURRENT steps array of the last step
 *   to keep unconditionally. Pass -1 to splice at the very top (no steps
 *   kept).
 * @param newBodyLines Lines generated by eventsToCodeLines for the newly
 *   recorded actions.
 * @param spliceMode "replace" drops everything from the anchor onward;
 *   "insert" keeps it, appended after the new lines.
 */
export function spliceRecordedSteps(
  code: string,
  anchorStepIndex: number,
  newBodyLines: string[],
  spliceMode: "replace" | "insert",
): { code: string; cleanBody: string; steps: DebugStep[] } | null {
  ...
  // lineEnd is 1-based and inclusive; slicing to that index in a 0-based
  // array keeps lines [0..lineEnd) — i.e. through the anchor step's last line.
  const keepBeforeEndLine =
    anchorStepIndex >= 0 && anchorStepIndex < steps.length
      ? steps[anchorStepIndex].lineEnd
      : 0;

  const tailLines =
    spliceMode === "replace" ? [] : lines.slice(keepBeforeEndLine);

  const newCleanBody = [
    ...lines.slice(0, keepBeforeEndLine),
    ...newBodyLines,
    ...tailLines,
  ].join("\n");

  const newSteps = parseSteps(newCleanBody);
  const newCode = rewrapTestBody(code, newCleanBody);

  return { code: newCode, cleanBody: newCleanBody, steps: newSteps };
}
