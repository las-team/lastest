...
export function parseExtractableSelector(stepCode: string): string | null {
  ...
  const lwfMatch = stepCode.match(
    /locateWithFallback\s*\(\s*page\s*,\s*(\[[\s\S]*?\])\s*,/,
  );
  if (lwfMatch) {
    try {
      const arr = JSON.parse(lwfMatch[1]) as Array<{
        type?: string;
        value?: string;
      }>;
  ...
  const roleMatch = stepCode.match(
    /\.getByRole\s*\(\s*(['"`])([^'"`]+)\1(?:\s*,\s*\{([^}]*)\})?/,
  );
  if (roleMatch) {
...
export function collectExtractableSelectors(
