...
describe("resolveBuildSetup", () => {
  ...
  it("build.buildSetupTestId takes precedence over per-test fallback", async () => {
    ...
    expect(out.setupInfo).toEqual({
      code: "BUILD_SETUP_CODE",
      setupId: "setup-test-id",
    });
  ...
  it("loads storage_state from per-test setupOverrides.extraSteps (las-team/lastest demo path)", async () => {
    ...
    expect(out.setupContext.storageState).toBe(
      '{"cookies":[{"name":"session_id"}],"origins":[]}',
    );
      ...
      tests: [
        makeTest({
          setupOverrides: {
            skippedDefaultStepIds: [],
            extraSteps: [
              { stepType: "storage_state", storageStateId: "ss-override" },
            ],
          },
        }),
      ],
      ...
      tests: [
        makeTest({
          setupOverrides: {
            skippedDefaultStepIds: [],
            extraSteps: [
              { stepType: "script", scriptId: "sc-1", storageStateId: null },
            ],
          },
        }),
      ],
