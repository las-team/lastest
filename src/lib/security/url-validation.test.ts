...
describe("validateUrl — SSRF Protection", () => {
  ...
  describe("protocol blocking", () => {
    ...
    it("blocks data: protocol", () => {
      expect(validateUrl("data:text/html,<h1>hi</h1>")).toBe(
        "Only HTTP(S) protocols are allowed",
      );
    });

    it("blocks gopher:// protocol", () => {
      expect(validateUrl("gopher://evil.com")).toBe(
        "Only HTTP(S) protocols are allowed",
      );
    });
  });
  ...
  describe("loopback", () => {
    ...
    it("blocks 127.0.0.2", () => {
      expect(validateUrl("http://127.0.0.2")).toBe(
        "URL targets a private network",
      );
    });
    ...
    it("blocks localhost", () => {
      expect(validateUrl("http://localhost")).toBe(
        "URL targets a blocked hostname",
      );
    });
  ...
  describe("link-local", () => {
    it("blocks 169.254.0.1", () => {
      expect(validateUrl("http://169.254.0.1")).toBe(
        "URL targets a private network",
      );
    });
  ...
  describe("cloud metadata endpoints", () => {
    it("blocks metadata.google.internal", () => {
      expect(validateUrl("http://metadata.google.internal")).toBe(
        "URL targets a blocked hostname",
      );
    });
    ...
    it("blocks metadata.google.internal with path", () => {
      ...
      ).toBe("URL targets a blocked hostname");
    });
  });
  ...
  describe("IPv6 private ranges", () => {
    ...
    it("blocks fd00:: (unique local)", () => {
      expect(validateUrl("http://[fd12::1]")).toBe(
        "URL targets a private network",
      );
    });
