 ...
 *
 * Pacing is swarm-wide, not per-explorer (`@/lib/qa-agent/politeness`): one
 * shared pacer serializes navigations across all N explorers, so the target
 * sees one request per slot no matter how many are running.
 */
