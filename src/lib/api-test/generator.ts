...
export async function generateApiTest(input: GenerateApiTestInput): Promise<GenerateApiTestResult> {
  const { repositoryId } = input;
  const settings = await queries.getAISettings(repositoryId);
  if (settings.provider === 'claude-cli') {
    return { status: 'ai_unavailable', summary: 'API test generation requires a JSON-capable AI provider (not claude-cli).' };
  }
  ...
  try {
    ...
    if (repo?.provider === 'github' && repo.teamId) {
      ...
      if (account?.accessToken) {
        ...
        const intel = await gatherCodebaseIntelligence(account.accessToken, repo.owner, repo.name, branch);
  ...
  const contextParts = [
    ...
    input.openapiSpec ? `OpenAPI spec:\n${input.openapiSpec.slice(0, 6000)}` : '',
    input.graphqlSchema ? `GraphQL schema:\n${input.graphqlSchema.slice(0, 6000)}` : '',
  ].filter(Boolean);
...
${contextParts.join('\n\n')}`;
  ...
  try {
    ...
    return { status: 'ai_unavailable', summary: `AI call failed: ${e instanceof Error ? e.message : String(e)}` };
  }

  const isShape = (v: unknown): v is { method?: unknown; url?: unknown; assertions?: unknown } =>
    typeof v === 'object' && v !== null;
  const parsed = parseAiJson(response, isShape, { source: 'generate_api_test' });
  if (!parsed || typeof parsed.method !== 'string' || typeof parsed.url !== 'string') {
    return { status: 'no_definition', summary: 'The model did not return a valid API test definition.' };
  }
  const method = parsed.method.toUpperCase();
  if (!VALID_METHODS.includes(method)) {
    return { status: 'no_definition', summary: `Invalid HTTP method: ${parsed.method}` };
  }

  const rawAssertions = Array.isArray(parsed.assertions) ? parsed.assertions : [];
  const assertions = rawAssertions.filter(
    (a): a is ApiAssertion => typeof a === 'object' && a !== null && typeof (a as { kind?: unknown }).kind === 'string',
  );
  ...
  const definition: ApiTestDefinition = {
    ...
    query: typeof p.query === 'object' && p.query !== null ? (p.query as Record<string, string>) : undefined,
    body: p.body,
    assertions,
  };

  return { status: 'generated', summary: `Generated ${definition.method} ${definition.url} with ${assertions.length} assertion(s).`, definition };
}
