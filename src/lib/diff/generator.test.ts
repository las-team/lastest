...
describe("detectPageShift", () => {
  ...
  describe("no shift", () => {
    it("returns undefined for 2 regions at same Y", () => {
      const regions = [makeRegion(0, 100), makeRegion(100, 100)];
      const result = detectPageShift(regions);
...
describe("generateDiff", () => {
  ...
  describe("basic functionality", () => {
    ...
    it("detects color changes in images", async () => {
      ...
      const png1 = createMockPNGWithRect(
        800,
        600,
      ...
      const png2 = createMockPNGWithRect(
        800,
        600,
        { x: 300, y: 225, w: 200, h: 150 },
        [255, 0, 0, 255],
    ...
    it("respects includeAntiAliasing parameter", async () => {
      ...
      const result1 = await generateDiff(
        baseline,
        current,
        tempDir,
        0.1,
...
describe("imagesMatch", () => {
  ...
  it("returns false for non-existent files", () => {
    expect(
      imagesMatch("/nonexistent/path1.png", "/nonexistent/path2.png"),
    ).toBe(false);
  });
