...
describe('check-modes — derivation', () => {
  ...
  it('collapses network capture + error mode into the network 3-way', () => {
    ...
    expect(
      deriveCheckModes({ enableNetworkInterception: true, networkErrorMode: 'fail' }).network,
    ).toBe('enforce');
    // capture on, warn → log
    expect(
      deriveCheckModes({ enableNetworkInterception: true, networkErrorMode: 'warn' }).network,
    ).toBe('log');
    // capture off, fail → enforce
    expect(
      deriveCheckModes({ enableNetworkInterception: false, networkErrorMode: 'fail' }).network,
    ).toBe('enforce');
    // capture off, ignore → disable
    expect(
      deriveCheckModes({ enableNetworkInterception: false, networkErrorMode: 'ignore' }).network,
    ).toBe('disable');
  });

  it('maps console error mode to the console 3-way', () => {
    expect(deriveCheckModes({ consoleErrorMode: 'fail' }).console).toBe('enforce');
    expect(deriveCheckModes({ consoleErrorMode: 'warn' }).console).toBe('log');
    expect(deriveCheckModes({ consoleErrorMode: 'ignore' }).console).toBe('disable');
  });
...
describe('check-modes — patch shape', () => {
  ...
  it('text mode mirrors to textDiffEnabled on diff_sensitivity_settings', () => {
    expect(checkModesToSettingsPatch({ text: 'enforce' }).textDiffEnabled).toBe(true);
    expect(checkModesToSettingsPatch({ text: 'log' }).textDiffEnabled).toBe(true);
    expect(checkModesToSettingsPatch({ text: 'disable' }).textDiffEnabled).toBe(false);
  });
...
describe('check-modes — per-layer round-trip', () => {
  ...
  const layers = ['visual', 'text', 'dom', 'network', 'console', 'a11y', 'design', 'perf', 'url'] as const;
...
describe('check-modes — executor gating', () => {
  ...
  const checkRunnerGate = (layer: 'a11y' | 'design' | 'dom' | 'network' | 'console' | 'text', mode: 'enforce' | 'log' | 'disable') => {
  ...
  it('enforce + log enable the runner capture for opt-in layers', () => {
    for (const layer of ['a11y', 'design', 'dom', 'network', 'console', 'text'] as const) {
  ...
  it('log on console/network downgrades the runner verdict to warn (not fail)', () => {
    ...
    expect(checkModesToSettingsPatch({ network: 'log' }).networkErrorMode).toBe('warn');
...
describe('check-modes — per-test overrides', () => {
  ...
  it('extracts only the layers the test touches', () => {
    const out = pickTestModeOverrides({ a11yMode: 'log', designMode: 'enforce' });
    expect(out).toEqual({ a11y: 'log', design: 'enforce' });
  });

  it('falls back to legacy networkErrorMode when networkMode is unset', () => {
    expect(pickTestModeOverrides({ networkErrorMode: 'warn' })).toEqual({ network: 'log' });
    expect(pickTestModeOverrides({ consoleErrorMode: 'ignore' })).toEqual({ console: 'disable' });
  });

  it('new networkMode wins over legacy networkErrorMode', () => {
    const out = pickTestModeOverrides({ networkMode: 'log', networkErrorMode: 'fail' });
    expect(out).toEqual({ network: 'log' });
  });

  it('ignores unknown mode strings', () => {
    // @ts-expect-error — feeding an invalid mode at runtime
