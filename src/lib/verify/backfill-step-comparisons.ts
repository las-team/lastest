...
export async function ensureStepComparisonsForBuild(
  buildId: string,
  ...
  for (const s of existing)
    haveByResultStep.add(stepKey(s.testResultId, s.stepLabel));
  ...
  for (const tr of testResults) {
    ...
    try {
      ...
      console.error("[verify-backfill] failed to score", {
        testResultId: tr.id,
