  ...
  const headline =
    status === "unchanged"
      ...
      : status === "baseline_only"
        ? "Baseline text only — current run captured nothing"
        : status === "current_only"
          ? "New text — no baseline to compare against"
          : status === "baseline_establishing"
            ? "Captured for next run's baseline"
            : status === "skipped"
              ? "Text capture not enabled for this diff"
            ...
            <pre className="text-xs font-mono bg-card border rounded max-h-96 overflow-auto p-2 leading-snug">
              {visible.map((l, i) => (
                <div
                  ...
                  className={
                    l.op === "add"
                      ? "bg-green-100/60 dark:bg-green-900/30 text-green-900 dark:text-green-200"
                      : l.op === "del"
                        ? "bg-red-100/60 dark:bg-red-900/30 text-red-900 dark:text-red-200"
