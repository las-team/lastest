...
function getDotColor(event: ActivityEvent): string {
  if (
    event.eventType === "beat_the_bot" ||
    event.eventType === "achievement:unlocked"
  )
...
function EventIcon({ event }: { event: ActivityEvent }) {
  if (event.eventType === "beat_the_bot")
    return <Trophy className="h-3.5 w-3.5 text-yellow-500 shrink-0" />;
  if (event.eventType === "achievement:unlocked")
    return <Sparkles className="h-3.5 w-3.5 text-yellow-500 shrink-0" />;
  if (
    event.eventType === "score:awarded" ||
    event.eventType === "score:penalty"
  )
  ...
  if (event.eventType === "artifact:created")
    return <Zap className="h-3.5 w-3.5 text-muted-foreground shrink-0" />;
  if (event.eventType === "session:complete")
    return (
      <CheckCircle2 className="h-3.5 w-3.5 text-muted-foreground shrink-0" />
    );
  if (event.eventType === "step:waiting_user")
    return <Clock className="h-3.5 w-3.5 text-muted-foreground shrink-0" />;
  if (event.sourceType === "mcp_server")
  ...
  return (
    <div className="flex gap-3 py-2 px-3 group">
      ...
      <div className="flex-1 min-w-0 pb-2">
        ...
        {event.eventType === "beat_the_bot" && (
          <ShareBeatBotButton event={event} />
        )}
