      ...
      <div className="space-y-1">
        {(sessions ?? []).map((s) => {
          ...
          const label = isSpecImport
            ? s.status === "paused"
              ? `Spec Import: ${(meta?.stories as unknown[])?.length ?? 0} stories ready`
              : "Importing spec..."
            : isHealAgent
...
export function ActivityFeedPanel() {
  const { isOpen, setIsOpen, events, isConnected, clearEvents } =
  ...
  const agentCount = events.filter((e) =>
    AGENT_SOURCES.has(e.sourceType),
  ).length;
    ...
    <Sheet open={isOpen} onOpenChange={setIsOpen}>
      ...
      >
        ...
        <ActiveSessionsSection
          ...
          setExpandedStream={setExpandedStream}
        />
            ...
            {filteredEvents.length === 0 ? (
              <div className="flex flex-col items-center justify-center py-16 text-muted-foreground">
                ...
                <p className="text-xs mt-1">
                  Events from Play Agent and MCP will appear here
                </p>
              </div>
            ) : (
