...
const ContextCollectorContext = createContext<ContextCollectorValue | null>(
  null,
);
...
export function ContextCollectorProvider({
  children,
}: {
  children: ReactNode;
}) {
    ...
    console.error = (...args: unknown[]) => {
      ...
      if (consoleErrors.current.length > MAX_CONSOLE_ERRORS) {
        consoleErrors.current =
          consoleErrors.current.slice(-MAX_CONSOLE_ERRORS);
      }
      ...
      if (!response.ok) {
        ...
        if (failedRequests.current.length > MAX_FAILED_REQUESTS) {
          failedRequests.current =
            failedRequests.current.slice(-MAX_FAILED_REQUESTS);
        }
      }
      return response;
    };
...
export function useContextCollector() {
  ...
  if (!ctx)
    throw new Error(
      "useContextCollector must be used within ContextCollectorProvider",
    );
  return ctx;
}
