...
export const JobPollingContext = createContext<JobPollingContextValue | null>(
  null,
);
...
export function JobPollingProvider({ children }: { children: ReactNode }) {
  ...
  const fetchSessionToken = useCallback(async (): Promise<string | null> => {
    ...
    try {
      ...
      // Silently fail — SSE will retry on next reconnect
    }
    return null;
  }, []);
  ...
  const connect = useCallback(async () => {
        ...
        if (data.type === "snapshot") {
          ...
          setJobs((prev) => {
            ...
            const updated: JobWithChildren = {
              ...
              ...(idx >= 0
                ? {
                    _children: prev[idx]._children,
                    _childSummary: prev[idx]._childSummary,
                  }
                : {}),
            };
    ...
    es.onerror = async () => {
      // Clear cached token on error so we retry fetching a fresh one
...
export function useJobPollingContext() {
  ...
  if (!ctx) {
    throw new Error(
      "useJobPollingContext must be used within JobPollingProvider",
    );
  }
  return ctx;
}
