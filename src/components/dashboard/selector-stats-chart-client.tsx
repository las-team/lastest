  ...
  return (
    <div className="rounded-lg border bg-card p-3 shadow-md text-sm min-w-[180px]">
      ...
      <div className="flex items-center gap-2 text-xs">
        ...
        />
        <span>
          Successes: <strong>{stat.totalSuccesses}</strong>
        </span>
      </div>
      <div className="flex items-center gap-2 text-xs">
        <span
        ...
        />
        <span>
          Failures: <strong>{stat.totalFailures}</strong>
        </span>
      </div>
      <div className="text-xs text-muted-foreground mt-1">
        Success Rate: {stat.successRate}%
        {stat.avgResponseTimeMs != null &&
          ` · Avg: ${stat.avgResponseTimeMs}ms`}
      </div>
    </div>
  );
}

function SelectorChart({
  stats,
  height,
}: {
  stats: SelectorTypeStats[];
