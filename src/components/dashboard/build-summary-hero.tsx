...
export function BuildSummaryHero({
  status,
  changesDetected,
  isRunning = false,
  errorMessage,
}: BuildSummaryHeroProps) {
  ...
  return (
    <div className="inline-flex flex-col gap-1">
      <div
        className={`inline-flex items-center gap-2 px-3 py-1.5 rounded-md border text-sm font-semibold ${config.bgColor} ${config.borderColor} ${config.textColor}`}
      >
        <Icon
          className={`w-4 h-4 ${config.iconColor} ${isRunning ? "animate-spin" : ""}`}
        />
