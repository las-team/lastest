...
export function IssuePickerDialog({
  ...
  const handleLink = (issue: GitHubIssueListItem) => {
    startTransition(async () => {
      const res = await linkIssueToCase({
        stepComparisonId,
        issueUrl: issue.url,
      });
      if (res.ok) {
    ...
    <Dialog open={open} onOpenChange={(o) => !o && onClose()}>
      <DialogContent className="verify-page max-w-xl">
        <DialogHeader>
          ...
          <DialogDescription>
            Browse open issues from this repo, or file a new one. The
            case&apos;s test/step context is pre-filled.
          </DialogDescription>
        </DialogHeader>
        ...
        >
          <button
            className={`v-tab ${tab === "browse" ? "active" : ""}`}
            onClick={() => setTab("browse")}
          >
            Browse
          </button>
          <button
            className={`v-tab ${tab === "create" ? "active" : ""}`}
        ...
        {issues.map((issue) => (
          ...
          >
            <CircleDot
              ...
              style={{
                marginTop: 2,
                color: issue.state === "open" ? "var(--c-teal)" : "var(--fg-3)",
                flexShrink: 0,
              }}
            />
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
      ...
      <div className="label" style={{ fontSize: 10 }}>
        Test name, step label, build, branch + evidence are auto-appended below
        your text.
      </div>
      {error && (
        <div className="v-chip regression" style={{ alignSelf: "flex-start" }}>
