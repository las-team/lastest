...
import {
  saveTestDiffOverrides,
  resetTestDiffOverrides,
...
type BooleanKeys = {
  [K in keyof Required<TestDiffOverridesType>]: NonNullable<
    TestDiffOverridesType[K]
  > extends boolean
    ? K
    : never;
}[keyof TestDiffOverridesType];
type NumberKeys = {
  [K in keyof Required<TestDiffOverridesType>]: NonNullable<
    TestDiffOverridesType[K]
  > extends number
...
export function TestDiffOverrides({
  testId,
  repositoryId,
  overrides: initialOverrides,
  defaults,
}: TestDiffOverridesProps) {
  const [overrides, setOverrides] = useState<Partial<TestDiffOverridesType>>(
    initialOverrides ?? {},
  );
  ...
  const getVal = <K extends keyof TestDiffOverridesType>(
    key: K,
  ): NonNullable<TestDiffOverridesType[K]> => {
    return (
      key in overrides
        ? overrides[key]
        : (defaults as Record<string, unknown>)[key]
    ) as NonNullable<TestDiffOverridesType[K]>;
  };

  const isOverridden = (key: keyof TestDiffOverridesType) => key in overrides;

  const setVal = <K extends keyof TestDiffOverridesType>(
    key: K,
    value: TestDiffOverridesType[K],
  ) => {
  ...
  const doSave = useCallback(
    async (current: Partial<TestDiffOverridesType>) => {
      ...
      try {
        const toSave =
          Object.keys(current).length === 0
            ? null
            : (current as TestDiffOverridesType);
  ...
  const OverrideIndicator = ({
    ...
    keys: (keyof TestDiffOverridesType)[];
  }) => {
    ...
    <div className="flex items-center justify-between">
      ...
      <Switch
        ...
        onCheckedChange={(checked) =>
          setVal(key, checked as TestDiffOverridesType[typeof key])
        }
      />
    </div>
  );
        ...
        <Input
          ...
          onChange={(e) =>
            setVal(
              ...
              ) as TestDiffOverridesType[typeof key],
            )
          }
          className="w-20"
          disabled={disabled}
  ...
  const renderSelect = (
    ...
    <div className="flex items-center justify-between">
      ...
      <Select
        ...
        onValueChange={(value) =>
          setVal(key, value as TestDiffOverridesType[typeof key])
        }
      >
    ...
    <Card>
      <CardHeader>
        ...
        <p className="text-xs text-muted-foreground">
          Per-test diff settings. Changed values override repo defaults and show
          a warning indicator.
        </p>
      </CardHeader>
      <CardContent className="space-y-4">
        ...
        <Collapsible
          defaultOpen={Object.keys(overrides).some((k) =>
            ["unchangedThreshold", "flakyThreshold"].includes(k),
          )}
        >
          ...
          <CollapsibleContent className="space-y-3 pt-2 pl-4">
            {renderNum(
              "unchangedThreshold",
              "Unchanged Threshold",
              { min: 0, max: 100, step: 0.1, fallback: 1 },
              "%",
            )}
            {renderNum(
              "flakyThreshold",
              "Flaky Threshold",
              { min: 0, max: 100, step: 0.1, fallback: 10 },
              "%",
            )}
          </CollapsibleContent>
        </Collapsible>
        ...
        <Collapsible
          defaultOpen={Object.keys(overrides).some((k) =>
            ["diffEngine", "includeAntiAliasing", "ignorePageShift"].includes(
              k,
            ),
          )}
        >
          ...
          <CollapsibleContent className="space-y-3 pt-2 pl-4">
            {renderSelect(
              "diffEngine",
              "Engine",
              "Algorithm used for pixel comparison",
              [
            ...
            {renderBool(
              "ignorePageShift",
              "Ignore Page Shift",
              "Compensate for minor page shifts before diffing",
            )}
          </CollapsibleContent>
        </Collapsible>
        ...
        >
          ...
          <CollapsibleContent className="space-y-3 pt-2 pl-4">
            {renderBool(
              ...
              "Apply separate thresholds to detected text regions",
            )}
            {(getVal("textRegionAwareDiffing") as boolean) && (
              <>
                {renderNum(
                  "textRegionThreshold",
                  "Text Region Threshold",
                  { min: 0, max: 100, step: 1, fallback: 30 },
                  "%",
                )}
                {renderNum(
                  "textRegionPadding",
                  "Text Region Padding",
                  { min: 0, max: 50, step: 1, fallback: 4 },
                  "px",
                )}
                {renderSelect(
                  "textDetectionGranularity",
                  "Detection Granularity",
                  "Level of text region detection",
                  [
                ...
                {renderSelect(
                  "regionDetectionMode",
                  "Detection Mode",
                  "Algorithm for finding text regions",
                  [
