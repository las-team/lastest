...
export function CsvSourcesSettingsCard({
  dataSources,
  repositoryId,
  onRefresh,
}: CsvSourcesSettingsCardProps) {
  ...
  const handleDelete = async (id: string, alias: string) => {
    if (
      !confirm(
        `Delete CSV source "${alias}"? Test references that use it will start failing.`,
      )
    )
      return;
    setDeletingId(id);
      ...
      <CardHeader>
        <div className="flex items-start justify-between gap-2">
          <div>
            ...
            <CardDescription>
              Upload CSV files for data-driven tests. Reference columns from
              test code via <code>{"{{csv:alias.column[row]}}"}</code> or bind
              them to variables on the Vars tab.
            </CardDescription>
          </div>
          {repositoryId && (
      ...
      <CardContent className="space-y-3">
        {!repositoryId ? (
          <p className="text-sm text-muted-foreground">
            Select a repository to manage CSV sources.
          </p>
        ) : (
          <>
            <div
