...
import {
  TESTING_TEMPLATES,
  TESTING_TEMPLATE_IDS,
  type TestingTemplateId,
...
export function TestingTemplateSelector({
  repositoryId,
  currentTemplate,
}: TestingTemplateSelectorProps) {
  ...
  function handleConfirm() {
    ...
    startTransition(async () => {
      ...
      if (result.success) {
        const name =
          TESTING_TEMPLATES[templateId as Exclude<TestingTemplateId, "custom">]
            ?.name ?? templateId;
  ...
  const pendingName =
    pendingTemplate && pendingTemplate !== "custom"
      ? TESTING_TEMPLATES[
          pendingTemplate as Exclude<TestingTemplateId, "custom">
        ]?.name
      : null;
      ...
      >
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              Apply &ldquo;{pendingName}&rdquo;? This will overwrite your
