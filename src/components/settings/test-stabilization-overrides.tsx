...
import {
  saveTestStabilizationOverrides,
  resetTestStabilizationOverrides,
} from "@/server/actions/stabilization-overrides";
...
type BooleanKeys = {
  [K in keyof StabilizationSettings]: StabilizationSettings[K] extends boolean
    ? K
    : never;
}[keyof StabilizationSettings];
type NumberKeys = {
  [K in keyof StabilizationSettings]: StabilizationSettings[K] extends number
    ? K
    : never;
}[keyof StabilizationSettings];

export function TestStabilizationOverrides({
  testId,
  overrides: initialOverrides,
  defaults,
}: TestStabilizationOverridesProps) {
  // Merged state: defaults + overrides. We track which keys are overridden separately.
  const [overrides, setOverrides] = useState<Partial<StabilizationSettings>>(
    initialOverrides ?? {},
  );
  ...
  const getVal = <K extends keyof StabilizationSettings>(
    key: K,
  ): StabilizationSettings[K] => {
    return key in overrides
      ? (overrides[key] as StabilizationSettings[K])
      : defaults[key];
  };
  ...
  const setVal = <K extends keyof StabilizationSettings>(
    key: K,
    value: StabilizationSettings[K],
  ) => {
  ...
  const doSave = useCallback(
    async (current: Partial<StabilizationSettings>) => {
  ...
  const OverrideIndicator = ({
    ...
    keys: (keyof StabilizationSettings)[];
  }) => {
    ...
    <div className="flex items-center justify-between">
      ...
      <Switch
        ...
        onCheckedChange={(checked) =>
          setVal(key, checked as StabilizationSettings[typeof key])
        }
      />
    </div>
  );
    ...
    <div className="flex items-center justify-between">
      ...
      <div className="flex items-center gap-2">
        <Input
          ...
          onChange={(e) =>
            setVal(
              ...
              Math.max(
                numConfig.min,
                parseInt(e.target.value) || numConfig.fallback,
              ) as StabilizationSettings[typeof numKey],
            )
          }
          className="w-20"
        ...
        <Switch
          ...
          onCheckedChange={(checked) =>
            setVal(boolKey, checked as StabilizationSettings[typeof boolKey])
          }
        />
      </div>
    </div>
              ...
              Math.max(
                ...
                Math.min(
                  ...
                  (config.step < 1
                    ...
                    : parseInt(e.target.value)) || config.fallback,
                ),
              ) as StabilizationSettings[typeof key],
            )
          }
          className="w-20"
          disabled={disabled}
    ...
    <Card>
      <CardHeader>
        ...
        <p className="text-xs text-muted-foreground">
          Per-test stabilization settings. Changed values override repo defaults
      ...
      <CardContent className="space-y-4">
        ...
        >
          ...
          <CollapsibleContent className="space-y-3 pt-2 pl-4">
            {renderBoolNum(
              ...
              "Wait until no network requests",
              { min: 0, max: 30000, step: 1000, fallback: 5000 },
            )}
            {renderBoolNum(
              "waitForDomStable",
              "domStableTimeout",
              ...
              { min: 0, max: 10000, step: 500, fallback: 2000 },
            )}
            {renderBool(
              "waitForFonts",
              "Wait for Fonts",
          ...
          <CollapsibleContent className="space-y-3 pt-2 pl-4">
                ...
                <Input
                  ...
                  onChange={(e) =>
                    setVal(
                      "frozenTimestamp",
                      e.target
                        .value as StabilizationSettings["frozenTimestamp"],
                    )
                  }
                  className="flex-1"
            ...
            {(getVal("freezeRandomValues") as boolean) && (
              <>
                    ...
                    onChange={(e) =>
                      setVal(
                        ...
                        (parseInt(e.target.value) ||
                          12345) as StabilizationSettings["randomSeed"],
                      )
                    }
                    className="w-24"
                  />
                ...
                {renderBool(
                  "reseedRandomOnInput",
                  "Reseed on Input Events",
                  "Reset RNG from event hash on user input",
                )}
              </>
            )}
            {renderBool(
        ...
        >
          ...
          <CollapsibleContent className="space-y-3 pt-2 pl-4">
            ...
            {renderBool(
              ...
              "Snap stroke coordinates to pixel centers",
            )}
          </CollapsibleContent>
        </Collapsible>
            ...
            {(getVal("burstCapture") as boolean) && (
              <>
                ...
                {renderNum(
                  "burstStabilityThreshold",
                  "Stability Threshold",
                  { min: 0, max: 10, step: 0.1, fallback: 0.5 },
                  "%",
                )}
              </>
            )}
        ...
        >
          ...
          <CollapsibleContent className="space-y-3 pt-2 pl-4">
            {renderBool(
              "waitForCanvasStable",
              "Wait for Canvas Stable",
              "Loop canvas comparisons until stable",
            )}
            {(getVal("waitForCanvasStable") as boolean) && (
              <>
                {renderNum(
                  "canvasStableTimeout",
                  "Timeout",
                  { min: 0, max: 30000, step: 1000, fallback: 3000 },
                  "ms",
                )}
                {renderNum("canvasStableThreshold", "Stable Checks", {
                  min: 1,
                  max: 10,
                  step: 1,
                  fallback: 3,
            ...
            {renderBool(
              ...
              "Set imageSmoothingEnabled = false on 2D contexts",
            )}
          </CollapsibleContent>
        </Collapsible>
      </CardContent>
