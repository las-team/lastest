...
export function NotificationSettingsCard({
  ...
  const [customWebhookEnabled, setCustomWebhookEnabled] = useState(
    settings.customWebhookEnabled || false,
  );
  const [customWebhookUrl, setCustomWebhookUrl] = useState(
    settings.customWebhookUrl || "",
  );
  const [customWebhookMethod, setCustomWebhookMethod] = useState<
    "POST" | "PUT"
  >((settings.customWebhookMethod as "POST" | "PUT") || "POST");
  const [issueTrackerProvider, setIssueTrackerProvider] =
  ...
  const headersToJson = useCallback(
    (headers: Array<{ key: string; value: string }>): string | null => {
      ...
      <CardContent className="space-y-6">
        ...
        <div className="space-y-4 p-4 border rounded-lg">
          ...
          {discordEnabled && (
            <div className="space-y-2">
              ...
              />
              <p className="text-xs text-muted-foreground">
                Create a webhook in your Discord server settings: Server
        ...
        <div className="space-y-4 p-4 border rounded-lg">
          ...
          {hasGithubAccount && githubPrCommentsEnabled && (
            <p className="text-xs text-muted-foreground">
              Build results will be posted as comments on open PRs matching the
              build branch
            </p>
          )}
        </div>
        ...
        <div className="space-y-4 p-4 border rounded-lg">
          ...
          {hasGitlabAccount && gitlabMrCommentsEnabled && (
            <p className="text-xs text-muted-foreground">
              Build results will be posted as notes on open MRs matching the
              build branch
            </p>
          )}
        </div>
        ...
        <div className="space-y-3 p-4 border rounded-lg">
          ...
          <p className="text-xs text-muted-foreground">
            Where the &quot;Submit as Issue&quot; button on a visual diff
            creates the issue.
          </p>
          <div className="space-y-2">
            ...
            <Select
              ...
              onValueChange={(v) =>
                setIssueTrackerProvider(v as IssueTrackerProvider)
              }
            >
            ...
            <div className="space-y-4">
              ...
              <div className="space-y-2">
                ...
                <Select
                  ...
                  onValueChange={(v) =>
                    setCustomWebhookMethod(v as "POST" | "PUT")
                  }
                >
              ...
              <div className="flex items-center gap-2">
                ...
                {webhookTestResult && (
                  <span
                    className={`flex items-center gap-1 text-sm ${webhookTestResult.success ? "text-green-600" : "text-red-600"}`}
                  >
