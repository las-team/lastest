...
export function RecordingSetupPicker({
  ...
  const handleAdd = (itemId: string) => {
    if (addStepType === "test") {
      ...
      onChange([
        ...extraSteps,
        { stepType: "test", testId: itemId, name: test.name },
      ]);
    } else {
      ...
      onChange([
        ...extraSteps,
        { stepType: "script", scriptId: itemId, name: script.name },
      ]);
    }
  };
  ...
  return (
    <div className="space-y-2 pt-2">
      ...
      {defaultSteps.map((step) => {
        ...
        const bgColor =
          step.stepType === "test" ? "bg-blue-500/10" : "bg-green-500/10";
        return (
          <div
            key={step.id}
            className={cn(
              "flex items-center gap-3 p-2.5 bg-background/50 border rounded-lg transition-opacity",
      ...
      {extraSteps.map((step, i) => {
        ...
        return (
          <div
            key={i}
            className="flex items-center gap-3 p-2.5 bg-background/80 border rounded-lg"
          >
            <div
              className={cn(
                "flex items-center justify-center w-6 h-6 rounded",
