...
interface TestTeardownOverridesProps {
  testId: string;
  teardownOverrides: TestTeardownOverridesType | null;
  defaultTeardownSteps: DefaultStep[];
  availableTests: Test[];
  availableScripts: SetupScript[];
}

export function TestTeardownOverrides({
  testId,
  teardownOverrides,
  defaultTeardownSteps,
  availableTests,
  availableScripts,
}: TestTeardownOverridesProps) {
  ...
  const handleToggleSkip = async (stepId: string, isCurrentlySkipped: boolean) => {
    ...
    try {
      if (isCurrentlySkipped) {
        await unskipDefaultTeardownStepForTest(testId, stepId);
      } else {
        await skipDefaultTeardownStepForTest(testId, stepId);
      }
      router.refresh();
    } catch {
  ...
  return (
    <div className="space-y-6">
      ...
      <Card>
        <CardHeader>
          <CardTitle className="text-sm">Default Teardown Steps</CardTitle>
        </CardHeader>
        <CardContent>
          {defaultTeardownSteps.length === 0 ? (
            <p className="text-sm text-muted-foreground py-4 text-center">
              No default teardown steps configured for this repository.
      ...
      <Card>
        <CardHeader>
          <CardTitle className="text-sm">Extra Teardown Steps</CardTitle>
        </CardHeader>
        <CardContent className="space-y-4">
          {extraSteps.length > 0 ? (
            ...
            <p className="text-sm text-muted-foreground text-center py-2">
              No extra teardown steps. Add steps that run after the defaults.
            </p>
          )}
