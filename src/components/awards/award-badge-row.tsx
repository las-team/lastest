...
function badgeUrl(base: string, slug: string, type: string, opts?: { theme?: 'light' | 'dark'; size?: 'sm' | 'md' | 'lg' }) {
...
export function AwardBadgeRow({
  ...
  const earnedBadges: Array<{ type: string; preview: React.ReactNode; alt: string }> = [
    {
      type: 'tier',
      alt: `Lastest, ${tierMap.value}`,
      preview: <SplitShield label="LASTEST" value={tierMap.value} tone={tierMap.tone} size="md" mark />,
    },
  ];
  if (cats.allPassing) {
    earnedBadges.push({
  ...
  return (
    <section className="not-prose mt-6 rounded-sm border border-border/60 bg-card p-5">
      <div className="flex items-baseline justify-between gap-3 mb-3">
        ...
        {award.highestTier !== 'none' && award.highestTier !== award.currentTier && (
          <div className="font-mono text-[10px] uppercase tracking-[0.10em] text-muted-foreground">
