...
function ValueChip({ category, value }: { category: DesignTokenCategory; value: string }) {
  if (category === 'color') {
    return (
...
export function DesignSystemViolationsCard({ buildId, rows, embedded }: DesignSystemViolationsCardProps) {
    ...
    <div className="space-y-2">
      <div className="flex items-center justify-between gap-3 flex-wrap">
        <div className="flex items-center gap-2 text-xs text-muted-foreground">
          <span className="font-medium text-foreground">{rows.length} off-token value{rows.length === 1 ? '' : 's'}</span>
          ...
          return (
            <Collapsible key={r.id} open={isOpen} onOpenChange={() => toggle(r.id)}>
              ...
              >
                ...
                <span className="font-mono text-xs font-medium truncate" title={r.property}>
                  {r.property}
                </span>
              ...
              <CollapsibleContent className="px-3 pb-3 pt-1 bg-muted/30 space-y-2">
                {r.expectedName && (
                  <p className="text-xs text-muted-foreground">
                    Closest allowed token: <span className="font-mono text-foreground">{r.expectedName}</span>
