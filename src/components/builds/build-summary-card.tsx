        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-2 font-medium">
            ...
            {isRemoteUrl(baseUrl) ? (
              ...
              <Badge
                variant={isActiveBranch ? "default" : "secondary"}
                className="text-xs font-normal gap-1 shrink-0"
              >
            ...
            {isBaseline && !isMainBaseline && !isBranchBaseline && (
              <Badge
                variant="outline"
                className="text-xs font-normal gap-1 shrink-0 border-success/30 text-success bg-success/10"
              >
            ...
            {isMainBaseline && (
              <Badge
                variant="outline"
                className="text-xs font-normal gap-1 shrink-0 border-foreground/20 text-foreground bg-foreground/5"
              >
                <Shield className="h-3 w-3" />
                Main Baseline
              </Badge>
            ...
            {isBranchBaseline && (
              <Badge
                variant="outline"
                className="text-xs font-normal gap-1 shrink-0 border-info/30 text-info bg-info/10"
              >
                <Shield className="h-3 w-3" />
                Branch Baseline
              </Badge>
          ...
          <div className="flex items-start gap-2 text-destructive">
            ...
            <div className="min-w-0 flex-1">
              <div className="font-medium">
                Executor crashed before any test ran
              </div>
              <pre className="mt-1 text-[11px] text-destructive/80 whitespace-pre-wrap break-all line-clamp-3">
                {build.executorError.split("\n").slice(0, 3).join("\n")}
              </pre>
            </div>
          </div>
        </div>
