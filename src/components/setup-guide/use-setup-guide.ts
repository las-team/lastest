...
export function useSetupGuide(initialStatus: SetupStatus) {
  ...
  useEffect(() => {
    ...
    const aiConfigured =
      localStorage.getItem("lastest-ai-configured") === "true";
    ...
    const resultsViewed =
      localStorage.getItem("lastest-results-viewed") === "true";
    if (resultsViewed) autoDetected.push(8);

    const merged = Array.from(
      new Set([...loaded.completedSteps, ...autoDetected]),
    );
