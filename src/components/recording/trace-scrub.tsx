...
export interface TraceScrubProps {
  ...
  replayStatusOf: (event: StepCardEvent) => {
    replayable: boolean;
...
export function TraceScrub({
  ...
  useEffect(() => {
    function onKey(e: KeyboardEvent) {
      ...
      if (e.key === "ArrowDown" || e.key === "j") {
        ...
        setFocusIdx(actionEvents.length - 1);
        e.preventDefault();
      }
    }
  ...
  const counts = useMemo(() => {
    ...
    for (const e of events) {
      ...
      const m = e.verification?.selectorMatches as
        | StepCardSelectorMatch[]
        | undefined;
      ...
      const chosen =
        m.find((x) => x.value === e.verification?.chosenSelector) ?? m[0];
  ...
  const focusedMatches = focused?.verification?.selectorMatches as
    | StepCardSelectorMatch[]
    | undefined;
  const chosenSelector =
    focused?.verification?.chosenSelector ?? focused?.data.selector;
            ...
            <div className="flex flex-col gap-2">
              <div className="rounded-md border border-border bg-background min-h-32 flex items-center justify-center overflow-hidden">
                {focusedThumb ? (
                  ...
                  <div className="text-xs text-muted-foreground p-6 text-center">
                    {focused?.type === "navigation"
                      ? "Navigation step (no element thumbnail)"
                      : "No thumbnail captured for this step"}
                  </div>
                )}
              </div>
              {focused && (
                <div className="text-xs space-y-1">
                  ...
                  {focusedMatches && focusedMatches.length > 0 && (
                    <div className="text-muted-foreground">
                      {focusedMatches.length} candidate
                      {focusedMatches.length > 1 ? "s" : ""} ·{" "}
                      {focusedMatches.filter((m) => m.count === 1).length}{" "}
                      unique
                    </div>
                  )}
                </div>
              ...
              <div className="flex items-center gap-1 mt-auto">
                ...
                <Button
                  ...
                  onClick={() =>
                    setFocusIdx((i) => Math.min(actionEvents.length - 1, i + 1))
                  }
                >
                  Next
