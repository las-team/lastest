  ...
  const testCountMap = useMemo(() => {
    ...
    for (const area of areas) {
      if (area.testPlan) {
        map.set(
          area.id,
          groupScenariosForGeneration(area.testPlan, area.name, area.routes)
            .length,
        );
      } else {
        map.set(area.id, 1);
        ...
        {areas.map((area) => (
          <div key={area.id} className="border rounded-md p-2">
            <div className="flex items-center gap-2">
              ...
              <span className="text-[10px] text-muted-foreground">
                {testCountMap.get(area.id) ?? 1} tests
              </span>
              {onRerunPlanner && (
                <button
                ...
                {area.routes.length > 0 && (
                  <div className="flex flex-wrap gap-1">
                    {area.routes.map((route, i) => (
                      <span
                        key={i}
                        className="inline-flex px-1.5 py-0.5 rounded bg-muted text-[10px] font-mono"
                      >
                        {route}
                      </span>
                    ))}
      ...
      <div className="flex items-center gap-2 pt-1">
        ...
        {onApprovePlan && (
          ...
          >
            ...
            Approve & Generate ({selectedIds.size} areas · {selectedTestCount}{" "}
            tests)
          </Button>
        )}
      </div>
      ...
      {tests.map((test) => (
        <div key={test.testId} className="border rounded-md p-2">
          <button
            onClick={() =>
              setExpandedId((prev) =>
                prev === test.testId ? null : test.testId,
              )
            }
...
function FixTestsDetail({
  ...
  return (
    <div className="space-y-1.5 max-h-64 overflow-y-auto">
      <div className="text-xs font-medium text-muted-foreground mb-1">
        {fixes.filter((f) => f.fixed).length} fixed,{" "}
        {fixes.filter((f) => !f.fixed).length} unfixed
      </div>
      {fixes.map((fix, i) => (
        <div key={i} className="border rounded-md p-2">
          ...
          >
            ...
            <span
              className={cn(
                "text-xs flex-1",
                fix.fixed ? "text-foreground" : "text-muted-foreground",
              )}
            >
              {fix.testName}
            </span>
...
function ScanAndTemplateDetail({
  routes,
  framework,
  template,
  intelligence,
  ...
  return (
    <div className="space-y-2 max-h-64 overflow-y-auto">
      ...
      <div className="text-xs font-medium text-muted-foreground">
        {routes.length} routes ({staticRoutes.length} static,{" "}
        {dynamicRoutes.length} dynamic)
      </div>
      <div className="flex flex-wrap gap-1">
        {routes.map((route, i) => (
          <span
  ...
  const fetchLog = useCallback(async () => {
    ...
    try {
      const res = await fetch(
        `/api/play-agent/${sessionId}/planner-log/${logId}`,
      );
  ...
  return (
    <div>
      <button
        onClick={fetchLog}
        disabled={loading}
        className="text-[10px] text-info hover:text-info/80 disabled:opacity-50 flex items-center gap-1"
      >
        {loading ? (
          <Loader2 className="h-2.5 w-2.5 animate-spin" />
        ) : (
      ...
      {expanded && log && (
        <div className="mt-1 space-y-1.5">
          <div>
            <div className="text-[9px] uppercase text-muted-foreground/50 font-medium">
              Prompt ({log.userPrompt.length} chars)
            </div>
          ...
          {log.response && (
            <div>
              <div className="text-[9px] uppercase text-muted-foreground/50 font-medium">
                Response ({log.response.length} chars)
              </div>
              <pre className="text-[10px] bg-muted/50 rounded p-1.5 overflow-x-auto whitespace-pre-wrap max-h-24 overflow-y-auto">
                {log.response}
              </pre>
          ...
          {log.errorMessage && (
            <div>
              <div className="text-[9px] uppercase text-destructive/70 font-medium">
                Error
              </div>
...
function PlannerObservabilityDetail({
  substeps,
  sessionId,
  onRerunPlanner,
}: {
        ...
        <div key={i} className="border rounded-md p-2 space-y-1">
          ...
          {sub.inputSummary && (
            <div className="text-[10px] text-muted-foreground/60 ml-5">
              Input: {sub.inputSummary}
            </div>
          )}

          {sub.outputSummary && sub.status === "done" && (
            <div className="text-[10px] text-muted-foreground/60 ml-5 truncate">
              Areas: {sub.outputSummary}
            </div>
          )}

          {sub.rawError && sub.status === "error" && (
            <div className="text-[10px] text-destructive/80 ml-5 break-words">
              {sub.rawError}
            </div>
          )}
...
export function PlayAgentStepDetail({
  step,
  sessionId,
  loading,
  onApprovePlan,
  onRerunPlanner,
}: PlayAgentStepDetailProps) {
  ...
  switch (rich.type) {
    case "scan_and_template":
      return (
        <ScanAndTemplateDetail
          routes={rich.routes}
          framework={rich.framework}
          template={rich.template}
          intelligence={rich.intelligence}
        />
      );
    case "plan":
      return (
    ...
    case "env_setup":
      return (
        <EnvSetupDetail
          loginScript={rich.loginScript}
          pageContext={rich.pageContext}
        />
      );
    case "fix_tests":
