...
export function ReviewContent({
  ...
  // Resync when the server re-renders with new props (e.g. branch switch).
  useEffect(() => {
  ...
  const handleClearResolved = async () => {
    const resolvedIds = todos
      .filter((t) => t.todo.status === "resolved")
      .map((t) => t.todo.id);
  ...
  for (const diff of initialDiffs) {
    ...
    const isFailed =
      diff.testResultStatus === "failed" || diff.status === "rejected";
    if (isFailed) byArea[area].failed++;
    else if (diff.status === "todo") byArea[area].todo++;
    else if (diff.status === "pending") byArea[area].pending++;
    else if (diff.status === "approved" || diff.status === "auto_approved")
      byArea[area].approved++;
  ...
  const totalFailed = initialDiffs.filter(
    (d) => d.testResultStatus === "failed" || d.status === "rejected",
  ).length;
  const totalTodo = initialDiffs.filter((d) => d.status === "todo").length;
  const totalPending = initialDiffs.filter(
    (d) => d.status === "pending",
  ).length;
  const totalApproved = initialDiffs.filter(
      ...
      <Card>
        <CardHeader>
          <CardTitle className="flex items-center gap-2">
            Developer Todos
            {openTodos.length > 0 && (
              <Badge variant="secondary">{openTodos.length} open</Badge>
            )}
          </CardTitle>
        </CardHeader>
        <CardContent>
          {todos.length === 0 ? (
            <div className="p-8 text-center text-muted-foreground space-y-2">
              ...
              <p className="text-sm">
                No todos for this branch. Todos appear when reviewers flag
                diffs.
              </p>
            </div>
          ) : (
            <div className="space-y-2">
              ...
              {resolvedTodos.length > 0 && (
                <div className="pt-2">
                  ...
                  {resolvedTodos.map(
                    ({ todo, testName, functionalAreaName }) => (
                      <div
                        key={todo.id}
                      ...
                      >
                        ...
                        <div className="flex-1 min-w-0">
                          ...
                          <div className="flex items-center gap-2 text-xs text-muted-foreground mt-0.5">
                            ...
                            {todo.resolvedBy && (
                              <span>&middot; by {todo.resolvedBy}</span>
                            )}
                          </div>
                        </div>
                        <Button
          ...
          {totalDiffs === 0 ? (
            ...
            <>
              <div className="flex items-center gap-4 mb-4 text-sm">
                ...
                {totalTodo > 0 && (
                  <span className="text-amber-600">{totalTodo} todos</span>
                )}
                {totalFailed > 0 && (
              ...
              <div className="space-y-2">
                {sortedAreas.map(([areaName, data]) => (
                  <details
                    ...
                    open={data.failed > 0 || data.todo > 0}
                  >
                      ...
                      {data.diffs.map((diff) => {
                        ...
                        return (
                          <a
                            ...
                            href={
                              latestBuildId
                                ? `/builds/${latestBuildId}/diff/${diff.id}`
                                : "#"
                            }
