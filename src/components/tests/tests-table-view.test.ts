...
describe("compareTests", () => {
  it("sorts names case-insensitively asc", () => {
    ...
    const sorted = [a, b].sort((x, y) =>
      compareTests(x, y, "name", "asc", noArea),
    );
  ...
  it("sorts names desc", () => {
    ...
    const b = baseTest({ id: "b", name: "apple" });
    const sorted = [a, b].sort((x, y) =>
      compareTests(x, y, "name", "desc", noArea),
    );
  ...
  it("puts null lastRunAt last regardless of direction", () => {
    ...
    const asc = [never, ran].sort((x, y) =>
      compareTests(x, y, "lastRun", "asc", noArea),
    );
    const desc = [ran, never].sort((x, y) =>
      compareTests(x, y, "lastRun", "desc", noArea),
    );
  ...
  it("sorts lastRun desc by recency", () => {
    ...
    const recent = baseTest({
      id: "recent",
      lastRunAt: new Date("2026-04-01T00:00:00Z"),
    });
    const sorted = [old, recent].sort((x, y) =>
      compareTests(x, y, "lastRun", "desc", noArea),
    );
  ...
  it("uses getAreaName for the area key", () => {
    ...
    const lookup = (id: string | null) =>
      id === "a-id" ? "Zebra" : id === "b-id" ? "Alpha" : null;
    const sorted = [t1, t2].sort((x, y) =>
      compareTests(x, y, "area", "asc", lookup),
    );
  ...
  it("null status sorts last", () => {
    ...
    const sorted = [none, passed].sort((x, y) =>
      compareTests(x, y, "status", "asc", noArea),
    );
...
describe("parseStoredColumns", () => {
  ...
  it("returns null on version mismatch", () => {
    expect(
      parseStoredColumns(JSON.stringify({ v: 99, cols: ["status"] })),
    ).toBeNull();
  });
  it("returns Set on valid input", () => {
    const cols = parseStoredColumns(
      JSON.stringify({ v: 1, cols: ["status", "area"] }),
    );
  ...
  it("filters unknown keys", () => {
    const cols = parseStoredColumns(
      JSON.stringify({ v: 1, cols: ["status", "bogus"] }),
    );
    expect(cols!.has("status")).toBe(true);
...
describe("parseStoredSort", () => {
  it("returns null on bad input", () => {
    ...
    expect(
      parseStoredSort(JSON.stringify({ v: 1, key: "invalid", dir: "asc" })),
    ).toBeNull();
  });
