...
export function parseStoredColumns(
  raw: string | null,
): Set<TestsTableColumnKey> | null {
  if (!raw) return null;
  try {
    const parsed = JSON.parse(raw) as { v?: number; cols?: unknown };
    if (parsed.v !== STORED_VERSION || !Array.isArray(parsed.cols)) return null;
    const valid = parsed.cols.filter(
      (c): c is TestsTableColumnKey =>
        typeof c === "string" &&
        (TESTS_TABLE_TOGGLEABLE_COLUMNS as string[]).includes(c),
    );
...
export function parseStoredSort(raw: string | null): TestsTableSort | null {
  ...
  try {
    ...
    const validKeys: TestsTableSortKey[] = [
      "name",
      "status",
      "lastRun",
      "lastModified",
    ...
    if (
      ...
      !validKeys.includes(parsed.key as TestsTableSortKey)
    )
      return null;
...
type NullableCompareResult =
  | { kind: "null"; value: number }
...
const DEFAULT_DESC_KEYS: ReadonlySet<TestsTableSortKey> = new Set([
  "lastRun",
  "lastModified",
]);
              ...
              >
                ...
                {visibleColumns.has("area") && (
                  <td className="px-3 py-2 align-middle text-muted-foreground">
                    <span
                      className="truncate block max-w-[10rem]"
                      title={areaName ?? undefined}
                    >
                      {areaName ?? "—"}
                    </span>
                  </td>
