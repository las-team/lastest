...
} from "@/components/tests/screenshot-viewer";
...
export function ScreenshotTimeline({
  ...
  const [optimisticPlans, setOptimisticPlans] = useState<
    Map<string, PlannedScreenshot>
  >(new Map());
  ...
  const handleContainerDrop = (e: React.DragEvent) => {
    ...
    if (!handledByCard && e.dataTransfer.files.length > 0) {
      toast.message(
        "Drop the image directly on a screenshot to attach it as that step’s plan.",
      );
    }
  };
  ...
  const currentSrc = currentGroup
    ? currentGroup.screenshots[viewer!.screenshotIdx]
    : null;
  const currentLabel = currentSrc ? extractStepLabel(currentSrc) : null;
  const currentPlan = currentLabel
    ? (plannedByLabel.get(currentLabel) ?? null)
    : null;
  const currentDiffEntry =
    currentSrc && currentGroup?.diffsByPath
  ...
  const hasNext =
    !!currentGroup &&
    (viewer?.screenshotIdx ?? 0) < currentGroup.screenshots.length - 1;
        ...
        <div className="flex items-start justify-between gap-4">
          ...
          <div className="flex items-start gap-2 rounded-md border border-purple-200 bg-purple-50 px-3 py-2 text-purple-900 dark:border-purple-900/60 dark:bg-purple-950/40 dark:text-purple-200 max-w-md">
            ...
            <p className="text-xs leading-snug">
              Drag an image file onto a screenshot to attach it as the planned
              screenshot for that step. Click any screenshot to view full-size;
              click the corner thumbnail to compare against its plan.
            </p>
          </div>
        </div>
      </CardHeader>
      ...
      <ScreenshotViewer
        ...
        onNext={() =>
          setViewer((v) => {
            ...
            const nextHasDiff = !!(
              nextSrc && currentGroup?.diffsByPath?.[nextSrc]?.diffImagePath
            );
            return {
              ...v,
              screenshotIdx: nextIdx,
              mode: v.mode === "diff" && nextHasDiff ? "diff" : "captured",
            };
          })
        }
        onPrev={() =>
          setViewer((v) => {
            ...
            const prevHasDiff = !!(
              prevSrc && currentGroup?.diffsByPath?.[prevSrc]?.diffImagePath
            );
            return {
              ...v,
              screenshotIdx: prevIdx,
              mode: v.mode === "diff" && prevHasDiff ? "diff" : "captured",
            };
          })
        }
