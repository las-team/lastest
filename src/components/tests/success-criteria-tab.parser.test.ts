...
import {
  parseExtractableSelector as parse,
  collectExtractableSelectors,
} from "@/lib/playwright/extractable-selector";
...
describe("parseExtractableSelector", () => {
  ...
  it("handles getByRole with name", () => {
    expect(
      parse(`await page.getByRole('button', { name: 'Submit' }).click();`),
    ).toBe(`role=button[name="Submit"]`);
  });
  it("handles getByRole without name", () => {
    expect(parse(`await page.getByRole('navigation').click();`)).toBe(
      "role=navigation",
    );
  });
  it("handles getByTestId", () => {
    expect(parse(`await page.getByTestId('email-input').fill('x');`)).toBe(
      '[data-testid="email-input"]',
    );
  });
  it("handles getByPlaceholder", () => {
    expect(parse(`await page.getByPlaceholder('Email').fill('x');`)).toBe(
      '[placeholder="Email"]',
    );
  });
  it("handles getByLabel", () => {
    expect(parse(`await page.getByLabel('Password').fill('x');`)).toBe(
      '[aria-label="Password"]',
    );
  });
  it("handles getByText", () => {
    expect(parse(`await page.getByText('Sign in').click();`)).toBe(
      "text=Sign in",
    );
  });
