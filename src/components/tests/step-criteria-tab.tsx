...
export function StepCriteriaTab({
  testId,
  screenshots,
  stepCriteria,
  ...
  const assertions =
    freshAssertions.length > 0 ? freshAssertions : (dbAssertions ?? []);
  ...
  const labels = collectStepLabels(screenshots, criteria).filter(
    (l) => l !== ASSERTION_STEP_LABEL,
  );
  const sortedAssertions = [...(assertions ?? [])].sort(
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const { listFocusRegionsByTest } =
          await import("@/server/actions/diffs");
  ...
  const setRule = (stepLabel: string, kind: StepRule["kind"], on: boolean) => {
    const current =
      criteria.find((c) => c.stepLabel === stepLabel)?.rules ?? [];
    ...
    startTransition(async () => {
      try {
        ...
        toast.error(
          `Failed to save criteria: ${err instanceof Error ? err.message : String(err)}`,
        );
      }
    });
  };
  ...
  const setAssertionRule = (assertionId: string, on: boolean) => {
    const current =
      criteria.find((c) => c.stepLabel === ASSERTION_STEP_LABEL)?.rules ?? [];
    ...
    setCriteria((prev) => {
      ...
      return next.length === 0
        ? others
        : [...others, { stepLabel: ASSERTION_STEP_LABEL, rules: next }];
    });
  ...
  const setAllStepsExecutedRule = (on: boolean) => {
    const others = (
      criteria.find((c) => c.stepLabel === ALL_STEPS_EXECUTED_LABEL)?.rules ??
      []
    ...
    setCriteria((prev) => {
      ...
      return next.length === 0
        ? rest
        : [...rest, { stepLabel: ALL_STEPS_EXECUTED_LABEL, rules: next }];
    });
      ...
      <CardContent className="space-y-4">
        ...
        <div className="border rounded-md p-3 bg-muted/30 flex items-center gap-2">
          ...
          >
            ...
            <span className="block text-xs text-muted-foreground">
              ...
              fails the test. Turn off only if you want partial runs to count as
              passing.
            </span>
          </Label>
        </div>
                  ...
                  >
                    <Checkbox
                      ...
                      onCheckedChange={(checked) =>
                        setRule(label, "screenshot_changed", !!checked)
                      }
                    />
                    <Label
                      htmlFor={inputId}
                    ...
                    >
                      ...
                      {focusCount > 0 && (
                        <Badge
                          variant="outline"
                          className="gap-1 border-green-300 text-green-700 bg-green-50 dark:bg-green-950/40 dark:text-green-300 dark:border-green-800"
                        >
                          <Crosshair className="h-3 w-3" />
                          {focusCount === 1
                            ? "1 focus region"
                            : `${focusCount} focus regions`}
                        </Badge>
                      )}
                    </Label>
                  </div>
        ...
        <div className="border rounded-md p-3 space-y-3">
          ...
          {sortedAssertions.length === 0 ? (
            <p className="text-xs text-muted-foreground">
              No assertions parsed from this test yet. Each{" "}
              <code>expect(...)</code> in the test code becomes an assertion you
              can promote to a hard failure here.
            </p>
          ) : (
            <div className="space-y-1">
          ...
          <div className="border rounded-md p-3 space-y-3">
            <div className="flex items-center gap-2 flex-wrap">
              ...
              <span className="text-sm font-medium">
                End-of-test variable assertions
              </span>
              <Badge variant="outline" className="text-xs">
                {extractVars.length} extract var
                {extractVars.length === 1 ? "" : "s"}
              </Badge>
              <span className="text-[11px] text-muted-foreground">
                These also appear in the Vars tab. Edits sync.
              </span>
              {varStatus !== "idle" && (
                <span
            ...
            {extractVars.length === 0 ? (
              <p className="text-xs text-muted-foreground">
                No extract-mode variables yet. Add one on the Vars tab to assert
                a page field&apos;s value at end of test.
              </p>
            ) : (
                ...
                {extractVars.map((v) => (
                  ...
                  >
                    ...
                    <Input
                      ...
                      onChange={(e) =>
                        updateVar(v.id, { expectedValue: e.target.value })
                      }
                    />
                    <div className="col-span-2">
                      <Select
                        ...
                        onValueChange={(val) =>
                          updateVar(v.id, {
                            assertSeverity: val as "fail" | "warn",
                          })
                        }
                    ...
                    <div className="col-span-1 flex justify-end">
                      <Switch
                        ...
                        onCheckedChange={(enabled) =>
                          updateVar(v.id, { assertEnabled: enabled })
                        }
                      />
                    </div>
                  </div>
