...
export async function GET(request: NextRequest) {
  ...
  if (error) {
    return NextResponse.redirect(
      new URL("/settings?error=google_sheets_denied", getPublicUrl(request)),
    );
  }
  ...
  if (!stateParam || !storedState || stateParam !== storedState) {
    return NextResponse.redirect(
      new URL("/settings?error=google_sheets_csrf", getPublicUrl(request)),
    );
  }
  ...
  if (!tokenResponse) {
    return NextResponse.redirect(
      new URL(
        "/settings?error=google_sheets_token_failed",
  ...
  if (!googleUser) {
    return NextResponse.redirect(
      new URL(
        "/settings?error=google_sheets_user_failed",
        getPublicUrl(request),
      ),
    );
  }
  ...
  return NextResponse.redirect(
    new URL("/settings?success=google_sheets_connected", getPublicUrl(request)),
  );
}
