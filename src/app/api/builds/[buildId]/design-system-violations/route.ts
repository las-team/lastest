...
export async function GET(
  ...
  if (!session.team) return NextResponse.json({ error: 'No team' }, { status: 403 });

  const { buildId } = await params;
  const build = await queries.getBuild(buildId);
  if (!build) return NextResponse.json({ error: 'Build not found' }, { status: 404 });
  if (!build.testRunId) return NextResponse.json({ error: 'Build has no run' }, { status: 404 });
  const run = await queries.getTestRun(build.testRunId);
  if (!run?.repositoryId) return NextResponse.json({ error: 'Build has no repo binding' }, { status: 404 });
  ...
  if (format === 'csv') {
    const header = [
      'rule_id', 'category', 'property', 'actual', 'expected', 'expected_name',
      'impact', 'occurrence_count', 'total_nodes',
      'sample_test_id', 'sample_test_name', 'sample_area', 'sample_selector',
    ].join(',');
    const lines = [header];
    for (const v of violations) {
      const samples = v.samples.length > 0 ? v.samples : [{ testResultId: '', testId: null, testName: null, areaName: null, nodes: 0, sampleNode: undefined }];
