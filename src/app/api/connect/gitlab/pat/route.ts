...
export async function POST(request: NextRequest) {
  ...
  if (!instanceUrl || !pat) {
    return NextResponse.json(
      { error: "instanceUrl and pat are required" },
      { status: 400 },
    );
  }
  ...
  if (!user) {
    return NextResponse.json(
      { error: "Invalid PAT or unreachable instance" },
      { status: 400 },
    );
  }
  ...
  try {
    ...
    if (result.error) {
      ...
      warning =
        "PAT validated but 0 projects were returned. Confirm the token has the `api` scope and that you are a member of the projects you want to import.";
    }
