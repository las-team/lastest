...
export async function GET(request: NextRequest) {
  ...
  if (error) {
    return NextResponse.redirect(
      new URL("/settings?error=github_auth_denied", getPublicUrl(request)),
    );
  }
  ...
  if (stateRecord.userId !== session.user.id) {
    return NextResponse.redirect(
      new URL("/settings?error=session_mismatch", getPublicUrl(request)),
    );
  }
  ...
  const existingOAuth = await queries.getOAuthAccount(
    "github",
    ghUser.id.toString(),
  );
  if (existingOAuth && existingOAuth.userId !== session.user.id) {
    ...
    return NextResponse.redirect(
      new URL(
        `/settings?error=this_GitHub_account_is_already_connected_to_${encodeURIComponent(masked)}`,
  ...
  return NextResponse.redirect(
    new URL("/settings?success=github_connected", getPublicUrl(request)),
  );
}
