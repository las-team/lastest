...
import {
  startHealTestAgent,
  startGeneratePlaceholderTestAgent,
} from "@/server/actions/ai";
import {
  startRemoteRouteScan,
  generateBasicTests,
} from "@/server/actions/scanner";
...
function findAreaInTree(
  areas: FunctionalAreaWithChildren[],
  id: string,
): FunctionalAreaWithChildren | null {
...
export function DefinitionPageClient({
  ...
  const [showImportFromSpecDialog, setShowImportFromSpecDialog] =
    useState(false);
  const [reviewSpecImportId, setReviewSpecImportId] = useState<string | null>(
    null,
  );
  ...
  const [openTestDetailData, setOpenTestDetailData] = useState<Awaited<
    ReturnType<typeof getTestDetailData>
  > | null>(null);
  ...
  const [isBulkGeneratingPlaceholders, setIsBulkGeneratingPlaceholders] =
    useState(false);
  ...
  const [sort, setSort] = useState<TestsTableSort>({
    key: "lastRun",
    dir: "desc",
  });
  const [visibleColumns, setVisibleColumns] = useState<
    Set<TestsTableColumnKey>
  ...
  useEffect(() => {
    ...
    const storedSort = parseStoredSort(
      window.localStorage.getItem("lastest:tests-table:sort"),
    );
    if (storedSort) setSort(storedSort);
    const touched =
      window.localStorage.getItem("lastest:tests-table:columns-touched") ===
      "1";
    setColumnsTouched(touched);
    if (touched) {
      const stored = parseStoredColumns(
        window.localStorage.getItem("lastest:tests-table:columns"),
      );
  ...
  const [selectedDeletedIds, setSelectedDeletedIds] = useState<Set<string>>(
    new Set(),
  );
  ...
  useEffect(() => {
    ...
    if (sessionId) {
      ...
      router.replace(`/tests${params.toString() ? `?${params}` : ""}`, {
        scroll: false,
      });
    }
  }, [searchParams, router]);
  ...
  const failedScopedTests = useMemo(
    () => scopedTests.filter((t) => t.latestStatus === "failed"),
    [scopedTests],
  );
  ...
  const handleVisibleColumnsChange = useCallback(
    (cols: Set<TestsTableColumnKey>) => {
      setVisibleColumns(cols);
      setColumnsTouched(true);
      if (typeof window !== "undefined") {
        window.localStorage.setItem(
          "lastest:tests-table:columns",
  ...
  const areasWithActiveTests = useMemo(
    () => new Set(tests.map((t) => t.functionalAreaId).filter(Boolean)),
    [tests],
  );
  const uncoveredRoutes = useMemo(() => {
    return routes.length > 0
      ...
      : areas
          .filter(
            (a) => a.name.startsWith("/") && !areasWithActiveTests.has(a.id),
          )
          .map(
            (a) =>
              ({
  ...
  const statsData = useMemo(() => {
    const passed = scopedTests.filter(
      (t) => t.latestStatus === "passed",
    ).length;
    const failed = scopedTests.filter(
      (t) => t.latestStatus === "failed",
    ).length;
    const pending = scopedTests.filter((t) => !t.latestStatus).length;
    return [
      {
        label: "Total",
        value: scopedTests.length,
  ...
  const allFilteredSelected =
    ...
    filteredTests.every((t) => selectedTestIds.has(t.id));
  ...
  const handleOpenTest = useCallback(
    async (
      testId: string,
      { pushState = true }: { pushState?: boolean } = {},
    ) => {
    ...
    [openTestId, repositoryId, handleCloseTest],
  );
  ...
  const handleDeleteArea = async (withContents: boolean) => {
    ...
    try {
      ...
      if (treeSelection?.type === "area" && treeSelection.id === deleteAreaId)
        setTreeSelection(null);
  ...
  const handleDeleteMultipleAreas = async (withContents: boolean) => {
    ...
    try {
      ...
      if (
        ...
        deleteAreaIds.includes(treeSelection.id)
      )
        setTreeSelection(null);
      setDeleteAreaIds([]);
      setSelectedAreaIds(new Set());
  ...
  const handleDeleteTest = async () => {
    ...
    try {
      ...
      if (treeSelection?.type === "test" && treeSelection.id === deleteTestId)
        setTreeSelection(null);
  ...
  const toggleSelect = (id: string, shiftKey = false) => {
    ...
    if (shiftKey && lastSelectedIdRef.current) {
      const lastIdx = filteredTests.findIndex(
        (t) => t.id === lastSelectedIdRef.current,
      );
  ...
  const handleBulkRun = async () => {
    ...
    try {
      ...
      if ("queued" in result && result.queued) {
        ...
        toast.success(
          `Build started with ${testIds.length} test${testIds.length === 1 ? "" : "s"}`,
        );
        setSelectedTestIds(new Set());
  ...
  const handleBulkFix = async () => {
    ...
    try {
      ...
      if (failed > 0) {
        toast.error(
          `Failed to start ${failed} healing${failed === 1 ? "" : "s"}`,
        );
      }
      setSelectedTestIds(new Set());
    } finally {
  ...
  const handleBulkGeneratePlaceholders = async () => {
    ...
    try {
      ...
      if (failed > 0) {
        toast.error(
          `Failed to start ${failed} generation${failed === 1 ? "" : "s"}`,
        );
      }
    } finally {
  ...
  const toggleDeletedSelect = (id: string, shiftKey = false) => {
    ...
    if (shiftKey && lastSelectedDeletedIdRef.current) {
      const lastIdx = deletedTests.findIndex(
        (t) => t.id === lastSelectedDeletedIdRef.current,
      );
  ...
  function flattenAreas(
    items: FunctionalAreaWithChildren[],
    depth = 0,
  ): Array<FunctionalAreaWithChildren & { depth: number }> {
  ...
  return (
    ...
    >
      ...
      {!isMobile && (
        <>
          <ResizablePanel
            ...
            minSize="15%"
            maxSize="35%"
            className="bg-muted/30 h-full overflow-hidden flex flex-col"
          >
            ...
            <TabsList className="h-11 w-full max-w-5xl p-1 bg-white dark:bg-zinc-950 border">
              ...
              >
                ...
                {flatAreas.length > 0 && (
                  <Badge
                    variant="secondary"
                    className="ml-1.5 h-5 px-1.5 text-[10px] data-[state=active]:bg-accent-foreground/20 data-[state=active]:text-accent-foreground"
                  >
                    {flatAreas.length}
                  </Badge>
                )}
                ...
                <div className="flex items-center justify-between gap-2 md:gap-4 flex-wrap">
                  <div className="flex items-center gap-1.5 text-sm min-w-0">
                    ...
                    {breadcrumb.map((crumb) => (
                      ...
                      >
                        ...
                        <button
                          onClick={() => {
                            setTreeSelection({ type: "area", id: crumb.id });
                            handleCloseTest();
                          }}
                          className={cn(
                            "hover:text-primary transition-colors truncate max-w-[160px]",
                            treeSelection?.type === "area" &&
                              treeSelection.id === crumb.id &&
                              !openTestId
                              ? "text-foreground font-medium"
                              : "text-muted-foreground",
                          )}
                  ...
                  {!openTestId && (
                    <div className="flex items-center gap-2 flex-wrap shrink-0">
                      ...
                      {treeSelection?.type === "area" && (
                        <>
                          <Button
                            ...
                            onClick={() => handleNewArea(treeSelection.id)}
                          >
                          ...
                          <Button
                            ...
                            onClick={() => setDeleteAreaId(treeSelection.id)}
                          >
                      ...
                      <Button
                        asChild
                        variant={isEditingArea ? "outline" : "default"}
                        size="sm"
                        className="h-8 text-sm"
                      >
                        <Link href="/record">
                ...
                <div className="flex flex-col h-full flex-1">
                  <TestDetailClient
                    ...
                    onRefresh={async () => {
                      ...
                      try {
                        const data = await getTestDetailData(
                          openTestId,
                          repositoryId,
                        );
                        if (data) {
                  ...
                  <Card className="border-border/50 overflow-hidden">
                    <CardHeader className="border-b border-border/50 py-3">
                      {(() => {
                        const selectedArea =
                          treeSelection?.type === "area"
                            ? findAreaInTree(tree, treeSelection.id)
                            : null;
                        return (
                          <>
                                ...
                                {selectedArea && !isEditingArea && (
                                  <div className="flex gap-1">
                                    <Button
                                      ...
                                      onClick={() =>
                                        handleRunAreaAsBuild(treeSelection!.id)
                                      }
                                      disabled={isRunningAreaBuild}
                                    >
                              ...
                              <div className="mt-3 space-y-3">
                                ...
                                <div className="space-y-1.5">
                                  ...
                                  <Textarea
                                    ...
                                    value={editAreaPlan}
                                    onChange={(e) =>
                                      setEditAreaPlan(e.target.value)
                                    }
                                    rows={6}
                              ...
                              >
                                <div className="flex items-center gap-4 min-w-0">
                                  <Checkbox
                                    ...
                                    onClick={(e) => {
                                      e.stopPropagation();
                                      toggleDeletedSelect(test.id, e.shiftKey);
                                    }}
                                  />
                                  <div className="min-w-0 flex-1">
                  ...
                  {allAreaTests.length > 0 && (
                    <div className="text-sm text-muted-foreground">
                      ...
                      <span className="font-medium text-foreground">
                        {realTestCount}/{allAreaTests.length}
                      </span>{" "}
                      ({Math.round((realTestCount / allAreaTests.length) * 100)}
                      %)
                    </div>
                  )}
                  <Button
                    ...
                    disabled={isExporting || flatAreas.length === 0}
                  >
                    {isExporting ? (
                      <Loader2 className="h-3.5 w-3.5 mr-1.5 animate-spin" />
                    ) : (
                    ...
                    <div key={area.id}>
                      ...
                      <div className="px-4 pb-4">
                        <AreaTestCasesPanel
                          ...
                          onOpenTest={(testId) => {
                            setTreeSelection({ type: "test", id: testId });
                            setActiveTab("tests");
      ...
      <Dialog open={isNewAreaOpen} onOpenChange={setIsNewAreaOpen}>
        <DialogContent>
          ...
          <div className="space-y-4 py-4">
            ...
            <div className="space-y-2">
              <Label htmlFor="area-plan">
                Initial plan (optional, markdown)
              </Label>
              <Textarea
                id="area-plan"
                value={newAreaPlan}
                onChange={(e) => setNewAreaPlan(e.target.value)}
                placeholder="What tests belong in this area? (use the planner agent later to expand)"
                rows={3}
      ...
      >
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              What would you like to do with the tests and sub-folders inside
              this area?
            </DialogDescription>
          </DialogHeader>
          <div className="flex flex-col gap-2 py-2">
            ...
            >
              <div className="text-left">
                <div className="font-medium">
                  {isDeletingArea ? "Deleting..." : "Delete area only"}
                </div>
                <div className="text-xs text-muted-foreground font-normal">
                  Move contents to Unsorted and sub-folders to root
                </div>
              </div>
            </Button>
            <Button
            ...
            >
              <div className="text-left">
                <div className="font-medium">
                  {isDeletingArea ? "Deleting..." : "Delete everything"}
                </div>
                <div className="text-xs font-normal opacity-90">
                  Remove the area and all its tests and sub-folders
                </div>
              </div>
            </Button>
          </div>
      ...
      {/* Bulk Delete Areas */}
      <Dialog
        open={deleteAreaIds.length > 0}
        onOpenChange={(open) => !open && setDeleteAreaIds([])}
      >
        <DialogContent>
          <DialogHeader>
          ...
          <div className="flex flex-col gap-2 py-2">
            ...
            >
              <div className="text-left">
                <div className="font-medium">
                  {isDeletingArea ? "Deleting..." : "Delete areas only"}
                </div>
                <div className="text-xs text-muted-foreground font-normal">
                  Move contents to Unsorted and sub-folders to root
                </div>
            ...
            >
              <div className="text-left">
                ...
                <div className="text-xs font-normal opacity-90">
                  Remove all {deleteAreaIds.length} areas and their tests and
                  sub-folders
                </div>
              </div>
            </Button>
      ...
      >
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              Are you sure? It will be soft-deleted and can be restored later.
            </DialogDescription>
          </DialogHeader>
          <DialogFooter>
      ...
      <Dialog open={showDeleteConfirm} onOpenChange={setShowDeleteConfirm}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle>
              Move {selectedTestIds.size} test
              {selectedTestIds.size !== 1 ? "s" : ""} to trash?
            </DialogTitle>
            <DialogDescription>
              Tests will be moved to the Recently Deleted section.
            </DialogDescription>
          </DialogHeader>
          <DialogFooter>
            <Button
      ...
      >
        <DialogContent>
          <DialogHeader>
            <DialogTitle>
              Permanently delete {selectedDeletedIds.size} test
              {selectedDeletedIds.size !== 1 ? "s" : ""}?
            </DialogTitle>
            <DialogDescription>
              This action cannot be undone. All related data will also be
              deleted.
            </DialogDescription>
          </DialogHeader>
          <DialogFooter>
