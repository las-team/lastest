...
function NetworkPane({ result }: { result: InspectionResult }) {
  ...
  return (
    <div className="space-y-3 text-sm">
      <div className="flex flex-wrap gap-2">
        <Badge variant="secondary">+{n.added.length} new</Badge>
        <Badge variant="secondary">−{n.removed.length} dropped</Badge>
        ...
        {failedDelta !== 0 ? (
          <Badge variant="destructive">
            failed Δ {failedDelta > 0 ? '+' : ''}{failedDelta}
          </Badge>
        ) : null}
      </div>
          ...
          <table className="w-full text-xs">
            <thead className="bg-muted/50 text-left">
              <tr>
                ...
                <th className="px-2 py-1">Δ bytes</th>
              </tr>
            </thead>
            <tbody>
              {interesting.map((row, i) => {
                ...
                return (
                  <tr key={`${row._kind}-${i}-${row.url}`} className="border-t">
                    <td className="px-2 py-1 font-mono">{row._kind}</td>
