...
function deriveExecutionKind(
  ...
  if (result?.isFlaky || result?.retryOf || visual?.classification === "flaky")
    return "flaky";
  ...
  if (verdict !== "green" || visual?.classification === "changed")
    return "changed";
...
export function BoardView(props: BoardViewProps) {
  ...
  const kindCounts = useMemo(() => {
    const c: Record<ExecutionKind, number> = {
      errored: 0,
      changed: 0,
      flaky: 0,
      passed: 0,
  ...
  const grouped = useMemo(() => {
    const map: Record<CaseStatus, CaseCardData[]> = {
      regression: [],
      missed: [],
      unknown: [],
      done: [],
    ...
    for (const c of cases) {
      if (props.statusFilter.size > 0 && !props.statusFilter.has(c.status))
        continue;
  ...
  const activeDragCase = activeDragId
    ? (cases.find((c) => c.step.id === activeDragId) ?? null)
    : null;
      ...
      >
        ...
        >
          ...
          >
            ...
            >
              <div style={{ position: "absolute", inset: 0, display: "flex" }}>
                <div
                  style={{
                    width: `${wPct(statusTotals.done)}%`,
                    background: "var(--c-teal)",
                  }}
                />
                <div
                  style={{
                    width: `${wPct(statusTotals.missed)}%`,
                    background: "var(--c-amber)",
                  }}
                />
                <div
                  style={{
                    width: `${wPct(statusTotals.regression)}%`,
                    background: "var(--c-red)",
                  }}
                />
                <div
                  style={{
                    width: `${wPct(statusTotals.unknown)}%`,
                    background: "var(--fg-3)",
                  }}
                />
              </div>
          ...
          >
            ...
            />
            {(["errored", "changed", "flaky", "passed"] as ExecutionKind[]).map(
              (k) => (
                <KindToken
                  key={k}
        ...
        {props.isRunning && (
          <div
            style={{
              padding: "8px 20px",
              background:
                "color-mix(in oklab, var(--c-blue) 6%, var(--c-white))",
              borderBottom: "1px solid var(--border)",
              display: "flex",
              alignItems: "center",
              gap: 10,
          ...
          >
            ...
            <span className="label">
              cards land in their column as each test finishes
            </span>
            <span style={{ flex: 1 }} />
        ...
        >
          ...
          {grouped.regression.length > 0 &&
            countWithoutIssue(grouped.regression) > 0 && (
              <span className="v-chip regression">
                <CircleDot size={11} />
                {countWithoutIssue(grouped.regression)} broken case
                {countWithoutIssue(grouped.regression) === 1 ? "" : "s"} need an
                issue
              </span>
            )}
          {grouped.done.length > 0 &&
            countWithLinkedIssue(grouped.done) > 0 && (
              <span className="v-chip done">
                <CheckCircleIcon size={11} />
                {countWithLinkedIssue(grouped.done)} verified case
                {countWithLinkedIssue(grouped.done) === 1 ? "" : "s"} ready to
                close
              </span>
            )}
          <span style={{ flex: 1 }} />
          <span className="label" style={{ fontSize: 9 }}>
            {verified === total && total > 0 ? "all verified" : "—"}
          </span>
        </div>
      </div>
...
function countWithLinkedIssue(cases: CaseCardData[]): number {
  return cases.filter(
    (c) => c.step.githubIssueUrl && c.step.githubIssueState !== "closed",
  ).length;
}
  ...
  return (
    <button
      ...
      style={{
        ...
        opacity: dim ? 0.55 : 1,
        transition:
          "background 120ms ease, opacity 120ms ease, border-color 120ms ease",
      }}
    >
      <span
  ...
  return (
    <div
      ...
      style={{
        ...
        borderRadius: 8,
        boxShadow: isHovered
          ? "inset 0 0 0 2px color-mix(in oklab, var(--c-teal) 35%, transparent)"
          : undefined,
        transition:
    ...
    >
      ...
      >
        ...
        {cases.length > visible.length && (
          ...
          >
            +{cases.length - visible.length} more
          </div>
        )}
      </div>
    </div>
      ...
      {[0, 1, 2].map((i) => (
        <div
          ...
          style={{
            ...
            height: 132,
            background:
              "linear-gradient(90deg, var(--c-soft) 0%, var(--c-soft-2) 50%, var(--c-soft) 100%)",
...
function renderVerifiedGrouped(
  ...
  const byArea = new Map<
    string,
    { area: CaseCardData["area"]; rows: CaseCardData[] }
  >();
  for (const c of cases) {
    const key = c.area?.id ?? "__unscoped__";
  ...
  const isCleanCase = (c: CaseCardData) =>
    !c.step.githubIssueUrl && !c.feedback.some((f) => f.status === "rejected");
      ...
      return (
        ...
        >
          ...
          >
            ...
            {fullyVerified && (
              <CheckCircleIcon size={12} style={{ color: "var(--c-teal)" }} />
            )}
  ...
  return (
    <div onClick={(e) => e.stopPropagation()} style={{ marginTop: 8 }}>
      ...
      {saving && (
        <div
          className="label"
          style={{ fontSize: 9, marginTop: 2, color: "var(--fg-3)" }}
        >
          saving…
        </div>
      )}
  ...
  const layerSummaries = useMemo(
    () =>
      summarizeLayersForCard(
        ...
        data.visual,
        checkModes,
        checkModesByTestId[data.test?.id ?? ""] ?? null,
      ),
    [
      data.step,
      data.result,
...
function summarizeLayersForCard(
  ...
  const evidenceByLayer = new Map<
    EvidenceLayer,
    StepComparison["evidence"][number]
  >();
...
function wasLayerCaptured(
  ...
  switch (layer) {
    case "visual":
      return !!(visual?.currentImagePath || visual?.baselineImagePath);
    case "dom":
    ...
    case "url":
      return result?.urlTrajectory != null;
    case "variable":
      return (
  ...
  switch (layer) {
    ...
    case "perf": {
      ...
      const worst = [...p.deltas].sort((a, b) => {
        if (a.budgetBreached !== b.budgetBreached)
          return a.budgetBreached ? -1 : 1;
  ...
  if (!step.githubIssueUrl) {
    ...
    return (
      <span
        ...
        title={
          onOpenPicker
            ? "Browse or file an issue for this case"
            : "No linked issue"
        }
      >
        <CircleDot size={11} />
