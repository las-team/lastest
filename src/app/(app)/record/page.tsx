...
export default async function RecordPage({ searchParams }: RecordPageProps) {
  ...
  const areas = selectedRepo
    ? await getFunctionalAreasByRepo(selectedRepo.id)
    : [];
  ...
  const availableTests = availableTestsRaw.map((t) => ({
    id: t.id,
    name: t.name,
  }));
  const availableScripts = availableScriptsRaw.map((s) => ({
    id: s.id,
    name: s.name,
  }));
