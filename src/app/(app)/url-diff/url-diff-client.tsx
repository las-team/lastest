...
export function UrlDiffClient() {
  ...
  const { job, isComplete, isFailed, error } = useJobResult(jobId, {
    pollInterval: 2000,
  });
  ...
  const onSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      toast.error(
        err instanceof Error ? err.message : "Failed to start URL Diff",
      );
    } finally {
      setSubmitting(false);
    }
    ...
    <div className="flex flex-col gap-6 p-6 max-w-7xl mx-auto w-full">
      <header>
        ...
        <p className="text-muted-foreground text-sm mt-1">
          Compare two URLs across visual, DOM, network, and accessibility
          dimensions.
        </p>
      </header>
      ...
      <Card className="p-6">
        <form onSubmit={onSubmit} className="grid gap-4">
          ...
          <div className="flex items-end gap-4 flex-wrap">
            ...
            <Button
              type="submit"
              disabled={submitting || (!!jobId && !isComplete && !isFailed)}
            >
        ...
        <Card className="p-6 flex items-center gap-3">
          ...
          <div>
            ...
            <div className="text-sm text-muted-foreground">
              {job?.label} · step {job?.completedSteps ?? 0}/
              {job?.totalSteps ?? 4}
            </div>
          </div>
        </Card>
    ...
    <Tabs defaultValue="visual" className="w-full">
      <TabsList>
        ...
        <TabsTrigger value="dom">
          DOM{" "}
          <Badge variant="secondary" className="ml-2">
            {result.dom.added.length +
              result.dom.removed.length +
              result.dom.changed.length}
          </Badge>
        </TabsTrigger>
        ...
        <Card className="p-4 space-y-4">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            ...
            <Stat
              ...
              value={result.network.changedStatus.length}
            />
                ...
                {result.network.changedStatus.map((c, i) => (
                  <div key={i} className="truncate">
                    ...
                    <span className="text-destructive">
                      {c.baseline?.status}→{c.current?.status}
                    </span>{" "}
                    {c.url}
                  </div>
                ))}
      ...
      <TabsContent value="a11y">
        <Card className="p-4 space-y-4">
          <div className="grid grid-cols-2 gap-4">
            <ScoreCard
              label="A"
              score={result.a11y.scoreA.score}
              violations={result.a11y.scoreA.violatedRules}
            />
            <ScoreCard
              label="B"
              score={result.a11y.scoreB.score}
              violations={result.a11y.scoreB.violatedRules}
            />
          </div>
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
...
function VisualTab({ visual }: { visual: UrlDiffResultShape["visual"] }) {
  ...
  const [selectedKey, setSelectedKey] = useState(
    visual.defaultKey ?? variants[0]!.key,
  );
  ...
  return (
    <Card className="p-4 space-y-4">
        ...
        {variants.map((v) => (
          ...
          >
            ...
            <span className="ml-2 opacity-70">
              {v.percentageDifference.toFixed(1)}%
            </span>
          </Button>
        ))}
      </div>
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        ...
        <ScreenshotPanel
          label={`Diff · ${selected.label}`}
          rel={selected.diffRelPath}
        />
      </div>
      <div className="text-sm text-muted-foreground">
  ...
  return (
    <div className="border rounded p-3">
      ...
      <div className="text-xs text-muted-foreground mt-1">
        {thirdParties.length} 3rd-party domain
