    ...
    case "changed":
      return diffs.filter(
        (d) =>
          ...
          (d.pixelDifference && d.pixelDifference > 0 && !d.classification),
      );
    case "failed":
      return diffs.filter(
...
const aiRecommendationBadge: Record<
  string,
...
function deriveBranchStatus(diff: VisualDiffWithTestStatus): BranchStatus {
  if (diff.metadata && (diff.metadata as { isNewTest?: boolean }).isNewTest)
    return "new_test";
  if (diff.status === "todo") return "has_todo";
  if (diff.status === "approved") return "branch_accepted";
  if (diff.classification === "unchanged" || diff.status === "auto_approved")
    return "baseline";
  // 0px diffs are visually identical — treat as baseline regardless of stale classification
  if (diff.pixelDifference === 0 || diff.pixelDifference === null)
    return "baseline";
  return "new_change";
}
...
export function BuildDetailClient({
  ...
  const [viewMode, setViewMode] = useState<"branch" | "main">(
  ...
  const aiSafeCount = banAiMode
    ? 0
    : diffs.filter((d) => d.aiRecommendation === "approve").length;
  const aiReviewCount = banAiMode
    ? 0
    : diffs.filter((d) => d.aiRecommendation === "review").length;
  const aiFlagCount = banAiMode
    ? 0
    : diffs.filter((d) => d.aiRecommendation === "flag").length;
  const analyzedCount = banAiMode
    ? 0
    : diffs.filter((d) => d.aiRecommendation).length;
  const analyzingCount = banAiMode
    ? 0
    : diffs.filter(
        (d) =>
          d.aiAnalysisStatus === "running" || d.aiAnalysisStatus === "pending",
      ).length;
  const failedAnalysisCount = banAiMode
    ? 0
    : diffs.filter((d) => d.aiAnalysisStatus === "failed").length;
  const pendingSafeCount = banAiMode
    ? 0
    : diffs.filter(
  ...
  const hasAIActivity =
    !banAiMode &&
    (analyzedCount > 0 || analyzingCount > 0 || failedAnalysisCount > 0);
  const totalAnalyzable = diffs.filter(
  ...
  const allFilteredSelected =
    ...
    filteredDiffs.every((d) => selectedIds.has(d.id));
  ...
  const handleBulkAddTodo = async () => {
    ...
    setIsProcessing(true);
    try {
      await batchAddDiffTodos(
        Array.from(selectedIds),
        bulkTodoDescription.trim(),
      );
      setSelectedIds(new Set());
              ...
              <div>
                ...
                {hasAIActivity && (
                  <div className="flex items-center gap-2 text-xs text-purple-600">
                    ...
                    <span>
                      {analyzedCount}/{totalAnalyzable} analyzed
                    </span>
                    {analyzingCount > 0 && (
          ...
          {selectedIds.size > 0 && (
            <div className="flex items-center gap-3 mb-3 px-4 py-2.5 bg-blue-50 border border-blue-200 rounded-lg">
              ...
              {showBulkTodoInput ? (
                ...
                <button
                  onClick={() => {
                    setShowBulkTodoInput(true);
                    setTimeout(() => bulkTodoInputRef.current?.focus(), 50);
                  }}
                  disabled={isProcessing}
            ...
            <div className="text-center py-8 text-muted-foreground border rounded-lg">
              {isFilterActive ? (
                <div className="space-y-2">
                  <p>
                    No tests match the &quot;{filterLabels[activeFilter]}&quot;
                    filter.
                  </p>
                  <button
                    onClick={() => setActiveFilter("all")}
              ...
              {!groupByArea && !groupByTest ? (
                ...
                <div className="space-y-3">
                  {groupedDiffs.map(([areaName, areaDiffs]) => {
                    const changedCount = areaDiffs.filter(
                      (d) => d.classification === "changed",
                    ).length;
                    const pendingCount = areaDiffs.filter(
                      (d) => d.status === "pending",
                    ).length;
                    const approvedCount = areaDiffs.filter(
                      (d) =>
                        d.status === "approved" || d.status === "auto_approved",
                    ).length;
                    const failedCount = areaDiffs.filter(
                      (d) =>
                              ...
                              <div className="space-y-2 p-3">
                                {areaDiffs.map((diff) => (
                                  <DiffRow
                                    ...
                                    isSelected={selectedIds.has(diff.id)}
                                    onToggleSelect={toggleSelect}
                                    hasCodeChange={codeChangeTestIdSet.has(
                                      diff.testId,
                                    )}
                                    hasMultipleBrowsers={hasMultipleBrowsers}
                                    banAiMode={banAiMode}
  ...
  return (
    <div className="space-y-2">
      {testGroups.map(([testId, testDiffs]) => {
        ...
        const pendingCount = testDiffs.filter(
          (d) => d.status === "pending",
        ).length;
        const failedCount = testDiffs.filter(
          (d) => d.testResultStatus === "failed" || d.status === "rejected",
        ).length;
        const stepLabel =
          testDiffs.length === 1 ? "1 case" : `${testDiffs.length} cases`;

        return (
          <Collapsible
            key={`test-${testId}-${expandKey}`}
            defaultOpen={allExpanded}
          >
            <div>
...
function DiffRow({
  ...
  const statusColor = isExecutionFailed
    ? "text-red-600 bg-red-50"
    : diffStatusColors[diff.status];
  const isFailed = isExecutionFailed || diff.status === "rejected";
  const aiBadge =
    !banAiMode && diff.aiRecommendation
      ? aiRecommendationBadge[diff.aiRecommendation]
      : null;
  const analysis = !banAiMode
    ? (diff.aiAnalysis as AIDiffAnalysis | null)
    : null;
  const isAnalyzing =
    !banAiMode &&
    (diff.aiAnalysisStatus === "running" ||
      diff.aiAnalysisStatus === "pending");
  ...
  const displayPixels =
    viewMode === "main" ? diff.mainPixelDifference : diff.pixelDifference;
  const hasViewData =
    viewMode === "main"
    ...
    >
      <div className="flex items-center gap-3 min-w-0">
        ...
        <div className="min-w-0">
          <div
            className={`font-medium truncate ${isFailed ? "text-red-800" : ""}`}
          >
            {diff.testName || "Unnamed Test"}
            {diff.stepLabel && (
              <span className="text-muted-foreground font-normal">
                {" "}
                &rsaquo; {diff.stepLabel}
              </span>
            )}
          </div>
      ...
      <div className="flex items-center gap-4 shrink-0">
        ...
        {hasMainDrift && (
          <span
            ...
            title="Drift from main baseline"
          >
        ...
        {aiBadge && (
          <span
            className={`inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-xs font-medium ${aiBadge.className}`}
          >
            <Sparkles className="w-3 h-3" />
            {aiBadge.label}
            {analysis && (
        ...
        <ExternalLink
          className={`w-4 h-4 ${isFailed ? "text-destructive/60" : "text-muted-foreground/50"}`}
        />
      </div>
    </div>
  );
