...
import { isValidClientId, isAllowedRedirectUri, LAUNCH_SCOPE } from '@/lib/launch/oauth-config';
...
export async function GET(request: NextRequest) {
  ...
  const granted =
    requestedScope.split(/\s+/).filter((s) => supported.includes(s)).join(' ') || LAUNCH_SCOPE;

  const token = randomBytes(32).toString('base64url');
  const ttl = DEFAULT_LAUNCH.tokenTtlSeconds;
  const expiresAt = new Date(Date.now() + ttl * 1000);
  await queries.createLaunchToken({ userId: session.user.id, token, scope: granted, expiresAt });
