 ...
 */
export async function setBuildManualScope(buildId: string, areaIds: string[]): Promise<void> {
 ...
 */
export async function computeChangeMap(buildId: string): Promise<ChangeMap | null> {
  ...
  const branch = (build.testRunId ? (await queries.getTestRun(build.testRunId))?.gitBranch : null)
    || repo.selectedBranch
    || repo.defaultBranch
    || 'main';
    ...
    if (account?.accessToken && branch !== baseBranch) {
      ...
      if (compareResult) {
        files = compareResult.files
          .filter((f) => ['added', 'modified', 'removed', 'renamed', 'changed'].includes(f.status))
          .map((f) => ({
            path: f.filename,
            pkg: f.filename.split('/').slice(0, 2).join('/'),
            status: f.status === 'added' ? 'A' : f.status === 'removed' ? 'D' : 'M',
  ...
  const affectedTests = changedPaths.length > 0
  ...
  const redOrYellowTestIds = new Set(
    stepRows.filter((s) => s.verdict === 'red' || s.verdict === 'yellow').map((s) => s.testId),
  );
  ...
  if (candidateAreaIds.length > 0 && files.length > 0) {
    ...
    if (aiResult.kind === 'ok') {
      ...
      for (const a of aiResult.areas) {
        areaAINarrative.set(a.areaId, { risk: a.risk, aiNarrative: a.aiNarrative });
  ...
  // ── Assemble & rank ─────────────────────────────────────────────────────
  const rankedAreas: ChangeMapArea[] = candidateAreaIds.map((id): ChangeMapArea => {
  ...
  rankedAreas.sort((a, b) => {
    ...
    if (RISK_RANK[a.risk] !== RISK_RANK[b.risk]) return RISK_RANK[b.risk] - RISK_RANK[a.risk];
  ...
  const steps = stepRows
    ...
    .map((s) => ({
      ...
      reason: s.verdict === 'red' ? 'Red verdict from multi-layer scorer' : 'Yellow verdict from multi-layer scorer',
    }));
...
type AIResult =
  | { kind: 'ok'; intentSummary: string; riskSummary: string; modelId: string; areas: Array<Pick<ChangeMapArea, 'areaId' | 'risk' | 'aiNarrative'>> }
...
async function runChangeMapAI(input: RunAIInput): Promise<AIResult> {
  ...
  if (!settings.aiDiffingEnabled) return { kind: 'skipped', reason: 'AI diffing disabled' };
  ...
  if (rawProvider === 'same-as-test-gen') {
    if (settings.provider === 'claude-cli') return { kind: 'skipped', reason: 'claude-cli has no JSON-mode chat' };
    if (settings.provider === 'ollama') {
      ...
      model = settings.openrouterModel || 'anthropic/claude-sonnet-4-5-20250929';
    }
  ...
  try {
    ...
    return { kind: 'skipped', reason: `AI call failed: ${(e as Error).message}` };
  }
}
