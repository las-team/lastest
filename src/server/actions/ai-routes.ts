...
import {
  ...
  createCodeDiffScanPrompt,
  SYSTEM_PROMPT,
  MCP_SYSTEM_PROMPT,
  ROUTE_SCAN_SYSTEM_PROMPT,
} from "@/lib/ai";
import type {
  AIProviderConfig,
...
function groupRoutesByArea(
  routes: (DiscoveredRoute & { functionalArea?: string })[],
): DiscoveredArea[] {
...
async function getCodebaseContext(
  ...
  for (const routeDir of routeDirs) {
    const filesInDir = tree.filter((entry) => {
      ...
      if (!entry.path.startsWith(routeDir + "/") && entry.path !== routeDir)
        return false;
...
export async function aiScanRoutes(
  ...
  try {
    ...
    const { context: codebaseContext, hasRoutingDirs } =
    ...
    if (!hasRoutingDirs) {
      ...
      return {
        ...
        functionalAreas: [],
        error:
          "No routing directories found in codebase — skipping AI route scan",
      };
    }

    const prompt = createRouteScanPrompt(
      codebaseContext,
      repo.fullName,
      intelligence,
    );
    ...
    if (objStr) {
      ...
      if (parsed.routes && Array.isArray(parsed.routes)) {
        ...
        return {
          success: true,
          functionalAreas: groupRoutesByArea(parsed.routes),
        };
      }
    }
    ...
    if (arrStr) {
      ...
      return {
        success: true,
        functionalAreas: [{ name: "Discovered Routes", routes }],
      };
    }
    ...
    const message =
      error instanceof Error ? error.message : "Failed to scan routes";
...
export async function mcpExploreRoutes(
  ...
  try {
    ...
    const prompt = createMCPExploreRoutesPrompt(
      baseURL,
      existingPaths,
      intelligence,
    );
    ...
    const message =
      error instanceof Error ? error.message : "Failed to explore routes";
    return { success: false, error: message };
  }
}
  ...
  try {
    ...
    for (const area of areas) {
      ...
      for (let i = 0; i < createdRoutes.length; i++) {
        ...
        if (
          originalRoute.testSuggestions &&
          originalRoute.testSuggestions.length > 0
        ) {
...
function filterRelevantSourceFiles(
  ...
  return files.filter((f) => {
    ...
    if (
      IGNORED_PATHS.some(
        (p) => f.filename.startsWith(p) || f.filename.includes("/" + p),
      )
    )
      return false;
...
export async function scanBranchDiff(repositoryId: string): Promise<{
  success: boolean;
  functionalAreas?: DiscoveredArea[];
  fileCount?: number;
  error?: string;
  ...
  try {
    ...
    if (baseBranch === headBranch) {
      await failJob(
        jobId,
        "Selected branch is the same as the default branch. Select a feature branch first.",
      );
      return {
        success: false,
        error:
    ...
    if (!comparison) {
      await failJob(
        jobId,
        "Could not compare branches. Check that both branches exist.",
      );
      return {
        success: false,
        error: "Could not compare branches. Check that both branches exist.",
      };
    }
    ...
    if (relevantFiles.length === 0) {
      ...
      return {
        success: false,
        error: "No relevant source files changed between branches",
      };
    }
    ...
    const config = await getAIConfig(repositoryId);
    const prompt = createCodeDiffScanPrompt(
      context,
      baseBranch,
      headBranch,
      repo.fullName,
    ...
    if (objStr) {
      ...
      if (parsed.functionalAreas && Array.isArray(parsed.functionalAreas)) {
        ...
        return {
          success: true,
          functionalAreas: parsed.functionalAreas,
    ...
    if (arrStr) {
      ...
      return {
        success: true,
        functionalAreas: [{ name: "Changed Routes", routes }],
        fileCount: relevantFiles.length,
      };
    }
    ...
    const message =
      error instanceof Error ? error.message : "Failed to analyze branch diff";
    await failJob(jobId, message);
    return { success: false, error: message };
  }
}
