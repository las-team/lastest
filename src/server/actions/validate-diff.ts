...
import { findAffectedTests, type AffectedTest } from '@/lib/smart-selection/file-matcher';
 ...
 */
export async function validateDiffCore(opts: ValidateDiffOptions): Promise<ValidateDiffResult> {
  const { repositoryId } = opts;
  const wait = opts.wait !== false;
  const maxWaitMs = opts.maxWaitMs && opts.maxWaitMs > 0 ? opts.maxWaitMs : DEFAULT_MAX_WAIT_MS;
  ...
  if (opts.diff && opts.diff.trim()) {
    ...
    if (!repo || repo.provider !== 'github' || !repo.teamId) {
      return {
        ...
        summary: 'Branch-range mode requires a GitHub-connected repo. Pass `diff` text for local repos.',
        changedFiles: [],
        affectedTests: [],
        scopedTestIds: [],
      };
  ...
  const run = await createAndRunBuildCore('manual', scopedTestIds, repositoryId);
  if (!run.buildId) {
    return {
  ...
  const [counts, diffs, changeMap, results] = await Promise.all([
    ...
    build.testRunId ? queries.getTestResultsByRun(build.testRunId) : Promise.resolve([]),
  ]);
  ...
  const failingTests = await Promise.all(
    failingResults.map(async (r) => {
      const t = r.testId ? await queries.getTest(r.testId).catch(() => null) : null;
      return {
        testId: r.testId ?? '',
