...
export async function syncCsvSource(
  ...
  if (!source.repositoryId)
    return { success: false, error: "Source has no repository" };
  ...
  if (incoming && incoming.length > 0) {
    ...
    try {
      ...
      return {
        success: false,
        error: "Original file no longer on disk; re-upload required",
      };
    }
  } else {
...
export async function updateCsvSourceAlias(id: string, alias: string) {
  ...
  if (alias !== source.alias) {
    const conflict = await queries.getCsvDataSourceByAlias(
      source.repositoryId,
      alias,
    );
    if (conflict)
