 ...
 */
async function assertOllamaBaseUrlSafe(
...
export async function saveAISettings(data: {
  ...
  if (isMaskedValue(settingsData.openrouterApiKey))
    delete settingsData.openrouterApiKey;
  if (isMaskedValue(settingsData.aiDiffingApiKey))
    delete settingsData.aiDiffingApiKey;
  if (isMaskedValue(settingsData.anthropicApiKey))
    delete settingsData.anthropicApiKey;
  if (isMaskedValue(settingsData.openaiApiKey))
  ...
  try {
    if (provider === "claude-cli") {
      ...
      const { stdout, stderr } = await execAsync(
        'claude -p "Say hello in one word" < /dev/null',
        {
          timeout: 30000,
          shell: "/bin/bash",
      ...
      return {
        success: false,
        message: error.error?.message || "OpenRouter connection failed",
      };
      ...
      return {
        success: false,
        message: "Claude Agent SDK returned empty response",
      };
      ...
      try {
        ...
        if (err instanceof SsrfBlockedError) {
          return {
            success: false,
            message: `Ollama baseUrl rejected: ${err.message}`,
          };
        }
        throw err;
      }
      ...
      try {
        ...
        if (response.ok) {
          return {
            success: true,
            message: `Ollama connected successfully (${ollamaModel})`,
          };
        }
      ...
      return {
        success: false,
        message: error.error?.message || "Anthropic connection failed",
      };
      ...
      return {
        success: false,
        message: error.error?.message || "OpenAI connection failed",
      };
    }
