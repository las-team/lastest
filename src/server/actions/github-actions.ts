...
export async function updateGithubActionConfigAction(
  ...
  const session = await requireTeamAdmin();
  const config = await queries.updateGithubActionConfig(
    id,
    session.team.id,
    input,
  );
...
export async function validateGithubActionSetup(
  ...
  if (!ghAccount) {
    ...
    result.githubAccount = {
      status: "pass",
      message: `Connected as ${ghAccount.githubUsername}`,
    };
  ...
  if (!serverUrl || serverUrl === "http://localhost:3000") {
    result.serverUrl = {
      status: "warn",
      message: `Server URL is "${serverUrl || "not set"}" — not reachable from GitHub Actions`,
    };
  } else {
    try {
