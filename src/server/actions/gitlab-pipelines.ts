...
export async function updateGitlabPipelineConfigAction(
  ...
  const session = await requireTeamAdmin();
  const config = await queries.updateGitlabPipelineConfig(
    id,
    session.team.id,
    input,
  );
...
export async function deployPipelineToGitlab(configId: string) {
  const session = await requireTeamAdmin();
  const config = await queries.getGitlabPipelineConfig(
    configId,
    session.team.id,
  );
  if (!config) throw new Error("Config not found");
  ...
  if (isEphemeral || isAuto) {
    if (!runnerId) {
      ...
      const regen = await regenerateRunnerTokenInternal(
        runnerId,
        session.team.id,
      );
      if ("error" in regen)
...
export async function validateGitlabPipelineSetup(
  ...
  if (!glAccount) {
    ...
    result.gitlabAccount = {
      status: "pass",
      message: `Connected as @${glAccount.gitlabUsername}`,
    };
    result.ciFile = {
      status: "fail",
  ...
  if (config.runnerId) {
    ...
    if (!runner) {
      result.runner = {
        status: "fail",
        message: "Linked runner not found in database",
      };
    } else if (runner.status === "online") {
      result.runner = {
        status: "pass",
        message: `Runner "${runner.name}" is online`,
      };
    } else {
      result.runner = {
        status: "warn",
    ...
    if (mode === "persistent") {
      ...
      result.runner = {
        status: "pass",
        message: `${mode} mode — runner created on demand`,
      };
    }
  }

  const serverUrl =
    process.env.NEXT_PUBLIC_APP_URL || process.env.BETTER_AUTH_BASE_URL;
  if (!serverUrl || serverUrl === "http://localhost:3000") {
    result.serverUrl = {
      status: "warn",
      message: `Server URL is "${serverUrl || "not set"}" — not reachable from GitLab CI`,
    };
  } else {
    try {
      ...
      if (healthRes.ok) {
        result.serverUrl = {
          status: "pass",
          message: `${serverUrl} is reachable`,
        };
      } else {
        result.serverUrl = {
          status: "fail",
          message: `${serverUrl} returned ${healthRes.status}`,
        };
      }
    } catch {
      result.serverUrl = {
        status: "fail",
        message: `${serverUrl} is not reachable`,
      };
    }
  }
