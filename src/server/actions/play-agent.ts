...
const STEP_DEFINITIONS: Array<{
...
}> = [
  {
    ...
    description: "Verify GitHub, AI, and environment configuration",
  },
  {
    id: "select_repo",
    label: "Select Repository",
    description: "Ensure a repository is selected",
  },
  {
    id: "env_setup",
    label: "Seed",
    description: "Verify server, detect login, configure seed",
  },
  {
    id: "scan_and_template",
    label: "Scan & Template",
    description: "Scan routes and apply testing template",
  },
  {
    id: "plan",
    label: "Plan Tests",
    description: "Discover functional areas and create test plans",
  },
  {
    id: "review",
    label: "Review Plan",
    description: "Review and approve the test plan",
  },
  {
    id: "generate",
    label: "Generate Tests",
    description: "Generate test code from approved plan",
  },
  {
    id: "run_tests",
    label: "Run Tests",
    description: "Create and run initial build",
  },
  {
    id: "fix_tests",
    label: "Fix Failing Tests",
    description: "AI-fix failing tests (max 3 attempts each)",
  },
  {
    id: "rerun_tests",
    label: "Re-run Tests",
    description: "Re-run build after fixes",
  },
  {
    id: "summary",
    label: "Summary",
    description: "Show results and pass/fail delta",
  },
];
...
async function setStepWaitingUser(
  sessionId: string,
  stepId: AgentStepId,
  userAction: string,
) {
...
async function isAbortedOrCancelled(
  sessionId: string,
  signal: AbortSignal,
): Promise<boolean> {
...
async function skipIfManualMode(
  sessionId: string,
  stepId: AgentStepId,
): Promise<boolean> {
  const session = await queries.getAgentSession(sessionId);
...
async function waitForBuild(
  ...
  signal: AbortSignal,
  sessionId?: string,
): Promise<Awaited<ReturnType<typeof getBuildSummary>>> {
  for (;;) {
    if (
      sessionId ? await isAbortedOrCancelled(sessionId, signal) : signal.aborted
    )
      return null;
...
async function runSettingsCheck(
  ...
  await setStepCompleted(sessionId, "settings_check", {
    ghAccount:
      ghAccount?.githubUsername || (skipGithub ? "Skipped" : "Connected"),
    aiProvider: isManual ? "manual" : aiSettings.provider,
    pwAgentEnabled: pwEnabled,
    activeAgents: pwEnabled
      ? ["scout", "diver", "planner", "generator", "healer"]
      : [],
  });
  return true;
}

async function runSelectRepo(
  sessionId: string,
  repositoryId: string,
  _teamId: string,
  _signal: AbortSignal,
...
async function detectLoginRequired(
  ...
  try {
    ...
    await page
      .goto(baseUrl, { waitUntil: "networkidle", timeout: 15000 })
      .catch(() => {});
    ...
    const pageContent = JSON.stringify(
      { url, title, bodyText: bodyText.slice(0, 2000), forms, links },
      null,
      2,
    );
    ...
    if (hasPasswordField || isLoginPage) {
      ...
      return {
        ...
        registerUrl: registerLink?.href
          ? normalizeUrl(registerLink.href)
          : undefined,
        pageContent,
      };
    }
...
${
  hasRegister
    ? `The app has a registration page${registerUrl ? ` at ${registerUrl}` : ""}.
First register a new test account, then log in with it.`
    : `The app requires login${loginUrl ? ` at ${loginUrl}` : ""}.
...
Use stepLogger.log() to report progress.`;
    ...
    const message =
      error instanceof Error
        ? error.message
        : "Failed to generate login script";
...
async function runEnvSetup(
  ...
  if (!connResult.success) {
    await updateSubsteps(sessionId, "env_setup", [
      {
        label: "Checking base URL",
        status: "error",
        detail: `Unreachable (${connResult.statusCode || "timeout"})`,
      },
      ...(!isManual
        ? [
            { label: "Detecting login", status: "pending" as const },
    ...
    await updateSubsteps(sessionId, "env_setup", [
      {
        label: "Checking base URL",
        status: "done",
        detail: `${connResult.responseTime}ms`,
      },
    ]);
  ...
  const hasLegacyDefault = !!(
    repoRecord?.defaultSetupTestId || repoRecord?.defaultSetupScriptId
  );

  if (usableDefaultSteps.length > 0 || hasLegacyDefault) {
    const stepCount =
      usableDefaultSteps.length +
      (hasLegacyDefault && usableDefaultSteps.length === 0 ? 1 : 0);
    await updateSubsteps(sessionId, "env_setup", [
      {
        label: "Checking base URL",
        status: "done",
      ...
      { label: "Detecting login", status: "done", detail: "Existing seed" },
      {
        label: "Login setup",
        status: "done",
        detail: `Already configured (${stepCount} step${stepCount === 1 ? "" : "s"})`,
      },
    ]);
    await setStepCompleted(sessionId, "env_setup", {
      url: baseUrl,
  ...
  await updateSubsteps(sessionId, "env_setup", [
    ...
    { label: "Claiming browser", status: "running" },
  ...
  if (!eb) {
    await updateSubsteps(sessionId, "env_setup", [
      ...
      {
        label: "Claiming browser",
        status: "error",
        detail: "No browsers available",
      },
    ]);
    await setStepFailed(
      sessionId,
      "env_setup",
      "No embedded browsers available — all browsers are busy. Please try again.",
    );
    return false;
  }
  ...
  try {
    ...
    const loginDetection = await detectLoginRequired(baseUrl, eb.cdpUrl);

    if (loginDetection.detectionError) {
      console.warn(
        `[play-agent] env_setup login detection error: ${loginDetection.detectionError}`,
      );
    }
    ...
    await updateSubsteps(sessionId, "env_setup", [
      ...
      {
        label: "Detecting login",
        status: "done",
        detail: loginDetection.hasRegisterLink
          ? "Register + Login"
          : "Login required",
      },
    ...
    if (!scriptResult.success || !scriptResult.code) {
      ...
      await setStepFailed(
        sessionId,
        "env_setup",
        scriptResult.error || "Failed to generate login script",
      );
      return false;
    }
    ...
    const testResult = await testLoginScript(
      scriptResult.code,
      baseUrl,
      repositoryId,
      eb.runnerId,
    ...
    if (!testResult.success) {
      ...
      await updateSubsteps(sessionId, "env_setup", [
        ...
        { label: "Detecting login", status: "done", detail: "Login required" },
        {
          label: "Testing login script",
          status: "error",
          detail: testResult.error?.slice(0, 80),
        },
      ]);

      const sessionAfter = await queries.getAgentSession(sessionId);
      if (sessionAfter) {
        await queries.updateAgentSession(sessionId, {
          metadata: { ...sessionAfter.metadata, loginScriptId: savedScript.id },
        });
      }
    ...
    await updateSubsteps(sessionId, "env_setup", [
      ...
      },
      {
        label: "Detecting login",
        status: "done",
        detail: loginDetection.hasRegisterLink ? "Register + Login" : "Login",
      },
      {
        label: "Login setup added",
        status: "done",
        detail: `${testResult.duration}ms`,
      },
    ]);
...
async function runScanAndTemplate(
  sessionId: string,
  repositoryId: string,
  _teamId: string,
  signal: AbortSignal,
  ...
  await updateSubsteps(sessionId, "scan_and_template", [
    {
      ...
      status: "done",
      detail: `${scanResult.routesFound} routes found`,
    },
  ...
  await updateSubsteps(sessionId, "scan_and_template", [
    ...
    { label: "Analyzing codebase", status: "done", detail: intelDetail },
    {
      ...
      status: "done",
      detail: `${templateId} (${classification.confidence}%)`,
    },
  ]);
  ...
  const richResult: AgentStepRichResult = {
    ...
    intelligence: intelligence
      ? buildIntelligenceBrief(intelligence, scanResult.routesFound || 0)
      : undefined,
  };
...
async function runPlanWithAgents(
  ...
  const intelligence = sessionData?.metadata?.codebaseIntelligence as
    | CodebaseIntelligenceContext
    | undefined;
  ...
  const plannerStates: SubstepState[] = [
    {
      label: "Coordinating multi-planner pipeline",
      status: "running",
      agent: "orchestrator",
    },
    {
      label: "Scanning codebase routes",
      status: "pending",
      agent: "planner",
      source: "code",
    },
    {
      label: "Checking route coverage",
      status: "pending",
      agent: "planner",
      source: "routes",
    },
    {
      label: "Analyzing spec files",
      status: "pending",
      agent: "planner",
      source: "spec",
    },
    {
      label: "Classifying areas",
      status: "pending",
      agent: "scout",
      source: "browser-scout",
    },
  ];

  const flushSubsteps = () =>
    updateSubsteps(sessionId, "plan", [...plannerStates]);
  await flushSubsteps();
  ...
  const enrichSubstep = (
    ...
    plannerStates[idx].outputSummary =
  ...
  emitActivity(
    ...
    sessionId,
    "substep:update",
    "Planner scanning codebase routes",
    { stepId: "plan", agentType: "planner" },
  );
  emitActivity(
    teamId,
    repositoryId,
    sessionId,
    "substep:update",
    "Planner checking route coverage",
    { stepId: "plan", agentType: "planner" },
  );
  emitActivity(
    teamId,
    ...
    sessionId,
    "substep:update",
    "Planner analyzing spec files via GitHub API",
    { stepId: "plan", agentType: "planner" },
  );

  const specPromise = runSpecPlanner(repositoryId, branch)
    .then((r) => {
      enrichSubstep(3, r);
      flushSubsteps();
      emitActivity(
        ...
        "substep:update",
        r.error
          ? `Planner spec analysis failed: ${r.error.slice(0, 80)}`
          : `Planner found ${r.areas.length} areas from spec files`,
        {
          stepId: "plan",
          agentType: "planner",
          ...
          detail: {
            source: "spec",
            areasFound: r.areas.length,
            areas: r.areas.map((a) => a.name),
          },
        },
      );
      return r;
  ...
  const [codeResult, routeResult] = await Promise.all([
    runCodePlanner(repositoryId, branch, intelligence)
      .then((r) => {
        enrichSubstep(1, r);
        flushSubsteps();
        emitActivity(
          ...
          "substep:update",
          r.error
            ? `Planner codebase scan failed`
            : `Planner discovered ${r.areas.length} areas from codebase`,
          {
            stepId: "plan",
            agentType: "planner",
            durationMs: r.durationMs,
      ...
      .catch((err) => {
        plannerStates[1].status = "error";
        plannerStates[1].rawError = String(err);
        flushSubsteps();
        return {
    ...
    runRoutePlanner(repositoryId)
      ...
      .catch((err) => {
        plannerStates[2].status = "error";
        plannerStates[2].rawError = String(err);
        flushSubsteps();
        return {
  ...
  emitActivity(
    ...
    `Scout classifying ${otherAreas.length} areas from code+route+spec planners`,
    {
      stepId: "plan",
      agentType: "scout",
      detail: { inputAreaCount: otherAreas.length },
    },
  );
  ...
  const browserResult = await runBrowserPlanner(repositoryId, baseUrl, {
    ...
    signal,
    onLogCreated: (id) => {
      plannerStates[4].promptLogId = id;
    },
    onScoutComplete: (scout) => {
      const exploreAreas = scout.areas.filter(
        (a) => a.classification === "explore",
      );
      const skipCount = scout.areas.filter(
        (a) => a.classification === "skip",
      ).length;
      ...
      emitActivity(
        ...
        `Scout classified areas: ${skipCount} skip, ${exploreAreas.length} marked for deep-dive`,
        {
          ...
          detail: {
            skipCount,
            exploreCount: exploreAreas.length,
            exploreAreas: exploreAreas.map((a) => a.name),
          },
        },
      );
    ...
    onDeepDiveStart: (areaName) => {
      const idx = plannerStates.findIndex(
        (s) => s.source === `browser-dive-${areaName}`,
      );
      if (idx >= 0) {
    ...
    onDeepDiveComplete: (areaName, areasFound, durationMs, promptLogId) => {
      ...
      if (idx >= 0) {
        ...
        plannerStates[idx].detail =
          areasFound > 0 ? `${areasFound} areas` : "No areas found";
      ...
      emitActivity(
          ...
          : `Diver found no testable areas in ${areaName}`,
        {
          ...
          agentType: "diver",
          durationMs,
          detail: { areaName, areasFound },
        },
      );
    },
  }).catch((err) => {
    plannerStates[4].status = "error";
    plannerStates[4].rawError =
      err instanceof Error ? err.message : String(err);
    flushSubsteps();
    return {
  ...
  const allPlannerResults: Record<
    string,
    {
      source: string;
  ...
  emitActivity(
    ...
    `Orchestrator merged ${fulfilledResults.length} planner results into ${mergedAreas.length} areas`,
    {
      ...
      detail: {
        sourcesUsed,
        areasFound: mergedAreas.length,
        areas: mergedAreas.map((a) => a.name),
      },
    },
  );
  ...
  await saveRoutes(
    ...
    mergedAreas.map((a) => ({
      ...
      routes: a.routes
        ...
        .map((r) => ({
          path: r,
          type: (r.includes("[") || r.includes(":") ? "dynamic" : "static") as
            | "static"
            | "dynamic",
        })),
    })),
...
async function runReview(
  ...
  if (
    session.metadata.approvedAreaIds &&
    session.metadata.approvedAreaIds.length > 0
  ) {
  ...
  if (session.metadata.autoApproveReview) {
    ...
    const planRich = planStep?.richResult as
  ...
  if (
    planResult?.cached ||
    planResult?.method === "fallback" ||
    planResult?.method === "ai_scan_fallback" ||
    planResult?.method === "spec_discovery"
  ) {
    await setStepCompleted(sessionId, "review", {
      skipped: true,
      reason: "No plan to review",
    });
    return true;
  }
...
async function runGenerate(
  ...
  if (targetAreas.length > 0) {
    ...
    const flushGenerators = () =>
      updateSubsteps(sessionId, "generate", [...generatorStates]);

    for (
      let batch = 0;
      batch < workItems.length;
      batch += GENERATOR_CONCURRENCY
    ) {
      if (isAborted(signal)) return false;
        ...
        chunk.map(async ({ area, group }, chunkIdx) => {
          ...
          try {
            ...
            const combinedSignal = signal
              ...
              : timeoutSignal;
            const genResult = await agentCreateTest(
              repositoryId,
              {
            ...
            if (genResult.success && genResult.code) {
              ...
              const existingPlaceholder = areaTests.find(
                (t) =>
                  t.isPlaceholder &&
                  t.name.toLowerCase() === group.name.toLowerCase(),
              );
              ...
              try {
                ...
                const matchingSpec = areaSpecs.find(
                  (s) =>
                    !s.testId &&
                    s.title.toLowerCase() === group.name.toLowerCase(),
                );
                const codeHash = createHash("sha256")
                  .update(genResult.code)
                  .digest("hex");
                if (matchingSpec) {
  ...
  for (const area of targetAreas) {
    const areaTestIds = generatedTests
...
async function runTests(
  ...
  try {
    const buildResult = await createAndRunBuild(
      "manual",
      undefined,
      repositoryId,
    );
    ...
    if (!buildResult.buildId) {
      await setStepFailed(
        sessionId,
        "run_tests",
        "Build was queued — please wait and retry",
      );
      return false;
    }
    ...
    if (agentModeTests.length > 0) {
      await updateSubsteps(sessionId, "run_tests", [
        {
          label: `Running ${proceduralTests.length} procedural tests`,
          status: "running",
        },
        {
          label: `Running ${agentModeTests.length} agent-mode tests`,
          status: "pending",
          agent: "orchestrator",
        },
      ]);
    ...
    if (!summary) {
      ...
      await setStepFailed(
        sessionId,
        "run_tests",
        "Build not found after creation",
      );
      return false;
    }
...
async function runFixTests(
  ...
  if (!summary || summary.failedCount === 0) {
    await setStepCompleted(sessionId, "fix_tests", {
      skipped: true,
  ...
  if (failedResults.length === 0) {
    await setStepCompleted(sessionId, "fix_tests", {
      skipped: true,
      reason: "No test errors to fix",
    });
    return true;
  }

  const fixAttempts: Record<string, number> = {
    ...session.metadata.fixAttempts,
  };
  const codeHashes: Record<string, string[]> = {
    ...session.metadata.codeHashes,
  };
  let fixedCount = 0;
...

  {
    ...
    for (
      let batch = 0;
      batch < fixableResults.length;
      batch += HEALER_CONCURRENCY
    ) {
      if (isAborted(signal)) return false;
      ...
      await updateSubsteps(sessionId, "fix_tests", [
        {
          label: "Coordinating fix pipeline",
          status: "done",
          agent: "orchestrator",
        },
        {
          ...
          emitActivity(
            ...
            `Healer diagnosing "${testName}" (attempt ${attempts + 1}/${MAX_FIX_ATTEMPTS})`,
            {
              ...
              detail: {
                ...
                testName,
                attempt: attempts + 1,
                error: result.errorMessage?.slice(0, 120),
              },
            },
          );
            ...
            if (hashes.includes(newHash)) {
              emitActivity(
                ...
                `Healer detected loop for "${testName}" — marking unfixable`,
                {
                  stepId: "fix_tests",
                  agentType: "healer",
                  detail: { testId, testName },
                },
              );
              fixes.push({
                testName: test?.name || testId,
                originalError: result.errorMessage!,
                fixed: false,
              });
...
async function runRerunTests(
  ...
  try {
    ...
    if (error instanceof Error && error.message === "Aborted") return false;
    const message =
      error instanceof Error ? error.message : "Failed to re-run tests";
    ...
    for (let i = startIdx; i < STEP_ORDER.length; i++) {
      ...
      if (success) {
        ...
        if (completedStep?.result) {
          ...
          if (
            (step.id === "run_tests" || step.id === "rerun_tests") &&
            completedStep.result.buildId
          ) {
            emitActivity(
              teamId,
              repositoryId,
        ...
        if (stepState?.status === "failed") {
          emitActivity(
            ...
            sessionId,
            "step:error",
            `${stepLabel} failed: ${stepState.error || "unknown"}`,
            {
              stepId: step.id,
              durationMs: Date.now() - stepStart,
            },
...
export async function startPlayAgent(
  repositoryId: string,
  ...
  emitActivity(
    ...
    session.id,
    "session:start",
    "Play Agent session started",
  );

  // Fire-and-forget: run steps
  executeFromStep(session.id, repositoryId, teamId, "settings_check").catch(
    (err) => {
...
export async function resumePlayAgent(
  sessionId: string,
  ...
  const waitingStep = session.steps.find(
    (s) => s.status === "waiting_user" || s.status === "failed",
  );
...
export async function pausePlayAgent(
  sessionId: string,
): Promise<{ success: boolean }> {
  const { team } = await requireTeamAccess();
  ...
  if (activeStep) {
    await updateStep(sessionId, activeStep.id, {
      status: "waiting_user",
      userAction: "paused",
    });
  }
...
export async function cancelPlayAgent(
  sessionId: string,
): Promise<{ success: boolean }> {
  const { team } = await requireTeamAccess();
...
export async function approvePlayAgentPlan(
  ...
  if (
    session.metadata?.approvedAreaIds &&
    reviewStep?.status !== "waiting_user"
  ) {
    return { success: true };
  }
...
export async function skipSettingsStep(
  sessionId: string,
): Promise<{ success: boolean }> {
  const { team } = await requireTeamAccess();
  const session = await queries.getAgentSession(sessionId);
  ...
  const highlights =
    (waitingStep?.result?.highlight as string[] | undefined) ?? [];
...
export async function skipDiscoverStep(
  sessionId: string,
): Promise<{ success: boolean }> {
  const { team } = await requireTeamAccess();
  const session = await queries.getAgentSession(sessionId);
...
export async function rerunPlanner(
  ...
  if (
    !planStep ||
    (planStep.status !== "completed" && planStep.status !== "failed")
  ) {
  ...
  const baseUrl = envConfig?.baseUrl || "http://localhost:3000";
  const intelligence = session.metadata?.codebaseIntelligence as
    | CodebaseIntelligenceContext
    | undefined;
  ...
  if (preIdx >= 0) {
    ...
    preSubsteps.push({
      label: isDiver
        ? `Deep-dive: ${plannerSource.replace("browser-dive-", "")}`
        : plannerSource,
      status: "running",
    ...
    if (plannerSource.startsWith("browser-dive-")) {
      ...
      switch (plannerSource) {
        ...
        default:
          return {
            success: false,
            error: `Unknown planner source: ${plannerSource}`,
          };
      }
    }
  } catch (error) {
  ...
  const plannerResults = {
    ...((freshSession.metadata.plannerResults as Record<string, unknown>) ||
      {}),
  };
  ...
  const existingRerunRoutes = await queries.getRoutesByRepo(
    session.repositoryId,
  );
  ...
  await updateStep(sessionId, "plan", {
    ...
    result: {
      method: "pw_agents_parallel",
      areasFound: mergedAreas.length,
      routesFound: mergedAreas.reduce((s, a) => s + a.routes.length, 0),
    },
    richResult,
    substeps,
  });
