 ...
 * browser work (loop frontier → goto → extract → add) on pages **core**
 * claimed, one per explorer.
 *
 * Two things this file used to do and no longer may (R4): connect to each EB
 * over CDP itself, and inject a storage state blob into a pod by address. Both
 * are core's now — the caller opens one `ctx.browser.withBrowser` scope per
 * explorer and passes `storageStateId`, so the credential material never
 * reaches this package. See `../actions.ts`'s `runQaDiscoverSwarm`.
 */
...
export interface ExploreTargetAppOptions {
  /**
   * One core-issued page per explorer. Index = explorer index. Each comes from
   * its own `withBrowser` scope, so core claims, meters, deadlines and releases
   * every one of them independently — a failing explorer cannot strand a pod.
   */
  pages: QaPage[];
  targetUrl: string;
  strategy: ExploreStrategy;
  ...
  deadline: number;
  /**
   * True when the caller claimed every page with a `storageStateId`, so core
   * already injected the session before this ran. The flag exists only so the
   * blocked-detection heuristic below knows the run is authenticated; the
   * plugin never holds the state itself.
   */
  storageStateInjected?: boolean;
...
export async function exploreTargetApp(opts: ExploreTargetAppOptions): Promise<{
  ...
  const authed = Boolean(opts.storageStateInjected || opts.credentials);
  ...
  await Promise.all(opts.pages.map((page, i) => runExplorer(i, page)));
