...
import { ReplayPlayer as ShareVideoPlayer } from "@lastest/ui";
import {
  resolveStepSegments,
  isInteractivePlaybackEnabled,
...
import { isValidShareSlug, buildShareUrl } from "../slug";
...
import { projectShareA11y, type ShareA11ySummary } from "../a11y-projection";
import { buildXrayElements, buildXrayFromDomDiff } from "../xray";
import type { XrayElement } from "../xray";
import { ShareWcagPanel } from "./share-wcag-panel";
import { MobileDiffGallery } from "./mobile-diff-gallery";
import { ChapterRail, type Chapter } from "./chapter-rail";
import { DomOverlay } from "./dom-overlay-client";
...
import { SocialShareKit, type ShareSlide } from "./social-share-kit-client";
import { buildSocialCopy } from "../social-copy";

// Dynamic — share content is live and render is cheap (pure server HTML).
export const revalidate = 0;

type Build = ShareBuild;

export interface PageProps {
  params: Promise<{ slug: string }>;
  /**
   * `<AwardBadgeRow>` lives in `src/components/awards/`, which this plugin
   * cannot import (`plugin-migration-recipe.md` §6: "app UI a plugin cannot
   * import goes down as a prop" — the same route `app-map` took for its
   * live-progress panel). The app's thin wrapper
   * (`src/app/(public)/r/[slug]/page.tsx`) supplies it; the plugin only
   * decides *whether* to render it.
   */
  awardBadgeRow?: ComponentType<{ award: RepoAward }>;
}
...
export async function generateMetadata({
  ...
  if (!rendered) return { title: "Share removed" };
  const { build, test } = rendered;

  const isTestShare = !!share.testId;
  const domain = share.targetDomain || test?.name || "this site";
  ...
  const description = buildShareDescription({
    ...
    runAt: build.completedAt ?? build.createdAt ?? null,
  });
  ...
  const ogAlt = build.changesDetected
    ? `${build.changesDetected} visual changes on ${domain}`
...
export default async function PublicSharePage({
  ...
  const a11ySummary: ShareA11ySummary = projectShareA11y(
    await host.getBuildA11yViolations(build.id),
  );
  ...
  const { notes: demoNotes, buildCaptions } = await host.getDemoNotes(
    build.id,
    repoIdForNotes,
  );

  // Subtitle track for the recording. Captions are time-coded to THIS build's
  // recording — `host.getDemoNotes` reads them from the build's OWN notes,
  // not the repo-latest `demoNotes` (which feeds the prose panel and may
  // belong to a sibling build whose video has different step timing). Only
  // emit a <track> when captions exist; absent captions → no track → the
  ...
  const award: RepoAward | null = repoIdForNotes
    ? ((await host.getRepoAward(repoIdForNotes)) ?? null)
    : null;
  ...
  // counts never read as embarrassing. "Products tested" is this plugin's own
  // aggregate; "test runs completed" is platform-wide and core's.
  const shareStats = {
    productsTested: await queries.countDistinctPublicTargetDomains(),
    testRunsCompleted: (await host.getPlatformStats()).testRunsCompleted,
  };
  ...
  return (
    <div className="min-h-screen bg-background text-foreground">
      ...
      <main className="mx-auto max-w-5xl px-4 sm:px-6 py-10 space-y-8">
        ...
        {showAwardBadges && award && AwardBadgeRow && (
          <AwardBadgeRow award={award} />
        )}
