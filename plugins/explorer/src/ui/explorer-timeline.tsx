...
import { Badge, cn } from "@lastest/ui";
...
function StepIcon({ status }: { status: ExplorerStepState["status"] }) {
  switch (status) {
    case "completed":
...
function StepRow({ step }: { step: ExplorerStepState }) {
  return (
...
export function ExplorerTimeline({ steps }: { steps: ExplorerStepState[] }) {
  const prefix = steps.filter((s) => s.iteration === undefined);
  const loops = new Map<number, ExplorerStepState[]>();
