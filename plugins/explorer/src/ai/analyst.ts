import type { AiCapability } from "@lastest/contracts";
...
export async function clusterFindings(
  ...
  try {
    parsed = await generateJson(ai, {
      prompt: `FINDINGS (${input.findings.length}):\n${digest}\n\nCluster by root cause. JSON only.`,
      systemPrompt: ANALYST_SYSTEM_PROMPT,
      actionType: "explorer_analyze",
      tier: "balanced",
      isValid: isAnalystOutput,
      source: "explorer-analyst",
      repositoryId: input.repositoryId,
      signal: input.signal,
    });
  } catch {
    // Out of AI budget, provider down, run cancelled — all the same here. The
    // findings are already persisted; clustering is a presentation nicety and
    // must not be able to fail the run that produced them.
    parsed = null;
  }
