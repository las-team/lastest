...
import { CardBadge, Pill, SplitShield, Wordmark } from "./badges";
import { DeltaMark } from "./delta-mark";
import { EmbedCodeBlock } from "./embed-code-block";

/**
 * The `/awards` marketing/criteria landing page — "Prove your app is not AI
 * slop". Entirely this feature's own content (no session, no repo, no core
 * read at all), so the app's `src/app/(public)/awards/page.tsx` keeps only
 * the route-segment config (`revalidate`, `metadata`) that Next.js requires
 * to be literal in the route file itself — see that file for why it is not
 * re-exported across the package boundary the way this component is.
 */
