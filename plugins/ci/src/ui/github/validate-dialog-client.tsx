...
import { validateGithubActionSetup } from "../../actions";
import type { ValidationResult, ValidationCheckStatus } from "../../types";
...
export function ValidateDialog({
  ...
  const checks = results
    ? (Object.keys(CHECK_LABELS) as (keyof ValidationResult)[])
    : [];
        ...
        <div className="space-y-3">
          {validating && !results && (
            <div className="rounded-md bg-muted p-3 space-y-2">
              {(Object.keys(CHECK_LABELS) as (keyof ValidationResult)[]).map(
                (key) => (
