/**
 * GitLab REST client — Repo Files (`.gitlab-ci.yml`), project variables,
 * project hooks, pipeline schedules, and the latest-pipeline read the
 * validation panel uses.
 *
 * Moved verbatim from `src/lib/gitlab/pipelines.ts`. Takes both the token and
 * the instance URL as arguments (self-managed GitLab is not gitlab.com), and
 * resolves neither — see `providers/github.ts` for the same note.
 */
// No libsodium here, unlike the GitHub side: GitLab variable values are sent
// in plaintext over HTTPS rather than sealed to a repo public key.
