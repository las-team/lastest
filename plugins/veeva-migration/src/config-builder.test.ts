...
import type { ConnectorLike } from "./connector-shapes";
import type { MigrationWave } from "./schema";

function connector(over: Record<string, unknown> = {}): ConnectorLike {
  return {
    id: "c1",
    repositoryId: "r1",
    environmentId: "e1",
...
function input(over: Partial<BuildConfigInput> = {}): BuildConfigInput {
  return {
    ...
    target: vault,
    waves: [wave()],
    // The composition root supplies these; core's own constants are the source
    // of truth (see connector-shapes.ts). Pinned here so a change to core's
    // defaults cannot silently change what this test asserts.
    apiDefaults: { salesforceApiVersion: "v62.0", vaultApiVersion: "v25.1" },
    ...over,
  };
}
...
describe("buildConfigSkeleton", () => {
  ...
  it("passes a history window through when there is no cutoff date", () => {
    const cfg = buildConfigSkeleton(
      input({ project: { config: { historyMonths: 36 } } }),
    );
