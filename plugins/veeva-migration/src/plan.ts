...
/**
 * A console shim, not pino.
 *
 * `@/lib/logger` is server-only core and a plugin cannot import it.
 * `src/lib/logger-console-bridge.ts` lifts a leading `[Prefix]` back onto
 * pino's `scope` in production, so the structured output survives the hop.
 */
const log = {
  warn: (detail: unknown, message?: string) =>
    console.warn(`[Migration] ${message ?? ""}`, detail),
};
...
export async function buildPlanPreview(
  ...
  try {
    ...
    const plan = await buildPlan({
      mode: "init",
      config,
      // Planning is pure: it reads the config and the object registry and
      // touches neither the network, the state store nor the filesystem. The
      // context is required by the type, so it is filled with values that say
      // so rather than with a tenant's real ones.
      context: {
        tenantKey: "plan-preview",
        actor: "plan-preview",
        runDir: "/nonexistent/plan-preview",
      },
      wave: opts.wave,
