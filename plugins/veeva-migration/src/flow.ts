 ...
 * a database.
 *
 * The nine stages mirror `libs/veeva-migration/demo/` — the screencast IS
...
export interface StageDefinition {
  ...
  /**
   * What this stage runs, spelled as the engine's own mode and flags.
   *
   * Documentation, not an operator path: the CLI is a development harness with
   * no `bin` entry, and its file-backed state store cannot reach the
   * application database (see `libs/veeva-migration/src/cli.ts`). The console
   * is the only supported way to run a migration.
   */
  command: string | null;
