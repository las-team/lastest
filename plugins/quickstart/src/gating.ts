import type { QuickstartHost, QuickstartRepoGateInfo } from "./host";
 ...
 */
export function pickRepoBaseUrl(
  repo: QuickstartRepoGateInfo,
): string | undefined {
  ...
  // write-once fallback that went stale and shadowed real branch URLs.
...
export async function isQuickstartEnabled(
  ...
  if (!baseUrl)
    return { enabled: false, reason: "no_base_url", repo, teamId: repo.teamId };

  return { enabled: true, repo, teamId: repo.teamId, baseUrl };
}
