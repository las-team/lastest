...
async function migrate() {
  ...
  const allRepos = await db
    .select()
    .from(repositories)
    .where(isNull(repositories.teamId))
    .all();
  ...
  const allGithubAccounts = await db
    .select()
    .from(githubAccounts)
    .where(isNull(githubAccounts.teamId))
    .all();
  ...
  console.log(
    `✓ Updated ${allGithubAccounts.length} GitHub accounts with teamId`,
  );
  ...
  const pendingInvitations = await db
    ...
    .where(isNull(userInvitations.teamId))
    .all();
  ...
  console.log(
    `✓ Updated ${pendingInvitations.length} pending invitations with teamId`,
  );
