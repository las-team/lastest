...
interface Args { sources: SourceName[]; out: string | null; limit: number; }
...
function parseArgs(): Args {
  ...
  for (const raw of process.argv.slice(2)) {
    ...
    switch (key) {
      ...
      case '--help':
      case '-h':
        console.log('Usage: pnpm tsx scripts/audit-playwright-parity.ts [--source=local|olares|both] [--out=PATH] [--limit=N]');
        process.exit(0);
      default:
        console.error(`Unknown arg: ${raw}`); process.exit(2);
    }
  }
  return a;
}
...
function makeClient(source: SourceName) {
  return postgres(connectionUrlFor(source), { max: 4, prepare: false, onnotice: () => {} });
}

// ─── Today's pipeline (the prior body extractor — kept verbatim) ─────────
const LEGACY_TEST_RE = /export\s+async\s+function\s+test\s*\(\s*page[^)]*\)\s*\{([\s\S]*)\}\s*$/;
...
const PRIOR_SHIM_MATCHERS = new Set([
  ...
  'toBeGreaterThan', 'toBeGreaterThanOrEqual', 'toBeLessThan', 'toMatch',
  'toHaveURL', 'toHaveTitle',
  'toBeVisible', 'toBeHidden', 'toHaveText', 'toContainText',
]);

const ADDED_MATCHERS = new Set([
  'toBeAttached', 'toBeEnabled', 'toBeDisabled', 'toBeChecked', 'toBeFocused',
  'toBeEditable', 'toBeEmpty', 'toBeInViewport',
  'toHaveValue', 'toHaveAttribute', 'toHaveCount', 'toHaveClass', 'toHaveCSS',
  'toHaveJSProperty', 'toHaveRole',
  'toStrictEqual', 'toBeNull', 'toBeUndefined', 'toBeNaN', 'toBeCloseTo',
  'toBeInstanceOf', 'toBeLessThanOrEqual', 'toHaveScreenshot',
]);

const HELPER_REFS = ['fileUpload', 'clipboard', 'network.', 'fixtures', 'replayCursorPath', 'locateWithFallback', 'stepLogger'] as const;
...
function scanBody(body: string): BodyScan {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (!t.endsWith(';') && !t.endsWith('}') && !t.endsWith(')')) { multilineAwait = true; break; }
    if (t.endsWith('.') || t.endsWith(',') || t.endsWith('+')) { multilineAwait = true; break; }
  }

  const hasTopLevelImport = /^\s*import\s+[\w{*,\s]+from\s+['"]/m.test(body);
  const hasFrameworkTestCall = /\btest(?:\.(?:only|skip|fixme))?\s*\(\s*['"`][^'"`]*['"`]\s*,(?:\s*\{[^}]*\}\s*,)?\s*async\s*\(\s*\{/.test(body);
...
interface AuditRow {
  ...
  helpersUsed: string[];
  flags: { multilineAwait: boolean; topLevelImport: boolean; frameworkTestCall: boolean };
  verdict: Verdict;
  reason: string;
}

function audit(row: { id: string; code: string; name: string; repoName: string; areaName: string | null; source: SourceName }): AuditRow {
  const code = row.code ?? '';
  ...
  const newBodyStripped = (() => { try { return stripTypeAnnotations(newExt.body); } catch { return newExt.body; } })();
  ...
  const matchersNewlySupported = scan.matchers.filter((m) => ADDED_MATCHERS.has(m) && !PRIOR_SHIM_MATCHERS.has(m));
  ...
  const stillBroken = !extractedNewOK && matchersStillMissing.length === 0 && newExt.shape === 'whole-code';
  ...
  } else if (extractedTodayOK && extractedNewOK && matchersStillMissing.length === 0) {
    verdict = 'GREEN';
...
function renderMarkdown(rows: AuditRow[]): string {
  ...
  lines.push('|---|---|---|');
  lines.push(`| GREEN | ${fmtCount(rows, 'GREEN')} | extracts identically under both pipelines, no new matchers needed |`);
  lines.push(`| YELLOW | ${fmtCount(rows, 'YELLOW')} | now extracts or now uses a newly-supported matcher (net gain) |`);
  lines.push(`| RED | ${fmtCount(rows, 'RED')} | still unsupported after this PR (known gap, not a regression) |`);
  lines.push(`| BLOCKER | ${fmtCount(rows, 'BLOCKER')} | extracts today but NOT under new pipeline (regression — HOLD) |`);
  lines.push('');
  ...
  if (blockers.length > 0) {
    ...
    for (const b of blockers) {
      lines.push(`- \`${b.source}\` test ${b.id.slice(0, 8)}… (${b.repoName}/${b.name}) — ${b.reason}`);
    }
    lines.push('');
  }
  ...
  for (const [name, n] of topByCount(allMatchers, 15)) {
    const tag = ADDED_MATCHERS.has(name) && !PRIOR_SHIM_MATCHERS.has(name) ? ' **(new)**'
              : !PRIOR_SHIM_MATCHERS.has(name) && !ADDED_MATCHERS.has(name) ? ' **(still missing)**'
              : '';
  ...
  if (missingAll.length === 0) lines.push('- (no unsupported matchers detected)');
  lines.push('');

  lines.push('## Pattern flags (counts)');
  lines.push('');
  lines.push(`- Top-level \`import\` lines (stripped in both runners now): ${rows.filter((r) => r.flags.topLevelImport).length}`);
  lines.push(`- Framework-style \`test('name', async ({ page }) => …)\` bodies: ${rows.filter((r) => r.flags.frameworkTestCall).length}`);
  lines.push(`- Multi-line \`await\` statements (deferred soft-wrap fix, see plan): ${rows.filter((r) => r.flags.multilineAwait).length}`);
  lines.push('');

  lines.push('## Per-test table (first 200)');
  lines.push('');
  lines.push('| Source | id | repo / name | bytes | today→new shape | verdict | reason |');
  lines.push('|---|---|---|---|---|---|---|');
  for (const r of rows.slice(0, 200)) {
    const shapeDelta = `${r.todayExtracted ? 'legacy' : 'fallback'} → ${r.newExtractShape}`;
    lines.push(`| ${r.source} | \`${r.id.slice(0, 8)}\` | ${escapeMd(r.repoName)} / ${escapeMd(r.name)} | ${r.bytes} | ${shapeDelta} | ${r.verdict} | ${escapeMd(r.reason)} |`);
  }
  if (rows.length > 200) lines.push('');
  if (rows.length > 200) lines.push(`_(${rows.length - 200} more rows truncated)_`);
...
// ─── Main ──────────────────────────────────────────────────────────────
async function auditSource(source: SourceName, limit: number): Promise<AuditRow[]> {
  ...
  try {
    ...
    `;
    const rows = (await query) as unknown as Array<{ id: string; code: string; name: string; repo_name: string; area_name: string | null }>;
    console.error(`[${source}] scanned ${rows.length} tests`);
    return rows.map((row) =>
      audit({ id: row.id, code: row.code, name: row.name, repoName: row.repo_name, areaName: row.area_name, source }),
    );
  } finally {
...
async function main() {
  ...
  for (const src of args.sources) {
    try {
      ...
      console.error(`[${src}] audit failed:`, err instanceof Error ? err.message : err);
