  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (elementCommentMatch) {
      ...
      for (let j = i + 1; j < Math.min(i + 10, lines.length); j++) {
        ...
        if (sm) {
          try {
            const arr = JSON.parse(sm[1]) as Array<{
              type: string;
              value: string;
            }>;
            if (arr && arr.length > 0)
              targetSelector = `${arr[0].type}: ${arr[0].value}`;
          } catch {
            /* ignore */
          }
        }
    ...
    if (downloadCommentMatch) {
      ...
      ids.push(
        oldAssertionId(orderIndex, "fileDownloaded", "download", filename),
      );
      orderIndex++;
      continue;
    }
 ...
 * unchanged.
 */
function rewriteStepCriteria(
  criteria: StepCriterion[] | null,
  ...
  for (const c of criteria) {
    ...
    for (const r of c.rules) {
      if (r.kind !== "assertion_failed") {
        newRules.push(r);
        continue;
      }
      const oldId = (r.params as { assertionId?: string } | undefined)
        ?.assertionId;
      if (!oldId) {
        newRules.push(r);
        continue;
      ...
      newRules.push({
        ...r,
        params: { ...(r.params ?? {}), assertionId: newId },
      });
    }
...
async function migrate() {
  ...
  for (const t of allTests) {
    ...
    if (!idMap) {
      ...
      const assertionsChanged =
        JSON.stringify(t.assertions ?? []) !== JSON.stringify(newAssertions);
      if (assertionsChanged) {
        await db
          .update(tests)
          .set({ assertions: newAssertions })
  ...
  console.log(
    `✓ Tests updated: ${testsUpdated} (${testsSkipped} skipped — no code, ${testsParserMismatch} parser-count mismatch — assertions only)`,
  );
  ...
  for (const v of allVersions) {
    ...
    const idMap = buildIdMap(
      v.code,
      versionAssertions.map((a) => a.id),
    );
    if (!idMap) continue;
    const newCriteria = rewriteStepCriteria(v.stepCriteria, idMap);
    if (JSON.stringify(v.stepCriteria) === JSON.stringify(newCriteria))
      continue;
    await db
      .update(testVersions)
