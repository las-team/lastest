...
const WEBSITE_IDS = (
  process.env.UMAMI_WEBSITE_IDS ||
  process.env.UMAMI_WEBSITE_ID ||
  DEFAULT_WEBSITE_IDS.join(",")
)
  .split(",")
  .map((s) => s.trim())
  .filter(Boolean);
...
if (!TOKEN && !(USER && PASS))
  die("Provide UMAMI_TOKEN or both UMAMI_USER + UMAMI_PASS");
...
const FUNNELS = [
  {
    ...
    description:
      "Day-1 activation via the recorder path: signup → repo → test_recorded → run started → run completed.",
    steps: [
  ...
  },
  {
    ...
    description:
      "Day-1 activation via the AI-create-test path: signup → repo → test_created → run started → run completed.",
    steps: [
      { type: "event", value: "signup_completed" },
      { type: "event", value: "repo_linked" },
  ...
  },
  {
    ...
    description:
      "Compact 3-step funnel for early activation, useful at low traffic where 5-step funnels rarely complete.",
    steps: [
      { type: "event", value: "signup_completed" },
      { type: "event", value: "repo_linked" },
      { type: "event", value: "test_run_started" },
...
const JOURNEY = {
  ...
  description:
    "Free-form path users take from signup_completed to diff_approved (5 intermediate nodes).",
  parameters: {
    startDate: fmt(START),
    endDate: fmt(END),
    steps: 5,
...
const RETENTION = {
  ...
  description:
    "Daily cohorts grouped by first visit; columns show return rate by day.",
  parameters: {
    startDate: fmt(START),
    endDate: fmt(END),
  },
...
async function main() {
  ...
  for (const websiteId of WEBSITE_IDS) {
    ...
    log(
      `Existing: ${existing.size === 0 ? "(none)" : `${existing.size} reports`}`,
    );
    ...
    await upsert(existing, JOURNEY.name, {
      websiteId,
      type: "journey",
      ...JOURNEY,
    });
    await upsert(existing, RETENTION.name, {
      websiteId,
      type: "retention",
      ...RETENTION,
    });
...
async function getExistingReports(websiteId) {
  const r = await fetch(
    `${URL_BASE}/api/reports?websiteId=${websiteId}&pageSize=200`,
    {
