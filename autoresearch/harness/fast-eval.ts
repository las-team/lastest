...
export async function evalRouteAccuracy(
  ...
  if (failures.length === 0) {
    return {
      ...
      details: [
        {
          label: "no_404_failures",
          passed: true,
          reason: "No 404 failures found",
        },
      ],
  ...
  console.error(
    `[route-accuracy] Regenerating ${failures.length} failed tests...`,
  );
  ...
  for (const f of failures) {
    ...
    try {
      ...
      details.push({
        ...
        passed: allValid,
        reason: allValid
          ? "All URLs match known routes"
          : `Invalid URLs: ${gotoUrls.join(", ")}`,
      });
    } catch (e) {
      details.push({
        label,
    ...
    try {
      ...
      if (hasImports) {
        details.push({
          label,
          passed: false,
          reason: "Contains import statements",
        });
        continue;
      }
      ...
      if (!hasSignature) {
        details.push({
          label,
          passed: false,
          reason: "Missing test function signature",
        });
        continue;
      }
      ...
      details.push({
        ...
        reason: hasScreenshot
          ? "Valid with screenshot"
          : "Valid but no screenshot call",
      });
    } catch (e) {
      details.push({
        label,
...
export async function evalAuthResilience(
  ...
  const authFailures = await getFailedTests(
    repositoryId,
    "auth_redirect",
    SAMPLE_SIZE,
  );
  ...
  if (authFailures.length === 0) {
    return {
      ...
      details: [
        {
          label: "no_auth_failures",
          passed: true,
          reason: "No auth redirect failures",
        },
      ],
      duration_ms: Date.now() - start,
    };
  ...
  const score =
    details.filter((d) => d.passed).length / Math.max(details.length, 1);
  return {
    track: "auth-resilience",
    score,
    details,
...
export async function evalFixLoop(
  ...
  const failures = await getFailedTests(
    repositoryId,
    "selector_timeout",
    SAMPLE_SIZE,
  );
    ...
    try {
      ...
      if (syntaxErr) {
        details.push({
          label,
          passed: false,
          reason: `Fix syntax error: ${syntaxErr}`,
        });
        continue;
      }
      ...
      if (!isDifferent) {
        details.push({
          label,
          passed: false,
          reason: "Fix produced identical code",
        });
        continue;
      }
      ...
      details.push({
        ...
        reason: allValid
          ? "Valid fix"
          : `Fix introduced hallucinated route: ${gotoUrls.join(", ")}`,
      });
    } catch (e) {
      details.push({
        label,
...
if (process.argv[1]?.includes("fast-eval")) {
  ...
  if (!trackArg || !repoIdArg) {
    console.error(
      "Usage: pnpm tsx autoresearch/harness/fast-eval.ts --track=<name> --repo-id=<id>",
    );
    console.error(
      "Tracks: route-accuracy, test-generation, auth-resilience, fix-loop",
    );
    process.exit(1);
  }
  ...
  const evaluators: Record<
    string,
    (repoId: string) => Promise<TrackEvalResult>
  > = {
  ...
  evaluator(repositoryId)
    .then((result) => {
      ...
      console.log(
        `passed:     ${result.details.filter((d) => d.passed).length}`,
      );
      console.log(
        `failed:     ${result.details.filter((d) => !d.passed).length}`,
      );
      console.log("---");
      for (const d of result.details) {
        console.log(
          `${d.passed ? "PASS" : "FAIL"} ${d.label}${d.reason ? ` | ${d.reason}` : ""}`,
        );
      }
    })
    .catch((e) => {
