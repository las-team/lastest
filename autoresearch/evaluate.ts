...
function createExpect(_timeout = 5000) {
  return function expect(_target: unknown, _message?: string) {
    throw new Error(
      "createExpect not available — local Playwright runner has been removed",
    );
  };
}
...
const AUTH_EMAIL =
  process.env.AUTH_EMAIL || "testuser1771664821751@example.com";
...
async function runTestCode(
  ...
  try {
    ...
    if (fs.existsSync(STORAGE_STATE_PATH))
      ctxOpts.storageState = STORAGE_STATE_PATH;
    ...
    if (!exportMatch) {
      return {
        passed: false,
        errorMessage: "Could not extract test function body",
      };
    }
    ...
    await Promise.race([
      ...
      new Promise((_, reject) =>
        setTimeout(
          () => reject(new Error(`Test timeout after ${TEST_TIMEOUT_MS}ms`)),
          TEST_TIMEOUT_MS,
        ),
      ),
    ]);
...
async function runGenerateMode(
  ...
  for (const route of GENERATE_ROUTES) {
    ...
    try {
      ...
      const { passed, errorMessage } = await runTestCode(
        code,
        route.path,
        browser,
      );
      console.error(
        `[gen:${route.path}] ${passed ? "PASSED" : "FAILED: " + errorMessage}`,
      );
      results.push({
        name: `gen:${route.path}`,
        route: route.path,
...
async function loadReplayScenarios(): Promise<ReplayScenario[]> {
  ...
  const [latestRun] = await db
    .select({ id: testRuns.id, repositoryId: testRuns.repositoryId })
    .from(testRuns)
...
async function runReplayMode(
  ...
  for (const scenario of scenarios) {
    ...
    try {
      ...
      console.error(
        `[replay:${label}] ${passed ? "PASSED" : "FAILED: " + (errorMessage || "").slice(0, 80)}`,
      );
      results.push({
        name: label,
        route: scenario.targetUrl,
...
async function main() {
  ...
  try {
    ...
    if (!res.ok && res.status !== 302 && res.status !== 307) {
      console.error(
        `ERROR: App not reachable at ${BASE_URL} (status ${res.status})`,
      );
      process.exit(1);
    }
  } catch {
  ...
  for (const r of results) {
    ...
    const errPart = r.errorMessage
      ? ` | ${r.errorMessage.split("\n")[0].slice(0, 80)}`
      : "";
    console.log(
      `${status} ${r.name} (${(r.durationMs / 1000).toFixed(1)}s)${errPart}`,
    );
  }
